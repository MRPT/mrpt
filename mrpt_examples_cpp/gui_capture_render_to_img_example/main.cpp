...
void TestDisplay3D()
{
  ...
  while (!end && win.isOpen())
  {
    ...
    obj1->setLocation(
        p1.x + cos(p1.y / 2) * 0.05, p1.y - sin(p1.x / 2) * 0.09, p1.z - sin(p1.x / 2) * 0.08);

    auto obj2 = scene->getByName("ball_2");
    const auto p2 = obj2->getPose();
    obj2->setLocation(
        p2.x + cos(p2.y / 2) * 0.05, p2.y - sin(p2.x / 2) * 0.09, p2.z - sin(p2.x / 2) * 0.08);

    win.addTextMessage(
        0.02, 0.98, mrpt::format("ball#1 pos: %s ", p1.translation().asString().c_str()),
        10,  // An arbitrary ID
        fp);
    ...
    // Grab frame:
    mrpt::img::CImage::ConstPtr img = win.getLastWindowImagePtr();
    if (img)
    {
      static int i = 0;
