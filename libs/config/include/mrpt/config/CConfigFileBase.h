...
class CConfigFileBase
{
 ...
 protected:
  ...
  virtual void writeString(
      const std::string& section, const std::string& name, const std::string& str) = 0;
 ...
 public:
  ...
  {
    this->write(
        section, name, mrpt::typemeta::TEnumType<enum_t>::value2name(value), name_padding_width,
  ...
  {
    writeString(
        section, name, mrpt::to_string(value), name_padding_width, value_padding_width, comment);
  }
  ...
  {
    std::string s;
    for (typename std::vector<data_t>::const_iterator it = value.begin(); it != value.end(); ++it)
    {
      s += mrpt::to_string(*it);
      s += " ";
    }
    writeString(section, name, s, name_padding_width, value_padding_width, comment);
  }
  void write(
      const std::string& section,
      const std::string& name,
      ...
      bool failIfNotFound = false) const
  {
    ...
    else
    {
      ...
      for (size_t i = 0; i < N; i++)
      {
        ...
        outValues[i] = static_cast<typename VECTOR_TYPE::value_type>(val);
      }
    }
  }
...
#define MRPT_LOAD_CONFIG_VAR(variableName, variableType, configFileObject, sectionNameStr) \
  {                                                                                        \
    variableName =                                                                         \
        configFileObject.read_##variableType(sectionNameStr, #variableName, variableName); \
  }
...
#define MRPT_LOAD_CONFIG_VAR_DEGREES(variableName, configFileObject, sectionNameStr)               \
  {                                                                                                \
    variableName = mrpt::DEG2RAD(                                                                  \
        configFileObject.read_double(sectionNameStr, #variableName, mrpt::RAD2DEG(variableName))); \
  }
#define MRPT_LOAD_CONFIG_VAR_DEGREESf(variableName, configFileObject, sectionNameStr)             \
  {                                                                                               \
    variableName = mrpt::DEG2RAD(                                                                 \
        configFileObject.read_float(sectionNameStr, #variableName, mrpt::RAD2DEG(variableName))); \
  }
...
#define MRPT_LOAD_CONFIG_VAR_DEGREES_NO_DEFAULT(variableName, configFileObject, sectionNameStr) \
  {                                                                                             \
    variableName = mrpt::DEG2RAD(configFileObject.read_double(                                  \
        sectionNameStr, #variableName, mrpt::RAD2DEG(variableName), true));                     \
  }

#define MRPT_LOAD_CONFIG_VAR_CAST(                                                          \
    variableName, variableType, variableTypeCast, configFileObject, sectionNameStr)         \
  {                                                                                         \
  ...
  {                                                                                       \
    ...
    {                                                                                     \
      THROW_EXCEPTION(mrpt::format(                                                       \
          ...
          static_cast<const char*>(#variableName), std::string(sectionNameStr).c_str())); \
    }                                                                                     \
  }
...
#define MRPT_LOAD_CONFIG_VAR_NO_DEFAULT(                                                           \
  ...
  {                                                                                                \
    ...
    {                                                                                              \
      variableName =                                                                               \
          configFileObject.read_##variableType(sectionNameStr, #variableName, variableName, true); \
    }                                                                                              \
    catch (std::exception&)                                                                        \
    {                                                                                              \
      THROW_EXCEPTION(mrpt::format(                                                                \
...
#define MRPT_LOAD_CONFIG_VAR_CAST_NO_DEFAULT(                                             \
  ...
  {                                                                                       \
    try                                                                                   \
    {                                                                                     \
      variableName = static_cast<variableTypeCast>(configFileObject.read_##variableType(  \
...
#define MRPT_LOAD_HERE_CONFIG_VAR_CAST(                                             \
    variableName, variableType, variableTypeCast, targetVariable, configFileObject, \
    sectionNameStr)                                                                 \
  targetVariable = static_cast<variableTypeCast>(                                   \
      configFileObject.read_##variableType(sectionNameStr, #variableName, targetVariable));
...
#define MRPT_LOAD_HERE_CONFIG_VAR_CAST_NO_DEFAULT(                                         \
  ...
  {                                                                                        \
    try                                                                                    \
    {                                                                                      \
      targetVariable = static_cast<variableTypeCast>(configFileObject.read_##variableType( \
          sectionNameStr, #variableName, targetVariable, true));                           \
    }                                                                                      \
    catch (std::exception&)                                                                \
    {                                                                                      \
...
#define MRPT_SAVE_CONFIG_VAR_DEGREES(variableName, configFileObject, sectionNameStr)    \
  {                                                                                     \
    configFileObject.write(sectionNameStr, #variableName, mrpt::RAD2DEG(variableName)); \
  }

#define MRPT_SAVE_CONFIG_VAR_COMMENT(variableName, __comment)                   \
  {                                                                             \
    c.write(                                                                    \
        s, #variableName, variableName, mrpt::config::MRPT_SAVE_NAME_PADDING(), \
...
#define MRPT_SAVE_CONFIG_VAR_DEGREES_COMMENT(__entryName, __variable, __comment)           \
  {                                                                                        \
    c.write(                                                                               \
        s, __entryName, mrpt::RAD2DEG(__variable), mrpt::config::MRPT_SAVE_NAME_PADDING(), \
        mrpt::config::MRPT_SAVE_VALUE_PADDING(), __comment);                               \
  }
