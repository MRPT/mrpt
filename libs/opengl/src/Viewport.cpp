...
/*--------------------------------------------------------------
          insert
  ---------------------------------------------------------------*/
void Viewport::insert(const CRenderizable::Ptr& newObject) { m_objects.push_back(newObject); }
...
int sizeFromRatio(const int startCoord, const double dSize, const int iLength)
{
  ...
  {  // Negative numbers: Specify the right side coordinates
     // instead of
    // the width:
    if (dSize >= -1)
...
int startFromRatio(const double frac, const int dSize)
{
  ...
  const int L = fracAbs > 1 ? static_cast<int>(fracAbs) : static_cast<int>(dSize * fracAbs);
...
void Viewport::renderImageMode() const
{
...
#if defined(OPENGLVIEWPORT_ENABLE_TIMEPROFILING)
  mrpt::system::CTimeLoggerEntry tle(glv_timlog, "Viewport::render imageview");
#endif

  // Do we have an actual image to render?
  if (!m_imageViewPlane || m_imageViewPlane->getTextureImage().isEmpty()) return;
  ...
  _.pmv_matrix.asEigen() = _.p_matrix.asEigen() * _.v_matrix.asEigen() * _.m_matrix.asEigen();
  ...
  mrpt::opengl::enqueueForRendering(lst, _, rq, true /*skip cull*/, false /* isShadowMap*/);
...
void Viewport::renderNormalSceneMode(const CCamera& useThisCamera, bool is1stShadowMapPass) const
{
#if MRPT_HAS_OPENGL_GLUT || MRPT_HAS_EGL
  MRPT_START

#ifdef MRPT_OPENGL_PROFILER
  mrpt::system::CTimeLoggerEntry tle(opengl_profiler(), "Viewport.renderNormalSceneMode");
#endif
...
// Enable point sizes>1
  ...
  // pass 2: render, sorted by shader program:
  auto& shaders = m_shadowsEnabled ? (is1stShadowMapPass ? m_threadedData.get().shadersShadow1st
...
#ifdef MRPT_OPENGL_PROFILER
  opengl_profiler().registerUserMeasure("render.totalObjects", rqStats.numObjTotal);
  opengl_profiler().registerUserMeasure("render.numObjRendered", rqStats.numObjRendered);
  opengl_profiler().registerUserMeasure(
      "render.ObjRenderedPercent",
      rqStats.numObjTotal != 0 ? 100.0 * rqStats.numObjRendered / rqStats.numObjTotal : 0);
#endif
...
void Viewport::renderTextMessages() const
{
  ...
  for (auto& kv : m_2D_texts.messages)
  {
    ...
    float x = label.x >= 1 ? label.x : (label.x < 0 ? w + label.x : label.x * w);
    float y = label.y >= 1 ? label.y : (label.y < 0 ? h + label.y : label.y * h);
    ...
    if (CText::Ptr& o = label.gl_text_shadow; o)
    {
      ...
      // (-1,-1)-(+1,+1):
      o->setLocation(-1.0f + 2 * (x + 1) / w, -1.0f + 2 * (y - 1) / h, 0.1);
      objs.push_back(o);
    }
  ...
  // Pass 1: Process all objects (recursively for sets of objects):
  mrpt::opengl::RenderQueue rq;
  mrpt::opengl::enqueueForRendering(objs, _, rq, true /*skip cull*/, false /* isShadowMap*/);
    ...
    [[maybe_unused]] const CCamera* forceThisCamera) const
{
  ...
  {  // Clear color & depth buffers:
    ...
    glClearColor(
        m_background_color.R, m_background_color.G, m_background_color.B, m_background_color.A);
    CHECK_OPENGL_ERROR_IN_DEBUG();
...
CRenderizable::Ptr Viewport::getByName(const string& str)
{
  for (auto& m_object : m_objects)
  {
    if (m_object->getName() == str)
      return m_object;
    else if (m_object->GetRuntimeClass() == CLASS_ID_NAMESPACE(CSetOfObjects, opengl))
    {
    ...
    mrpt::poses::CPose3D* out_cameraPose) const
{
  ...
  const TPoint3D pointing_dir = {
      -cos(_.azimuth) * cos(_.elev), -sin(_.azimuth) * cos(_.elev), -sin(_.elev)};
...
void Viewport::updateMatricesFromCamera(const CCamera& myCamera) const
{
  ...
  else
  {
    ...
    if (myCamera.is6DOFMode())
    {
      ...
      const auto viewDirection = mrpt::poses::CPose3D::FromTranslation(0, 0, 1);
  ...
  _.computeLightProjectionMatrix(m_lightShadowClipMin, m_lightShadowClipMax, m_light);
