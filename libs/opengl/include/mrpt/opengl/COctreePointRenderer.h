...
class COctreePointRenderer
{
 ...
 private:
  ...
  {
    TRenderQueueElement(size_t id, float area_sq) : node_id(id), render_area_sqpixels(area_sq) {}
      ...
      float approx_area_sqpixels = 0) const
  {
    ...
    if (!corners_are_all_computed)
    {
      for (int i = 0; i < 8; i++)
      {
        ...
        ri.projectPointPixels(
            node.getCornerX(i), node.getCornerY(i), node.getCornerZ(i), cr_px[i].x, cr_px[i].y,
            cr_z[i]);
      }
    }
    ...
    mrpt::img::TPixelCoordf px_min(
        ...
        px_max(-std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());
    if (!trust_me_youre_visible)
    {
      ...
      const bool any_cr_zs_neg =
          (cr_z[0] < 0 || cr_z[1] < 0 || cr_z[2] < 0 || cr_z[3] < 0 || cr_z[4] < 0 || cr_z[5] < 0 ||
           cr_z[6] < 0 || cr_z[7] < 0);
      ...
      if (!box_crosses_image_plane &&
          (px_min.x >= ri.viewport_width || px_min.y >= ri.viewport_height || px_max.x < 0 ||
           px_max.y < 0))
        return;  // Not visible
    }
    ...
    {  // Render children nodes:
      ...
      if (!trust_me_youre_visible)  // Trust my parent... otherwise:
      {
        for (int i = 0; i < 8; i++)
        {
          if (!(cr_px[i].x >= 0 && cr_px[i].y >= 0 && cr_px[i].x < ri.viewport_width &&
      ...
      {
        mrpt::img::TPixelCoordf child_cr_px[8];  // No need to initialize
        ...
        for (int i = 0; i < 8; i++)
          this->octree_recursive_render(
              node.child_id[i], ri, child_cr_px, child_cr_z, true, true, approx_child_area);
      }
      else
      {
#ifdef __clang__
...
#define PROJ_SUB_NODE(POSTFIX)                                                     \
  ...
  ri.projectPointPixels(                                                           \
      p_##POSTFIX.x, p_##POSTFIX.y, p_##POSTFIX.z, px_##POSTFIX.x, px_##POSTFIX.y, \
      depth_##POSTFIX);
...
#define DO_RECURSE_CHILD(INDEX, SEQ0, SEQ1, SEQ2, SEQ3, SEQ4, SEQ5, SEQ6, SEQ7)            \
  {                                                                                        \
    mrpt::img::TPixelCoordf child_cr_px[8] = {px_##SEQ0, px_##SEQ1, px_##SEQ2, px_##SEQ3,  \
                                              px_##SEQ4, px_##SEQ5, px_##SEQ6, px_##SEQ7}; \
    float child_cr_z[8] = {depth_##SEQ0, depth_##SEQ1, depth_##SEQ2, depth_##SEQ3,         \
                           depth_##SEQ4, depth_##SEQ5, depth_##SEQ6, depth_##SEQ7};        \
    this->octree_recursive_render(node.child_id[INDEX], ri, child_cr_px, child_cr_z);      \
        ...
        //                     4         5        6         7
        DO_RECURSE_CHILD(
            0, Xm_Ym_Zm, X0_Ym_Zm, Xm_Y0_Zm, X0_Y0_Zm, Xm_Ym_Z0, X0_Ym_Z0, Xm_Y0_Z0, X0_Y0_Z0)
        DO_RECURSE_CHILD(
            1, X0_Ym_Zm, Xp_Ym_Zm, X0_Y0_Zm, Xp_Y0_Zm, X0_Ym_Z0, Xp_Ym_Z0, X0_Y0_Z0, Xp_Y0_Z0)
        DO_RECURSE_CHILD(
            2, Xm_Y0_Zm, X0_Y0_Zm, Xm_Yp_Zm, X0_Yp_Zm, Xm_Y0_Z0, X0_Y0_Z0, Xm_Yp_Z0, X0_Yp_Z0)
        DO_RECURSE_CHILD(
            3, X0_Y0_Zm, Xp_Y0_Zm, X0_Yp_Zm, Xp_Yp_Zm, X0_Y0_Z0, Xp_Y0_Z0, X0_Yp_Z0, Xp_Yp_Z0)
        DO_RECURSE_CHILD(
            4, Xm_Ym_Z0, X0_Ym_Z0, Xm_Y0_Z0, X0_Y0_Z0, Xm_Ym_Zp, X0_Ym_Zp, Xm_Y0_Zp, X0_Y0_Zp)
        DO_RECURSE_CHILD(
            5, X0_Ym_Z0, Xp_Ym_Z0, X0_Y0_Z0, Xp_Y0_Z0, X0_Ym_Zp, Xp_Ym_Zp, X0_Y0_Zp, Xp_Y0_Zp)
        DO_RECURSE_CHILD(
            6, Xm_Y0_Z0, X0_Y0_Z0, Xm_Yp_Z0, X0_Yp_Z0, Xm_Y0_Zp, X0_Y0_Zp, Xm_Yp_Zp, X0_Yp_Zp)
        DO_RECURSE_CHILD(
            7, X0_Y0_Z0, Xp_Y0_Z0, X0_Yp_Z0, Xp_Yp_Z0, X0_Y0_Zp, Xp_Y0_Zp, X0_Yp_Zp, Xp_Yp_Zp)
...
#pragma clang diagnostic pop
  ...
  void internal_recursive_split(const size_t node_id, const bool all_pts = false)
  {
    ...
    const size_t N = all_pts ? octree_derived().size_unprotected() : node.pts.size();
    ...
    if (N <= mrpt::global_settings::OCTREE_RENDER_MAX_POINTS_PER_NODE())
    {
      ...
      {
        if (all_pts)
          for (size_t i = 0; i < N; i++) node.update_bb(octree_derived().getPoint3Df(i));
        else
          for (size_t i = 0; i < N; i++) node.update_bb(octree_derived().getPoint3Df(node.pts[i]));
      }
    }
    else
    {
      ...
      for (int i = 0; i < 8; i++) node.child_id[i] = children_idx_base + i;
      ...
      for (int i = 0; i < 8; i++) internal_recursive_split(node.child_id[i]);
    }
 ...
 public:
  ...
  void octree_debug_dump_tree(std::ostream& o) const
  {
    ...
    for (size_t i = 0; i < m_octree_nodes.size(); i++)
    {
      ...
      {
        o << "parent, center=(" << node.center.x << "," << node.center.y << "," << node.center.z
          << "), children: " << node.child_id[0] << "," << node.child_id[1] << ","
          << node.child_id[2] << "," << node.child_id[3] << "," << node.child_id[4] << ","
          << node.child_id[5] << "," << node.child_id[6] << "," << node.child_id[7] << "; ";
      }
