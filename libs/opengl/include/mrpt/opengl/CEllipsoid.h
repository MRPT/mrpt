...
namespace mrpt
{
	namespace opengl
	{
		...
		DEFINE_SERIALIZABLE_PRE_CUSTOM_BASE_LINKAGE( CEllipsoid, CRenderizable, OPENGL_IMPEXP )
		...
		public:
			...
			void enableDrawSolid3D(bool v) { m_drawSolid3D = v; } //!< If set to true (default), a whole ellipsoid surface will be drawn, or if set to "false" it will be drawn as a "wireframe".
			void setQuantiles(float q) { m_quantiles=q; } //!< The number of "sigmas" for drawing the ellipse/ellipsoid (default=3)
			float getQuantiles() const { return m_quantiles; }

			void set2DsegmentsCount(unsigned int N) { m_2D_segments=N; }  //!< The number of segments of a 2D ellipse (default=20)
			void set3DsegmentsCount(unsigned int N) { m_3D_segments=N; } //!< The number of segments of a 3D ellipse (in both "axis") (default=20)
