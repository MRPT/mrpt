...
struct TImageROI
{
  // Constructors
  TImageROI() = default;
  TImageROI(size_t x1, size_t x2, size_t y1, size_t y2) : xMin(x1), xMax(x2), yMin(y1), yMax(y2) {}
...
#define CHECK_MEMBER(_m) this->_m == o._m
  ...
  bool operator==(const TMatchingOptions& o) const
  {
    return CHECK_MEMBER(useXRestriction) && CHECK_MEMBER(useDisparityLimits) &&
           ...
           CHECK_MEMBER(enable_robust_1to1_match) && CHECK_MEMBER(epipolar_TH) &&
           CHECK_MEMBER(estimateDepth) && CHECK_MEMBER(F) && CHECK_MEMBER(hasFundamentalMatrix) &&
           CHECK_MEMBER(matching_method) && CHECK_MEMBER(maxDepthThreshold) &&
           CHECK_MEMBER(maxEDD_TH) && CHECK_MEMBER(maxEDSD_TH) && CHECK_MEMBER(maxORB_dist) &&
...
struct TMultiResDescMatchOptions : public mrpt::config::CLoadableOptions
{
  ...
  void saveToConfigFile(
      mrpt::config::CConfigFileBase& cfg, const std::string& section) const override;
...
struct TMultiResDescOptions : public mrpt::config::CLoadableOptions
{
      ...
      cropValue(_cropValue)
  {
    ...
    for (unsigned int k = 0; k < _scales.size(); ++k) scales[k] = _scales[k];
  }
