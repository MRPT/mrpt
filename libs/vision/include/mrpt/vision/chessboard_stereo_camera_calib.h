...
struct VISION_IMPEXP TImageStereoCallbackData
{
	...
	size_t current_iter;
	double current_rmse;  //!< Current root-mean square reprojection error (in
						  //!pixels)
...
struct VISION_IMPEXP TStereoCalibParams
{
	...
	bool verbose;  //!< Show progress messages to std::cout console
				   //!(default=true)
	size_t maxIters;  //!< Maximum number of iterations of the optimizer
					  //!(default=300)
	...
	bool use_robust_kernel;  //!< Employ a Pseudo-Huber robustifier kernel
							 //!(Default: false)
	double robust_kernel_param;  //!< The parameter of the robust kernel, in
								 //!pixels (only if use_robust_kernel=true)
								 //!(Default=10)

	TSteroCalibCallbackFunctor callback;  //!< If set to !=NULL, this function
										  //!will be called within each
										  //!Lev-Marq. iteration (don't do heavy
										  //!stuff here since performance will
										  //!degrade)
	void* callback_user_param;  //!< If using a callback function, you can use
								//!this to pass custom data to your callback.
...
struct VISION_IMPEXP TStereoCalibResults
{
	...
	mrpt::poses::CPose3D right2left_camera_pose;  //!< The pose of the left
	...
	double final_rmse;  //!< Final reprojection square Root Mean Square Error
						//!(in pixels).
