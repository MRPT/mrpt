...
namespace slam
{
	...
	public:
		...
		/** Simulates a noisy bearing-range observation of all the beacons (landamrks with type glBeacon) in the landmarks map, if any.
		  * \param in_robotPose The robot pose.
		  * \param in_sensorLocationOnRobot The 3D position of the sensor on the robot
		  * \param out_Observations The results will be stored here.
		  * \param sensorDetectsIDs If this is set to false, all the landmarks will be sensed with INVALID_LANDMARK_ID as ID.
		  * \param in_stdRange The sigma of the sensor noise in range (meters).
		  * \param in_stdYaw The sigma of the sensor noise in yaw (radians).
		  * \param in_stdPitch The sigma of the sensor noise in pitch (radians).
		  * \param out_real_associations If it's not a NULL pointer, this will contain at the return the real indices of the landmarks in the map in the same order than they appear in out_Observations. Useful when sensorDetectsIDs=false.
		...
		void  simulateRangeBearingReadings(
			...
			vector_size_t 					*out_real_associations = NULL
			 ) const;
