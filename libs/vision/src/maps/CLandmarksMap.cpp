...
MAP_DEFINITION_REGISTER("mrpt::maps::CLandmarksMap,landmarksMap", mrpt::maps::CLandmarksMap)
...
void CLandmarksMap::TMapDefinition::dumpToTextStream_map_specific(std::ostream& out) const
{
  ...
  for (const auto& initialBeacon : initialBeacons)
    out << mrpt::format(
        "      %03u         (%8.03f,%8.03f,%8.03f)\n", initialBeacon.second, initialBeacon.first.x,
        initialBeacon.first.y, initialBeacon.first.z);
    ...
    const mrpt::maps::TMetricMapInitializer& _def)
{
  ...
  for (const auto& initialBeacon : def.initialBeacons)
  {
    ...
    lm.pose_cov_11 = lm.pose_cov_22 = lm.pose_cov_33 = lm.pose_cov_12 = lm.pose_cov_13 =
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
std::map<std::pair<mrpt::maps::CLandmark::TLandmarkID, mrpt::maps::CLandmark::TLandmarkID>, double>
    CLandmarksMap::_mEDD;
    ...
    const CObservation& obs, const CPose3D& robotPose3D) const
{
  ...
  else if (CLASS_ID(CObservationBeaconRanges) == obs.GetRuntimeClass())
  {
    ...
    const auto unif_val = std::log(1.0 / (o.maxSensorDistance - o.minSensorDistance));
    ...
    for (const auto& meas : o.sensedData)
    {
      ...
      {
        if ((lm.getType() != featBeacon) || (lm.ID != sensedID)) continue;  // Skip

        lm.getPose(beaconPDF);
        const auto& beacon3D = beaconPDF.mean;

        const double expectedRange = point3D.distanceTo(beacon3D);
        const double sensedDist = std::max<double>(.0, meas.sensedDistance);
        MRPT_CHECK_NORMAL_NUMBER(expectedRange);

        ret += (-0.5 * mrpt::square((expectedRange - sensedDist) / sensorStd));
        found = true;
  ...
  else if (CLASS_ID(CObservationGPS) == obs.GetRuntimeClass())
  {
    ...
    ********************************************************************/
    ...
    if ((o.has_GGA_datum()) && (likelihoodOptions.GPSOrigin.min_sat <=
                                o.getMsgByClass<gnss::Message_NMEA_GGA>().fields.satellitesUsed))
    {
      // Compose GPS robot position

      x = DEG2RAD(
              (o.getMsgByClass<gnss::Message_NMEA_GGA>().fields.longitude_degrees -
               likelihoodOptions.GPSOrigin.longitude)) *
          earth_radius * 1.03;
      y = DEG2RAD(
              (o.getMsgByClass<gnss::Message_NMEA_GGA>().fields.latitude_degrees -
               likelihoodOptions.GPSOrigin.latitude)) *
          earth_radius * 1.15;
      GPSpose.x(
          (x * cos(likelihoodOptions.GPSOrigin.ang) + y * sin(likelihoodOptions.GPSOrigin.ang) +
           likelihoodOptions.GPSOrigin.x_shift));
      GPSpose.y(
          (-x * sin(likelihoodOptions.GPSOrigin.ang) + y * cos(likelihoodOptions.GPSOrigin.ang) +
...
void CLandmarksMap::loadSiftFeaturesFromImageObservation(
    const CObservationImage& obs, const mrpt::vision::CFeatureExtraction::TOptions& feat_options)
{
  CImage corImg;
  ...
  for (sift = siftList.begin(); sift != siftList.end(); sift++)
  {
    ...
    mrpt::math::TPoint3D dir =
        vision::pixelTo3D(sift->keypoint.pt, obs.cameraParams.intrinsicParams);
    ...
    const mrpt::vision::CFeatureExtraction::TOptions& feat_options)
{
  ...
  fExt.detectFeatures(
      obs.imageLeft, leftSiftList, fID, insertionOptions.SIFTs_numberOfKLTKeypoints);
  fExt.detectFeatures(
      obs.imageRight, rightSiftList, fID, insertionOptions.SIFTs_numberOfKLTKeypoints);
...
void CLandmarksMap::changeCoordinatesReference(const CPose3D& newOrg)
{
  ...
  for (lm = landmarks.begin(); lm != landmarks.end(); lm++)
  {
    ...
    c11 = R11 * (C11 * R11 + C12 * R12 + C13 * R13) + R12 * (C12 * R11 + C22 * R12 + C23 * R13) +
          R13 * (C13 * R11 + C23 * R12 + C33 * R13);
    c12 = (C11 * R11 + C12 * R12 + C13 * R13) * R21 + (C12 * R11 + C22 * R12 + C23 * R13) * R22 +
          (C13 * R11 + C23 * R12 + C33 * R13) * R23;
    c13 = (C11 * R11 + C12 * R12 + C13 * R13) * R31 + (C12 * R11 + C22 * R12 + C23 * R13) * R32 +
          (C13 * R11 + C23 * R12 + C33 * R13) * R33;
    c22 = R21 * (C11 * R21 + C12 * R22 + C13 * R23) + R22 * (C12 * R21 + C22 * R22 + C23 * R23) +
          R23 * (C13 * R21 + C23 * R22 + C33 * R23);
    c23 = (C11 * R21 + C12 * R22 + C13 * R23) * R31 + (C12 * R21 + C22 * R22 + C23 * R23) * R32 +
          (C13 * R21 + C23 * R22 + C33 * R23) * R33;
    c33 = R31 * (C11 * R31 + C12 * R32 + C13 * R33) + R32 * (C12 * R31 + C22 * R32 + C23 * R33) +
...
void CLandmarksMap::fuseWith(CLandmarksMap& other, bool justInsertAllOfThem)
{
  ...
  if (!justInsertAllOfThem)
  {
    ...
    for (i = n - 1; i >= 0; i--)
    {
      ...
      {
        const double dt = 1e-3 * std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  if (verbose)
  {
    printf(
        ...
        static_cast<unsigned int>(other.size() - corrs.size()), static_cast<unsigned int>(nRemoved),
    ...
    std::vector<bool>& otherCorrespondences) const
{
  ...
  switch (insertionOptions.SIFTMatching3DMethod)
  {
    case 0:
      ...
      for (k = 0, otherIt = anotherMap->landmarks.begin(); otherIt != anotherMap->landmarks.end();
           otherIt++, k++)
      {
        ...
        if (otherIt->getType() == featSIFT)
        {
          ...
          for (j = 0, thisIt = landmarks.begin(); thisIt != landmarks.end(); thisIt++, j++)
          {
            if (thisIt->getType() == featSIFT &&
                thisIt->features.size() == otherIt->features.size() && !thisIt->features.empty() &&
                    ...
                    otherIt->features[0].descriptors.SIFT->size())
            {
              ...
              if (lik_dist > 1e-2)
              {
                ...
                std::pair<mrpt::maps::CLandmark::TLandmarkID, mrpt::maps::CLandmark::TLandmarkID>
          ...
          if (maxLik > insertionOptions.SiftLikelihoodThreshold)
          {
            ...
            if (!thisLandmarkAssigned[maxIdx])
            {
              ...
              match.localIdx = k;
              match.local.x = anotherMap->landmarks.get(k)->pose_mean.x;
              match.local.y = anotherMap->landmarks.get(k)->pose_mean.y;
              match.local.z = anotherMap->landmarks.get(k)->pose_mean.z;
...
bool CLandmarksMap::saveToTextFile(std::string file)
{
  ...
  {
    os::fprintf(
        f, "%10f %10f %10f %4i %4u %10f", it->pose_mean.x, it->pose_mean.y, it->pose_mean.z,
        static_cast<int>(it->getType()), it->seenTimesCount,
    ...
    if (it->getType() == featSIFT)
    {
      ...
      for (unsigned char i : *it->features[0].descriptors.SIFT) os::fprintf(f, " %u ", i);
    }
    ...
    std::string file, const char* style, float confInterval) const
{
  ...
  {
    os::fprintf(
        f, "m=[%.4f %.4f %.4f];", landmark.pose_mean.x, landmark.pose_mean.y, landmark.pose_mean.z);
    os::fprintf(
        f, "c=[%.8f %.8f %.8f;%.8f %.8f %.8f;%.8f %.8f %.8f]; ", landmark.pose_cov_11,
        landmark.pose_cov_12, landmark.pose_cov_13, landmark.pose_cov_12, landmark.pose_cov_22,
        landmark.pose_cov_23, landmark.pose_cov_13, landmark.pose_cov_23, landmark.pose_cov_33);
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
bool CLandmarksMap::saveToMATLABScript2D(std::string file, const char* style, float stdCount)
{
  FILE* f = os::fopen(file.c_str(), "wt");
  if (!f) return false;
  ...
  for (auto& landmark : landmarks)
  {
    ...
    // ----------------------------------------------
    for (x = X.begin(), y = Y.begin(), Cos = COS.begin(), Sin = SIN.begin(); x != X.end();
         x++, y++, Cos++, Sin++)
    {
      *x = (landmark.pose_mean.x + stdCount * (*Cos * M(0, 0) + *Sin * M(1, 0)));
      *y = (landmark.pose_mean.y + stdCount * (*Cos * M(0, 1) + *Sin * M(1, 1)));
    }
...
void CLandmarksMap::TCustomSequenceLandmarks::push_back(const CLandmark& l)
{
  ...
  m_grid.resize(
      min(m_grid.getXMin(), l.pose_mean.x - 0.1), max(m_grid.getXMax(), l.pose_mean.x + 0.1),
      min(m_grid.getYMin(), l.pose_mean.y - 0.1), max(m_grid.getYMax(), l.pose_mean.y + 0.1),
      dummyEmpty);
...
void CLandmarksMap::TCustomSequenceLandmarks::isToBeModified(unsigned int indx)
{
  std::vector<int32_t>* cell =
      m_grid.cellByPos(m_landmarks[indx].pose_mean.x, m_landmarks[indx].pose_mean.y);
    ...
    std::vector<bool>* otherCorrespondences) const
{
  ...
  switch (insertionOptions.SIFTLikelihoodMethod)
  {
    case 0:  // Our method
    {
      ...
      for (idx1 = 0, lm1 = theMap->landmarks.begin(); lm1 < theMap->landmarks.end();
            ...
            if (lm2->getType() == featSIFT)
            {
              ...
              if (likByDist > 1e-2)
              {
                ...
                if (CLandmarksMap::_mEDD[mPair] == 0)
                {
                  ...
                  ASSERT_(
                      lm1->features[0].descriptors.SIFT->size() ==
                  ...
                  for (it1 = lm1->features[0].descriptors.SIFT->begin(),
                      it2 = lm2->features[0].descriptors.SIFT->begin();
                       it1 != lm1->features[0].descriptors.SIFT->end(); it1++, it2++)
    ...
    case 1:  // SIM, ELINAS, GRIFFIN, LITTLE
      ...
      for (size_t k = 1; k <= (theMap->size() - correspondences->size()); k++)
...
void CLandmarksMap::TInsertionOptions::dumpToTextStream(std::ostream& out) const
{
  ...
  out << "\n";
  out << mrpt::format("SiftCorrRatioThreshold                  = %f\n", SiftCorrRatioThreshold);
  out << mrpt::format("SiftLikelihoodThreshold                 = %f\n", SiftLikelihoodThreshold);
  out << mrpt::format("SiftEDDThreshold                        = %f\n", SiftEDDThreshold);
  out << mrpt::format("SIFTMatching3DMethod                    = %d\n", SIFTMatching3DMethod);
  out << mrpt::format("SIFTLikelihoodMethod                    = %d\n", SIFTLikelihoodMethod);

  out << mrpt::format("SIFTsLoadDistanceOfTheMean              = %f\n", SIFTsLoadDistanceOfTheMean);
  out << mrpt::format("SIFTsLoadEllipsoidWidth                 = %f\n", SIFTsLoadEllipsoidWidth);
  out << "\n";
  out << mrpt::format("SIFTs_stdXY                             = %f\n", SIFTs_stdXY);
  out << mrpt::format("SIFTs_stdDisparity                      = %f\n", SIFTs_stdDisparity);
  out << "\n";
  out << mrpt::format("SIFTs_numberOfKLTKeypoints              = %i\n", SIFTs_numberOfKLTKeypoints);
  out << mrpt::format("SIFTs_stereo_maxDepth                   = %f\n", SIFTs_stereo_maxDepth);
  out << mrpt::format("SIFTs_epipolar_TH                       = %f\n", SIFTs_epipolar_TH);
  out << mrpt::format("PLOT_IMAGES                             = %c\n", PLOT_IMAGES ? 'Y' : 'N');
    ...
    const mrpt::config::CConfigFileBase& iniFile, const std::string& section)
{
  insert_SIFTs_from_monocular_images = iniFile.read_bool(
      section.c_str(), "insert_SIFTs_from_monocular_images", insert_SIFTs_from_monocular_images);
  insert_SIFTs_from_stereo_images = iniFile.read_bool(
      section.c_str(), "insert_SIFTs_from_stereo_images", insert_SIFTs_from_stereo_images);
  insert_Landmarks_from_range_scans = iniFile.read_bool(
      section.c_str(), "insert_Landmarks_from_range_scans", insert_Landmarks_from_range_scans);
  SiftCorrRatioThreshold =
      iniFile.read_float(section.c_str(), "SiftCorrRatioThreshold", SiftCorrRatioThreshold);
  SiftLikelihoodThreshold =
      iniFile.read_float(section.c_str(), "SiftLikelihoodThreshold", SiftLikelihoodThreshold);
  SiftEDDThreshold = iniFile.read_float(section.c_str(), "SiftEDDThreshold", SiftEDDThreshold);
  SIFTMatching3DMethod =
      iniFile.read_int(section.c_str(), "SIFTMatching3DMethod", SIFTMatching3DMethod);
  SIFTLikelihoodMethod =
      iniFile.read_int(section.c_str(), "SIFTLikelihoodMethod", SIFTLikelihoodMethod);
  SIFTsLoadDistanceOfTheMean =
      iniFile.read_float(section.c_str(), "SIFTsLoadDistanceOfTheMean", SIFTsLoadDistanceOfTheMean);
  SIFTsLoadEllipsoidWidth =
      iniFile.read_float(section.c_str(), "SIFTsLoadEllipsoidWidth", SIFTsLoadEllipsoidWidth);
  SIFTs_stdXY = iniFile.read_float(section.c_str(), "SIFTs_stdXY", SIFTs_stdXY);
  SIFTs_stdDisparity =
      iniFile.read_float(section.c_str(), "SIFTs_stdDisparity", SIFTs_stdDisparity);
  SIFTs_numberOfKLTKeypoints =
      iniFile.read_int(section.c_str(), "SIFTs_numberOfKLTKeypoints", SIFTs_numberOfKLTKeypoints);
  SIFTs_stereo_maxDepth =
      iniFile.read_float(section.c_str(), "SIFTs_stereo_maxDepth", SIFTs_stereo_maxDepth);
...
void CLandmarksMap::TLikelihoodOptions::dumpToTextStream(std::ostream& out) const
{
  ...
  out << mrpt::format("rangeScan2D_decimation                  = %i\n", rangeScan2D_decimation);
  out << mrpt::format("SIFTs_sigma_euclidean_dist              = %f\n", SIFTs_sigma_euclidean_dist);
  out << mrpt::format(
      "SIFTs_sigma_descriptor_dist             = %f\n", SIFTs_sigma_descriptor_dist);
  out << mrpt::format("SIFTs_mahaDist_std                      = %f\n", SIFTs_mahaDist_std);
  out << mrpt::format("SIFTs_decimation                        = %i\n", SIFTs_decimation);
  out << mrpt::format(
      "SIFTnullCorrespondenceDistance          = %f\n", SIFTnullCorrespondenceDistance);
  out << mrpt::format("beaconRangesStd                         = %f\n", beaconRangesStd);
  out << mrpt::format(
      "beaconRangesUseObservationStd           = %c\n", beaconRangesUseObservationStd ? 'Y' : 'N');
  out << mrpt::format("extRobotPoseStd                         = %f\n", extRobotPoseStd);

  out << mrpt::format("GPSOrigin:LATITUDE                      = %f\n", GPSOrigin.latitude);
  out << mrpt::format("GPSOrigin:LONGITUDE                     = %f\n", GPSOrigin.longitude);
  out << mrpt::format("GPSOrigin:ALTITUDE                      = %f\n", GPSOrigin.altitude);
  out << mrpt::format("GPSOrigin:Rotation_Angle                = %f\n", GPSOrigin.ang);
  out << mrpt::format("GPSOrigin:x_shift                       = %f\n", GPSOrigin.x_shift);
  out << mrpt::format("GPSOrigin:y_shift                       = %f\n", GPSOrigin.y_shift);
  out << mrpt::format("GPSOrigin:min_sat                       = %i\n", GPSOrigin.min_sat);
...
{
  rangeScan2D_decimation =
      iniFile.read_int(section.c_str(), "rangeScan2D_decimation", rangeScan2D_decimation);
  SIFTs_sigma_euclidean_dist = iniFile.read_double(
      section.c_str(), "SIFTs_sigma_euclidean_dist", SIFTs_sigma_euclidean_dist);
  SIFTs_sigma_descriptor_dist = iniFile.read_double(
      section.c_str(), "SIFTs_sigma_descriptor_dist", SIFTs_sigma_descriptor_dist);
  SIFTs_mahaDist_std =
      iniFile.read_float(section.c_str(), "SIFTs_mahaDist_std", SIFTs_mahaDist_std);
  ...
  SIFTnullCorrespondenceDistance = iniFile.read_float(
      section.c_str(), "SIFTnullCorrespondenceDistance", SIFTnullCorrespondenceDistance);

  GPSOrigin.latitude =
      iniFile.read_double(section.c_str(), "GPSOriginLatitude", GPSOrigin.latitude);
  GPSOrigin.longitude =
      iniFile.read_double(section.c_str(), "GPSOriginLongitude", GPSOrigin.longitude);
  GPSOrigin.altitude =
      iniFile.read_double(section.c_str(), "GPSOriginAltitude", GPSOrigin.altitude);
  GPSOrigin.ang =
      iniFile.read_double(section.c_str(), "GPSOriginAngle", GPSOrigin.ang) * M_PI / 180;
  ...
  beaconRangesUseObservationStd = iniFile.read_bool(
      section.c_str(), "beaconRangesUseObservationStd", beaconRangesUseObservationStd);
    ...
    mrpt::obs::CObservationBeaconRanges& out_Observations) const
{
  ...
  for (it = landmarks.begin(); it != landmarks.end(); it++)
  {
    if (it->getType() == featBeacon)
    {
      ...
      // Add noise:
      range += out_Observations.stdError * getRandomGenerator().drawGaussian1D_normalized();
...
void CLandmarksMap::getVisualizationInto(mrpt::opengl::CSetOfObjects& o) const
{
  ...
  for (const auto& landmark : landmarks)
  {
    ...
    ellip->setName(mrpt::format("LM.ID=%u", static_cast<unsigned int>(landmark.ID)));
...
/**** FAMD ****/
mrpt::maps::CLandmark::TLandmarkID CLandmarksMap::getMapMaxID() { return _mapMaxID; }
/**** END FAMD ****/
    ...
    const TMatchingRatioParams& params) const
{
  ...
  for (itOther = otherMap->landmarks.begin(), itPoseOther = poses3DOther.begin();
  ...
  for (itThis = landmarks.begin(), itPoseThis = poses3DThis.begin(); itThis != landmarks.end();
       itThis++, itPoseThis++)
  {
       ...
       itOther != otherMap->landmarks.end(); itOther++, itPoseOther++)
  {
         ...
         itThis++, itPoseThis++)
    {
      ...
      float distMaha = sqrt(mrpt::math::multiply_HCHt_scalar(d, COV.inverse_LLt()));
      ...
      if (distMaha < params.maxMahaDistForCorr)
      {
                ...
                itOther->features[0].descriptors.SIFT->size())
        {
          ...
          float descrDist_f = sqrt(d2f(descrDist)) / itThis->features[0].descriptors.SIFT->size();
    ...
    const double spurious_count_std) const
{
  ...
  if (spurious_count_std != 0 || spurious_count_mean != 0)
    nSpurious = static_cast<size_t>(mrpt::round_long(std::max(0.0, fSpurious)));
  ...
  for (size_t i = 0; i < nSpurious; i++)
  {
    ...
    if (out_real_associations)
      out_real_associations->push_back(std::string::npos);  // Real index: spurious
  }                                                         // end for it
