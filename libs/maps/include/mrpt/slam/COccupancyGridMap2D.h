...
{
// Fwd decls:
namespace obs { class CObservation2DRangeScan; class CObservationRange; }
namespace slam
{
	...
	protected:
		...
		inline void   setCell_nocheck(int x,int y,float value) { map[x+y*size_x]=p2l(value); }

		/** Read the real valued [0,1] contents of a cell, given its index. */
		inline float  getCell_nocheck(int x,int y) const { return l2p(map[x+y*size_x]); }
	...
	public:
		...
		double	 computeObservationLikelihood( const mrpt::obs::CObservation *obs, const CPose3D &takenFrom );
