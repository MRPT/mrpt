...
class COctoMapBase : public mrpt::maps::CMetricMap
{
 public:
  ...
  struct TInsertionOptions : public mrpt::config::CLoadableOptions
  {
    ...
    double maxrange{-1.};  //!< maximum range for how long individual beams are
   ...
   private:
    ...
    // model) (Default=0.5)
    double probHit{0.7};  // sets the probablility for a "hit" (will be converted
    ...
    double clampingThresMin{0.1192};  // sets the minimum threshold for occupancy
    // clamping (sensor model) (Default=0.1192, -2
    // in log odds)
    double clampingThresMax{0.971};  // sets the maximum threshold for occupancy
  ...
  struct TRenderingOptions
  {
    ...
    bool generateOccupiedVoxels{true};  //!< Generate voxels for the occupied
    //! volumes  (Default=true)
    bool visibleOccupiedVoxels{true};  //!< Set occupied voxels visible (requires
  ...
  void insertPointCloud(
      const CPointsMap& ptMap, const float sensor_x, const float sensor_y, const float sensor_z);
