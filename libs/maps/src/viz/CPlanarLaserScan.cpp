...
IMPLEMENTS_SERIALIZABLE(CPlanarLaserScan, CRenderizable, mrpt::viz)
...
void CPlanarLaserScan::onUpdateBuffers_Triangles()
{
  ...
  {
    tris.emplace_back(
        P3f(m_scan.sensorPose.x(), m_scan.sensorPose.y(), m_scan.sensorPose.z()),
...
void CPlanarLaserScan::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  switch (version)
  {
    ...
    case 3:
    {
      ...
      if (version >= 3) CRenderizableShaderTriangles::params_deserialize(in);
    }
    break;
    default:
