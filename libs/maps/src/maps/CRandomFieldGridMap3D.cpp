...
CRandomFieldGridMap3D::CRandomFieldGridMap3D(
	...
	CDynamicGrid3D<TRandomFieldVoxel>( x_min,x_max,y_min,y_max, z_min, z_max, resolution /*xy*/, resolution /*z*/ ),
{
}

/** Changes the size of the grid, erasing previous contents. \sa resize */
void CRandomFieldGridMap3D::setSize(const double x_min, const double x_max, const double y_min, const double y_max, const double resolution, const TRandomFieldCell * fill_value)
{
...
/*---------------------------------------------------------------
						clear
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::internal_clear()
{
	...
	case mrKalmanFilter:
		{
			if (m_rfgm_verbose)
				printf("[CRandomFieldGridMap3D::clear] Setting covariance matrix to %ux%u\n",(unsigned int)(m_size_y*m_size_x),(unsigned int)(m_size_y*m_size_x));
	...
	case mrKalmanApproximate:
		{
			...
			if (m_rfgm_verbose)
				printf("[CRandomFieldGridMap3D::clear] Resetting compressed cov. matrix and cells\n");
			TRandomFieldCell	def(
				m_insertOptions_common->KF_defaultCellMeanValue,									// mean
				m_insertOptions_common->KF_initialCellStd		// std
				);
			...
			if (m_rfgm_verbose)
				printf("[CRandomFieldGridMap3D::clear] %ux%u cells done in %.03fms\n", unsigned(m_size_x),unsigned(m_size_y),1000*tictac.Tac());
		}
		break;
	...
	case mrGMRF_G:
		{
			...
			if (m_rfgm_verbose)
				printf("[CRandomFieldGridMap3D::clear] Generating Prior based on Gaussian\n");
	...
	case mrGMRF_SD:
		{
			...
			if (m_rfgm_verbose) {
				printf("[CRandomFieldGridMap3D::clear] Generating Prior based on 'Squared Differences'\n");
				printf("[CRandomFieldGridMap3D::clear] Initial map dimension: %u cells, x=(%.2f,%.2f) y=(%.2f,%.2f)\n", static_cast<unsigned int>(m_map.size()), getXMin(),getXMax(),getYMin(),getYMax());
			}
			...
			nObsFactors = 0;		// M
			nFactors = nPriorFactors + nObsFactors;
			if (m_rfgm_verbose) cout << "[CRandomFieldGridMap3D::clear] Generating " << nFactors << " factors for a map size of N=" << N << endl;
			...
			{
				if (m_rfgm_verbose) cout << "[CRandomFieldGridMap3D::clear] Initiating prior (fully connected)\n";
				//---------------------------------------------------------------
			...
			if (m_rfgm_verbose) {
				cout << " [CRandomFieldGridMap3D::clear] Prior built in " << tictac.Tac() << " s ----------\n";
			}
...
/*---------------------------------------------------------------
						isEmpty
  ---------------------------------------------------------------*/
bool  CRandomFieldGridMap3D::isEmpty() const
{
	return false;
}
...
*/
void  CRandomFieldGridMap3D::insertObservation_KernelDM_DMV(
	double normReading,
	...
	bool             is_DMV )
{
	...
	{
		if (m_rfgm_verbose) printf("[CRandomFieldGridMap3D::insertObservation_KernelDM_DMV] Precomputing window %ux%u\n",Ac_all,Ac_all);
		...
		if (m_rfgm_verbose) printf("[CRandomFieldGridMap3D::insertObservation_KernelDM_DMV] Done!\n");
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::TInsertionOptions::internal_dumpToTextStream_common(mrpt::utils::CStream	&out) const
{
	out.printf("sigma                                   = %f\n", sigma);
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::TInsertionOptions::internal_loadFromConfigFile_common(
...
/*---------------------------------------------------------------
					saveAsBitmapFile
 ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::saveAsBitmapFile(const std::string &filName) const
{
	MRPT_START
...
void CRandomFieldGridMap3D::getAsMatrix( mrpt::math::CMatrixDouble &cells_mat) const
{
	MRPT_START
...
/*---------------------------------------------------------------
					getAsBitmapFile
 ---------------------------------------------------------------*/
void CRandomFieldGridMap3D::getAsBitmapFile(mrpt::utils::CImage &out_img) const
{
	MRPT_START
	...
	double additionalMarginMeters)
{
	...
	if ( m_size_x != old_sizeX || m_size_y != old_sizeY )
	{
		...
		if (m_mapType==mrKalmanFilter)
		{
			...
			printf("[CRandomFieldGridMap3D::resize] Resizing from %ux%u to %ux%u (%u cells)\n",
			...
			// Resize done!
			printf("[CRandomFieldGridMap3D::resize] Done\n");
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::insertObservation_KF(
	double normReading,
	const mrpt::math::TPoint2D &point )
{
	MRPT_START
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::saveMetricMapRepresentationToFile(
...
/*---------------------------------------------------------------
					saveAsMatlab3DGraph
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::saveAsMatlab3DGraph(const std::string  &filName) const
{
	MRPT_START
	...
	os::fprintf(f,"%%'CRandomFieldGridMap3D::saveAsMatlab3DGraph'\n");
	os::fprintf(f,"%%\n");
	os::fprintf(f,"%%                        ~ MRPT ~\n");
...
/*---------------------------------------------------------------
						getAs3DObject
---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::getAs3DObject( mrpt::opengl::CSetOfObjectsPtr	&outObj ) const
{
...
/*---------------------------------------------------------------
						getAs3DObject
---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::getAs3DObject( mrpt::opengl::CSetOfObjectsPtr	&meanObj, mrpt::opengl::CSetOfObjectsPtr &varObj ) const
{
	if (!genericMapParams.enableSaveAs3DObject) return;
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
double  CRandomFieldGridMap3D::computeConfidenceCellValue_DM_DMV (const TRandomFieldCell *cell ) const
{
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
double  CRandomFieldGridMap3D::computeMeanCellValue_DM_DMV (const TRandomFieldCell *cell ) const
{
	// A confidence measure:
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
double CRandomFieldGridMap3D::computeVarCellValue_DM_DMV (const TRandomFieldCell *cell ) const
{
	// A confidence measure:
	const double alpha = 1.0 - std::exp(-square(cell->dm_mean_w/m_insertOptions_common->dm_sigma_omega));
...
/*---------------------------------------------------------------
					predictMeasurement
  ---------------------------------------------------------------*/
void CRandomFieldGridMap3D::predictMeasurement(
	const double	x,
	const double	y,
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::insertObservation_KF2(
	double normReading,
	const mrpt::math::TPoint2D &point )
{
	MRPT_START
...
/*---------------------------------------------------------------
					recoverMeanAndCov
  ---------------------------------------------------------------*/
void  CRandomFieldGridMap3D::recoverMeanAndCov() const
{
...
/*---------------------------------------------------------------
					getMeanAndCov
  ---------------------------------------------------------------*/
void CRandomFieldGridMap3D::getMeanAndCov( CVectorDouble &out_means, CMatrixDouble &out_cov) const
{
...
/*---------------------------------------------------------------
					getMeanAndSTD
  ---------------------------------------------------------------*/
void CRandomFieldGridMap3D::getMeanAndSTD( CVectorDouble &out_means, CVectorDouble &out_STD) const
{
	const size_t N = BASE::m_map.size();
	out_means.resize(N);
	out_STD.resize(N);
...
/*---------------------------------------------------------------
					setMeanAndSTD
  ---------------------------------------------------------------*/
void CRandomFieldGridMap3D::setMeanAndSTD( CVectorDouble &in_means, CVectorDouble &in_std)
{
...
CRandomFieldGridMap3D::TMapRepresentation	 CRandomFieldGridMap3D::getMapType()
{
	return m_mapType;
}
...
void CRandomFieldGridMap3D::insertIndividualReading(const double sensorReading,const mrpt::math::TPoint2D & point, const bool update_map,const bool time_invariant)
{
	switch (m_mapType)
	{
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CRandomFieldGridMap3D::insertObservation_GMRF(
	double normReading,
...
bool CRandomFieldGridMap3D::ENABLE_GMRF_PROFILER  = false;

/*---------------------------------------------------------------
					updateMapEstimation_GMRF
  ---------------------------------------------------------------*/
void CRandomFieldGridMap3D::updateMapEstimation_GMRF()
{
#if EIGEN_VERSION_AT_LEAST(3,1,0)
	const size_t N = m_map.size();
	...
	if (!numActiveObs) {
		if (m_rfgm_verbose) printf("[CRandomFieldGridMap3D] 0 active observations: skipping map update.\n");
		return;
	}
	...
	if (m_rfgm_verbose) printf("[CRandomFieldGridMap3D] Solving...\n");
	...
	if (m_rfgm_verbose) printf("[CRandomFieldGridMap3D] Solved.\n");
	...
	if (m_rfgm_verbose) printf("[CRandomFieldGridMap3D] Computing variance: U matrix...\n");
	Eigen::SparseMatrix<double> UT = solver.matrixU();
	if (m_rfgm_verbose) printf("[CRandomFieldGridMap3D] Computing variance: U done.\n");
	Sigma.reserve(UT.nonZeros());

	//Apply custom equations to obtain the inverse -> inv( P*H*inv(P) )
	for (int l=N-1; l>=0; l--)
	{
		if (m_rfgm_verbose && !(l%100)) printf("[CRandomFieldGridMap3D] Computing variance %6.02f%%... \r", (100.0*(N-l-1))/N );
...
bool CRandomFieldGridMap3D::exist_relation_between2cells(
...
float  CRandomFieldGridMap3D::compute3DMatchingRatio(const mrpt::maps::CMetricMap *otherMap, const mrpt::poses::CPose3D &otherMapPose, const TMatchingRatioParams &params) const
{
