    ...
    const CObservation& obs, const CPose3D& takenFrom3D) const
{
  ...
  if (IS_CLASS(obs, CObservation2DRangeScan))
  {
    ...
    if (!scan.isPlanarScan(insertionOptions.horizontalTolerance)) return -10;
    ...
    const CObservation& obs, const CPose2D& takenFrom) const
{
  ...
  const auto* compareMap = o.buildAuxPointsMap<mrpt::maps::CPointsMap>(&insOpt);
  ...
  for (size_t i = 0; i < n; i++)
  {
    ...
    for (int cx = cxMin; cx <= cxMax; cx++)
      for (int cy = cyMin; cy <= cyMax; cy++) lik += 1 - getCell_nocheck(cx, cy);
  ...
  if (IS_CLASS(obs, CObservation2DRangeScan))
  {
    ...
    for (int x = x0; x < x1; x += 1)
    {
      for (int y = y0; y < y1; y += 1)
      {
        ...
        {
          nCellsCompared++;
          if ((c1 > 0.5 && c2 < 0.5) || (c1 < 0.5 && c2 > 0.5)) cellsDifference++;
        }
      }
    }
    ...
    const mrpt::config::CConfigFileBase& iniFile, const std::string& section)
{
  ...
  enableLikelihoodCache =
      iniFile.read_bool(section, "enableLikelihoodCache", enableLikelihoodCache);
  ...
  LF_alternateAverageMethod =
      iniFile.read_bool(section, "LF_alternateAverageMethod", LF_alternateAverageMethod);
  ...
  MI_ratio_max_distance =
      iniFile.read_float(section, "MI_ratio_max_distance", MI_ratio_max_distance);

  rayTracing_useDistanceFilter =
      iniFile.read_bool(section, "rayTracing_useDistanceFilter", rayTracing_useDistanceFilter);
  rayTracing_stdHit = iniFile.read_float(section, "rayTracing_stdHit", rayTracing_stdHit);

  consensus_takeEachRange =
      iniFile.read_int(section, "consensus_takeEachRange", consensus_takeEachRange);
...
void COccupancyGridMap2D::TLikelihoodOptions::dumpToTextStream(std::ostream& out) const
{
  ...
  out << mrpt::format("LF_maxCorrsDistance                     = %f\n", LF_maxCorrsDistance);
  out << mrpt::format(
  ...
  out << mrpt::format("MI_ratio_max_distance                   = %f\n", MI_ratio_max_distance);
  out << mrpt::format(
      "rayTracing_useDistanceFilter            = %c\n", rayTracing_useDistanceFilter ? 'Y' : 'N');
  out << mrpt::format("rayTracing_decimation                   = %u\n", rayTracing_decimation);
  out << mrpt::format("rayTracing_stdHit                       = %f\n", rayTracing_stdHit);
  out << mrpt::format("consensus_takeEachRange                 = %u\n", consensus_takeEachRange);
  out << mrpt::format("consensus_pow                           = %.02f\n", consensus_pow);
  out << "OWA_weights   = [";
