...
void TVoxelMap_InsertionOptions::writeToStream(
...
void TVoxelMap_InsertionOptions::readFromStream(
	mrpt::serialization::CArchive& in)
{
	...
	switch (version)
	{
		...
		case 1:
			...
			if (version >= 1) { in >> remove_voxels_farther_than; }
			break;
		default: MRPT_THROW_UNKNOWN_SERIALIZATION_VERSION(version);
	}
}

void TVoxelMap_RenderingOptions::writeToStream(
	mrpt::serialization::CArchive& out) const
{
...
void TVoxelMap_RenderingOptions::readFromStream(
	mrpt::serialization::CArchive& in)
{
	const uint8_t version = in.ReadAs<uint8_t>();
	switch (version)
