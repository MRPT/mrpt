...
void CObservationGasSensors::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  switch (version)
  {
    ...
    case 5:
    {
      ...
      for (i = 0; i < n; i++)
      {
        ...
        if (version >= 3)
        {
          ...
          if (m_readings[i].hasTemperature) in >> m_readings[i].temperature;
        }
        else
        {
    ...
    case 1:
    {
      ...
      // (1)
      eNose.eNosePoseOnTheRobot = TPose3D(0.20, -0.15, 0.10, 0, 0, 0);  // (x,y,z) only
  ...
  try
  {
    ...
    if (!first_iteration)
    {
      ...
      if (incT > 0)
        ...
        last_Obs.estimation = d2f(((reading - last_Obs.reading) * last_Obs.tau / incT) + reading);
      else
...
void CObservationGasSensors::CMOSmodel::save_log_map(
    const mrpt::system::TTimeStamp& timestamp, float reading, float estimation, float tau)
{
