    ...
    public mrpt::opengl::Visualizable
{
 ...
 private:
  ...
  virtual bool internal_insertObservation(
      ...
      const std::optional<const mrpt::poses::CPose3D>& robotPose = std::nullopt) = 0;

  /** Internal method called by computeObservationLikelihood() */
  virtual double internal_computeObservationLikelihood(
      const mrpt::obs::CObservation& obs, const mrpt::poses::CPose3D& takenFrom) const = 0;
 ...
 public:
...
#if defined(MRPT_BUILDING_PYTHON_WRAPPER)
  bool insertObs(
      const mrpt::obs::CObservation& obs, const mrpt::poses::CPose3D* robotPose = nullptr)
  {
    if (robotPose)
      return insertObservation(obs, std::optional<const mrpt::poses::CPose3D>(*robotPose));
    else
      return insertObservation(obs, std::optional<const mrpt::poses::CPose3D>());
  }
  bool insertObs(
      const mrpt::obs::CSensoryFrame& sf, const mrpt::poses::CPose3D* robotPose = nullptr)
  {
  ...
  double computeObservationsLikelihood(
      const mrpt::obs::CSensoryFrame& sf, const mrpt::poses::CPose3D& takenFrom);
