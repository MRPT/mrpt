   ...
   public:
	...
	std::vector<uint32_t>
		AIN_32bits; /** Readings from 32-bit analog input (ADCs) channels
					   (vector length=channel count) in ADC units. */
	std::vector<float>
		AIN_float; /** Readings from analog input (ADCs) channels (vector
	...
	std::vector<uint8_t>
		AOUT_8bits; /** Present output values for 8-bit analog output (DACs)
	...
	std::vector<float> AOUT_float; /** Present output values for 16-bit analog
									  ...
									  length=channel count) in volts.*/
	std::vector<double>
		AOUT_double; /** Present output values for 16-bit analog output (DACs)
						channels (vector length=channel count) in volts.*/
	...
	std::vector<double>
		CNTRIN_double; /** Readings from ticks counters, such as quadrature
						  encoders. (vector length=channel count) in radians,
						  degrees or any other unit (depends on the source
						  driver). */
