...
namespace poses
{
	...
	DEFINE_SERIALIZABLE_PRE_CUSTOM_BASE( CPoint2D, CPoint )
...
<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0
 ...
 <tr style='height:108.3pt'>
  ...
  <p   align=center style='text-align:center'><img src="CPoint2D.gif"></p>
	 ...
	 */
	class BASE_IMPEXP CPoint2D : public CPoint
	{
	 ...
	 public:
		  ...
		  */
		 explicit CPoint2D( const CPose2D &o );

		 /** Constructor from x/y coordinates given from other pose.
		  */
		 explicit CPoint2D( const CPose3D &o );

		 /** Constructor from x/y coordinates given from a 3D point.
		  */
		 explicit CPoint2D(const CPoint3D &o);
		...
		void  getHomogeneousMatrix(CMatrixDouble44 & out_HM ) const
		{
		 ...
		 /** Returns a human-readable textual representation of the object (eg: "[0.02 1.04]" )
		   * \sa fromString
		   */
		void asString(std::string &s) const { s = mrpt::format("[%f %f]",m_x,m_y); }
