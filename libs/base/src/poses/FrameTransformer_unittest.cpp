...
void run_tf_test1(const mrpt::poses::CPose2D &A2B_)
{
	...
	// Pub:
	const typename FrameTransformer<DIM>::pose_t  real_A2B = typename FrameTransformer<DIM>::pose_t(A2B_);
	...
	// Read:
	{
		...
		EXPECT_NEAR(.0, (real_A2B.getAsVectorVal() - A2B_looked_up.getAsVectorVal()).array().abs().sum(), 1e-6);
	}
