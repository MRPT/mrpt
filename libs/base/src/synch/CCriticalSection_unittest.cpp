...
struct TAux
{
	...
	void run(int)
	{
		if (m_func)
			...
			throw std::runtime_error("functor is NULL!!");
		terminated = true;
	}
...
void launchTestWithTimeout(
	void (*func)(void), double timeout_secs, const std::string& fail_msg)
{
	TAux obj(func);
...
TEST(Synch, CriticalSections_Simple)
{
	launchTestWithTimeout(
		my_CriticalSections_Simple, 5.0, "CriticalSections_Simple");
}
...
void thread_example(int id)
{
	...
	try
	{
		...
		// printf("[thread_example %i, ID:%lu] Started, will run for %f
...
void my_CriticalSections_Multi()
{
	...
	for (int i = 1; i <= 10; i++) createThread(thread_example, i);
...
{
	launchTestWithTimeout(
		my_CriticalSections_Multi, 6.0, "CriticalSections_Multi");
}
