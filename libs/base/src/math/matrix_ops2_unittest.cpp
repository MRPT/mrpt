...
TEST(Matrices,inv_6x6_fix)
{
	...
	CHECK_AND_RET_ERROR( std::isnan(C(0,0)) || !std::isfinite(C(0,0)) || (AInv-C).array().abs().sum() >1e-4,  "Error in inv, 6x6 fix")
}
...
TEST(Matrices,inv_6x6_dyn)
{
	...
	CHECK_AND_RET_ERROR( std::isnan(C(0,0)) || !std::isfinite(C(0,0)) || (AInv-C).array().abs().sum() >1e-4,  "Error in inv, 6x6 dyn")
}
