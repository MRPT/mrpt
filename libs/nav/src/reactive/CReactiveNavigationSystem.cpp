...
// Dtor:
CReactiveNavigationSystem::~CReactiveNavigationSystem() { this->preDestructor(); }
...
void CReactiveNavigationSystem::STEP1_InitPTGs()
{
  if (m_PTGsMustBeReInitialized)
  {
    ...
    for (unsigned int i = 0; i < PTGs.size(); i++)
    {
      ...
      {
        auto* ptg = dynamic_cast<mrpt::nav::CPTG_RobotShape_Polygonal*>(PTGs[i].get());
        if (ptg) ptg->setRobotShape(m_robotShape);
      }
      // Circular robot shape?
      {
        auto* ptg = dynamic_cast<mrpt::nav::CPTG_RobotShape_Circular*>(PTGs[i].get());
