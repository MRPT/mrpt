...
CReactiveNavigationSystem3D::CReactiveNavigationSystem3D(
    ...
    CAbstractPTGBasedReactive(
        react_iterf_impl, enableConsoleOutput, enableLogToFile, logFileDirectory)
{
}
  ...
  if (m_PTGsMustBeReInitialized)
  {
    ...
    for (unsigned int j = 0; j < m_ptgmultilevel.size(); j++)
    {
      for (unsigned int i = 0; i < m_robotShape.size(); i++)
      {
        ...
        {
          auto* ptg =
              dynamic_cast<mrpt::nav::CPTG_RobotShape_Polygonal*>(m_ptgmultilevel[j].PTGs[i].get());
          if (ptg) ptg->setRobotShape(m_robotShape.polygon(i));
        }
        // Circular robot shape?
        {
          auto* ptg =
              dynamic_cast<mrpt::nav::CPTG_RobotShape_Circular*>(m_ptgmultilevel[j].PTGs[i].get());
          if (ptg) ptg->setRobotShapeRadius(m_robotShape.getRadius(i));
        }
