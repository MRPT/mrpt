...
void CHierarchicalMHMap::readFromStream(mrpt::utils::CStream& in, int version)
{
	switch (version)
	{
		case 0:
		{
			...
			for (i = 0; i < n; i++)
			{
				CHMHMapNode::Ptr node = std::make_aligned_shared<CHMHMapNode>(
			...
			for (i = 0; i < n; i++)
			{
				...
				CHMHMapArc::Ptr arc = std::make_aligned_shared<CHMHMapArc>(
...
void CHierarchicalMHMap::loadFromXMLfile(std::string fileName)
{
	...
	for (j = 0; j < numnodes; j++)
	{
		...
		node = std::make_aligned_shared<CHMHMapNode>(this);
	...
	for (j = 0; j < numarcs; j++)
	{
		...
		arc = std::make_aligned_shared<CHMHMapArc>(nodefrom, nodeto, 0, this);
		...
		if (atoi(table->get(j, "bidirectional").c_str()) == 1)
		{
			...
			arcrev = std::make_aligned_shared<CHMHMapArc>(nodeto, nodefrom, 0, this);
	...
	for (size_t j = 0; j < numannot; j++)
	{
		...
		if (nodeanotmapit != nodeanotmap.end())
		{
			if (type == "placePose")
			{
				CPoint2D::Ptr o = std::make_aligned_shared<CPoint2D>();
				o->fromString(value);
