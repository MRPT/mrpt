...
getAs3DObject(
	...
	const bool show_ID_labels = 0 != viz_params.getWithDefaultVal(
			"show_ID_labels", 0);
	const bool show_ground_grid = 0 != viz_params.getWithDefaultVal(
			"show_ground_grid", 1);
	const bool show_edges = 0 != viz_params.getWithDefaultVal(
			"show_edges", 1);
	const bool show_node_corners = 0 != viz_params.getWithDefaultVal(
			"show_node_corners", 1);
	const bool show_edge_rel_poses = 0 != viz_params.getWithDefaultVal(
...
drawGroundGrid(mrpt::opengl::CSetOfObjects::Ptr& object,
	...
	for (typename GRAPH_T::global_poses_t::const_iterator
			n_it = m_graph.nodes.begin();
			n_it!=m_graph.nodes.end();
			++n_it) {
...
template<class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::
drawNodeCorners(mrpt::opengl::CSetOfObjects::Ptr& object,
		const mrpt::utils::TParametersDouble* viz_params/*=NULL*/) const {
	using namespace mrpt::opengl;
	using namespace mrpt::utils;
	...
	const bool show_ID_labels = 0 != viz_params->getWithDefaultVal(
			"show_ID_labels", 0);
	const double nodes_corner_scale = viz_params->getWithDefaultVal(
	...
	for (typename GRAPH_T::global_poses_t::const_iterator
		...
		// Convert to 3D from whatever its real type.  CSetOfObjects::Ptr gl_corner = show_node_corners ?
...
template<class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::
drawEdgeRelPoses(mrpt::opengl::CSetOfObjects::Ptr& object,
		const mrpt::utils::TParametersDouble* viz_params/*=NULL*/) const {
	using namespace mrpt::opengl;
	using namespace mrpt::utils;
	ASSERTMSG_(viz_params, "Pointer to viz_params was not provided.");

	const double nodes_edges_corner_scale = viz_params->getWithDefaultVal(
	...
	for (typename GRAPH_T::const_iterator edge_it = m_graph.begin();
		...
		typename GRAPH_T::global_poses_t::const_iterator n_it = m_graph.nodes.find(node_id_start);
		if (n_it!=m_graph.nodes.end()) {
			...
			mrpt::opengl::CSimpleLine::Ptr gl_line =
				mrpt::opengl::CSimpleLine::Create(
						0,0,0,
						edge_pose_pt.x(), edge_pose_pt.y(), edge_pose_pt.z());
...
drawEdges(mrpt::opengl::CSetOfObjects::Ptr& object,
	...
	for (typename GRAPH_T::const_iterator
		...
		typename GRAPH_T::global_poses_t::const_iterator n_it1 = m_graph.nodes.find(id1);
		typename GRAPH_T::global_poses_t::const_iterator n_it2 = m_graph.nodes.find(id2);
		if (n_it1!=m_graph.nodes.end() && n_it2!=m_graph.nodes.end()) { // both nodes found?
