...
void CFaceDetection::experimental_viewFacePointsAndEigenVects(  const vector<CArrayDouble<3> > &pointsVector, const CMatrixDouble &eigenVect, const CVectorDouble &eigenVal )
{
	...
	CArrow::Ptr arrow1 = std::make_shared<CArrow>( center.x, center.y, center.z, p1.x, p1.y, p1.z );
	CArrow::Ptr arrow2 = std::make_shared<CArrow>( center.x, center.y, center.z, p2.x, p2.y, p2.z );
	CArrow::Ptr arrow3 = std::make_shared<CArrow>( center.x, center.y, center.z, p3.x, p3.y, p3.z );
	...
	//mrpt::opengl::CSetOfLines::Ptr lines = std::make_shared<mrpt::opengl::CSetOfLines>( sgms );
...
void CFaceDetection::experimental_viewRegions( const vector<TPoint3D> regions[9], const TPoint3D meanPos[3][3] )
{
	...
	scene->insert( gl_points );
	scene->insert( std::make_shared<mrpt::opengl::CGridPlaneXY>() );
	scene->insert(std::make_shared<mrpt::opengl::CAxis>(-5,-5,-5,5,5,5,2.5,3,true));
