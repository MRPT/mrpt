...
DECLARE_OP_FUNCTION(op_deexternalize)
{
  ...
  class CRawlogProcessor_DeExternalize : public CRawlogProcessorOnEachObservation
  {
   protected:
   ...
   public:
    ...
    bool processOneObservation(CObservation::Ptr& obs) override
    {
      ...
      if (IS_CLASS(*obs, CObservationStereoImages))
      {
        ...
        {
          bool loadOk =
              obsSt->imageLeft.loadFromFile(obsSt->imageLeft.getExternalStorageFileAbsolutePath());
          ASSERT_(loadOk);
          entries_converted++;
        }
        else
      ...
      else if (IS_CLASS(*obs, CObservationImage))
      {
        ...
        {
          bool loadOk =
              obsIm->image.loadFromFile(obsIm->image.getExternalStorageFileAbsolutePath());
          ASSERT_(loadOk);
          entries_converted++;
        }
        else
      ...
      {
        auto obs3D = std::dynamic_pointer_cast<CObservation3DRangeScan>(obs);

        if (obs3D->rangeImage_isExternallyStored() || obs3D->points3D_isExternallyStored() ||
