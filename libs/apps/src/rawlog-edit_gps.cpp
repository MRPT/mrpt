  ...
  class CRawlogProcessor_ExportGPS_KML : public CRawlogProcessorOnEachObservation
  {
   ...
   public:
    ...
    bool processOneObservation(CObservation::Ptr& o) override
    {
      ...
      const auto& gga = obs->getMsgByClass<mrpt::obs::gnss::Message_NMEA_GGA>();
    ...
    void generate_KML()
    {
      ...
      int color_idx = 0;
      for (auto it = m_gps_paths.begin(); it != m_gps_paths.end(); ++it, color_idx++)
      {
        ...
        if (hasSomeRTK)
        {
          ...
          for (const auto& itP : D.path)
          {
            ...
            if (last_valid.lat != 0 && last_valid.lon != 0)
            {
              ...
              const double dist =
                  6.371e6 * DEG2RAD(::hypot(last_valid.lon - d.lon, last_valid.lat - d.lat));
   ...
   protected:
        ...
        const char* gpsKindLabel)
    {
      ...
      for (auto a = lstxyz.begin(); a != lstxyz.end(); ++a, nLabels++)
      {
        ...
        auto& m = a->second;
        int k = 0;
        for (auto it = lstlabels.begin(); it != lstlabels.end(); ++it, k++)
        {
    ...
    bool processOneObservation(CObservation::Ptr& o) override
    {
      ...
      if (it == lstFiles.end())  // A new file for this sensorlabel??
      {
        ...
        ::fprintf(
            ...
            "%14s "               // SAT Time
            "\n",
            "Time", "Lat", "Lon", "Alt", "fix", "#sats", "speed", "dir", "Local X", "Local Y",
            "Local Z", "rawlog ID", "Geocen X", "Geocen Y", "Geocen Z", "GPS X", "GPS Y", "GPS Z",
            "GPS VX", "GPS VY", "GPS VZ", "Local VX", "Local VY", "Local VZ", "SAT Time");
      }
      else
        f_this = it->second;
      ...
      if (obs->has_GGA_datum())
      {
        ...
        mrpt::topography::geodeticToENU_WGS84(gga.getAsStruct<TGeodeticCoords>(), p, refCoords);

        // Geocentric XYZ:
        TPoint3D geo;
        mrpt::topography::geodeticToGeocentric_WGS84(gga.getAsStruct<TGeodeticCoords>(), geo);
        ...
        {
          const auto& pzs = obs->getMsgByClass<mrpt::obs::gnss::Message_TOPCON_PZS>();
        ...
        ::fprintf(
            ...
            "%14.4f "                   // SAT Time
            "\n",
            tim, DEG2RAD(gga.fields.latitude_degrees), DEG2RAD(gga.fields.longitude_degrees),
            gga.fields.altitude_meters, gga.fields.fix_quality, gga.fields.satellitesUsed,
            obs->has_RMC_datum()
                ? DEG2RAD(
                      obs->getMsgByClass<mrpt::obs::gnss::Message_NMEA_RMC>().fields.speed_knots)
                : 0.0,
            obs->has_RMC_datum() ? DEG2RAD(obs->getMsgByClass<mrpt::obs::gnss::Message_NMEA_RMC>()
  ...
  class CRawlogProcessor_ExportGPS_ALL : public CRawlogProcessorOnEachObservation
  {
   ...
   public:
    ...
    bool processOneObservation(CObservation::Ptr& o) override
    {
      ...
      for (auto it = obs->messages.begin(); it != obs->messages.end(); ++it)
      {
