...
bool CEnoseModular::getObservation(mrpt::obs::CObservationGasSensors& obs)
{
  try
  {
    ...
    {
      if (mrpt::system::timeDifference(t1, mrpt::Clock::now()) > time_out_val) time_out = true;
    }
    ...
    if (msg.content.size() > 0)
    {
      ...
      if (enose_poses_x.size() != 0)
      {
        newRead.eNosePoseOnTheRobot = TPose3D(
            enose_poses_x[0], enose_poses_y[0], enose_poses_z[0], enose_poses_yaw[0],
            enose_poses_pitch[0], enose_poses_roll[0]);
      }
      else
