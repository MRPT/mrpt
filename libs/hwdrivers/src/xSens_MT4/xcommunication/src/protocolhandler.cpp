...
int expectedMessageSize(const unsigned char* buffer, int sz)
{
	...
	{
		if (sz < 6)
			return XS_EXTLENCODE + XS_LEN_MSGEXTHEADERCS;  // typical minimum
		// size at which
		// point extended
		// size is needed
	...
	for (int pre = 0; pre < bufferSize; ++pre)
	{
		if (buffer[pre] == XS_PREAMBLE)
		{
			...
			if (rcv.loadFromString(msgStart, (uint16_t)target))
			{
				...
				rv.m_startPos = pre;  // we do this again here because this may
				// not be the first preamble encountered
				// (the check for -1 at the start of the
				// loop is necessary)
				return rv;
			}
