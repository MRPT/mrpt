...
struct XsMessageHeader
{
	...
	union LengthData {
		...
		struct ExtendedLength
		{
			 ...
			 * always at least 1 byte since it has to contain the checksum, but
			 * it can be bigger. */
			uint8_t m_data[1];
...
struct XsMessage
{
	...
	*/
	...
	/*! \brief Return current data value as a double(64 bits), after converting
	  it from float, double, FP1632 or FP1220 depending on outputSettings
