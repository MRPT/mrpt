    ...
    const mrpt::config::CConfigFileBase& configSource, const std::string& iniSection)
{
  ...
  m_raw_dump_file_prefix = configSource.read_string(
      iniSection, "raw_dump_file_prefix", m_raw_dump_file_prefix, false /*Allow default values*/);
  ...
  {
    std::string sLine =
        configSource.read_string(iniSection, mrpt::format("setup_cmd%i", i), std::string());
  ...
  {
    std::string sLine =
        configSource.read_string(iniSection, mrpt::format("shutdown_cmd%i", i), std::string());
    sLine = mrpt::system::trim(sLine);
    if (sLine.empty()) break;
  ...
  m_JAVAD_rtk_src_port =
      configSource.read_string(iniSection, "JAVAD_rtk_src_port", m_JAVAD_rtk_src_port);
  m_JAVAD_rtk_src_baud =
      configSource.read_int(iniSection, "JAVAD_rtk_src_baud", m_JAVAD_rtk_src_baud);
  ...
  m_topcon_data_period =
      1.0 / configSource.read_double(iniSection, "outputRate", m_topcon_data_period);
}

CGPSInterface::~CGPSInterface() { OnConnectionShutdown(); }

void CGPSInterface::setParser(CGPSInterface::PARSERS parser) { m_parser = parser; }
    ...
    const std::shared_ptr<std::mutex>& csOptionalExternalStream)
{
  ...
  m_data_stream_cs = csOptionalExternalStream ? csOptionalExternalStream : m_data_stream_mine_cs;
}
void CGPSInterface::setSetupCommandsDelay(const double delay_secs)
{
  m_custom_cmds_delay = delay_secs;
}
double CGPSInterface::getSetupCommandsDelay() const { return m_custom_cmds_delay; }
void CGPSInterface::setSetupCommands(const std::vector<std::string>& cmds) { m_setup_cmds = cmds; }
const std::vector<std::string>& CGPSInterface::getSetupCommands() const { return m_setup_cmds; }
...
bool CGPSInterface::isEnabledSetupCommandsAppendCRLF() const { return m_custom_cmds_append_CRLF; }
...
void CGPSInterface::setSerialPortName(const std::string& COM_port)
{
  ...
  if (m_data_stream)
  {
    ...
    auto serial = dynamic_cast<mrpt::comms::CSerialPort*>(m_data_stream.get());
...
void CGPSInterface::doProcess()
{
  ...
  {  // "Advanced" (RTK,mmGPS) device  (kept for
    ...
    else
    {
      const double time_diff =
          mrpt::system::timeDifference(m_last_timestamp, m_parsed_messages->timestamp);
      ...
      do_append_obs = (m_parsed_messages->has_GGA_datum() && m_parsed_messages->has_RMC_datum());
    }  // end-else
...
void CGPSInterface::flushParsedMessagesNow()
{
  ...
  if (m_sensorLabelAppendMsgType)
    m_parsed_messages->sensorLabel = m_sensorLabel + string("_") + m_parsed_messages->sensorLabel;
  else
...
void CGPSInterface::parseBuffer()
{
  ...
  if (parser_ptr)
  {
    ...
    do
    {
      ...
      {
        if (m_rx_buffer.size() != 0) m_rx_buffer.pop();  // Not the start of a frame, skip 1 byte
      }
  ...
  else
  {
    ...
    do
    {
      ...
      for (auto all_parser : all_parsers)
      {
        ...
        if ((*this.*parser_ptr)(this_parser_min_bytes)) all_parsers_want_to_skip = false;
