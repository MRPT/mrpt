...
void CCameraSensor::getNextFrame(vector<CSerializable::Ptr>& out_obs)
{
	...
	if (m_cap_cv)
	{
		obs = std::make_aligned_shared<CObservationImage>();
	...
	{
		obs3D = std::make_aligned_shared<CObservation3DRangeScan>();
	...
	{
		stObs = std::make_aligned_shared<CObservationStereoImages>();
	...
	{
		stObs = std::make_aligned_shared<CObservationStereoImages>();
		obsIMU = std::make_aligned_shared<CObservationIMU>();
	...
	{  // Yes
		if (++m_preview_counter > m_preview_decimation)
		{
			...
			if (!m_preview_win1)
			{
				...
				m_preview_win1 =
					std::make_aligned_shared<mrpt::gui::CDisplayWindow>(caption);
			}
...
CCameraSensor::Ptr mrpt::hwdrivers::prepareVideoSourceFromUserSelection()
{
	...
	CCameraSensor::Ptr cam = std::make_aligned_shared<CCameraSensor>();
...
CCameraSensor::Ptr mrpt::hwdrivers::prepareVideoSourceFromPanel(void* _panel)
{
	...
	try
	{
		...
		CCameraSensor::Ptr video = std::make_aligned_shared<CCameraSensor>();
