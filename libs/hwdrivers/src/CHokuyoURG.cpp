    ...
    bool& hardwareError)
{
  ...
  if (do_timestamp_sync)
  {
    ...
    uint32_t nowUI = ((m_rcv_data[0] - 0x30) << 18) + ((m_rcv_data[1] - 0x30) << 12) +
    ...
    const mrpt::config::CConfigFileBase& c, const std::string& s)
{
  ...
  m_sensorPose.setFromValues(
      c.read_float(s, "pose_x", 0), c.read_float(s, "pose_y", 0), c.read_float(s, "pose_z", 0),
      DEG2RAD(c.read_float(s, "pose_yaw", 0)), DEG2RAD(c.read_float(s, "pose_pitch", 0)),
  ...
  m_disable_firmware_timestamp =
      c.read_bool(s, "disable_firmware_timestamp", m_disable_firmware_timestamp);
  ...
  MRPT_LOAD_HERE_CONFIG_VAR(comms_between_timeout_ms, int, m_comms_between_timeout_ms, c, s);
...
bool CHokuyoURG::turnOn()
{
  ...
  if (m_reduced_fov > 0 && m_reduced_fov < 2 * M_PI)
  {
    ...
    const int half_range =
        static_cast<int>((m_sensor_info.scans_per_360deg / 360.0) * RAD2DEG(m_reduced_fov)) >> 1;
    m_firstRange = center - half_range;
    m_lastRange = center + half_range;
    MRPT_LOG_INFO_STREAM(
        "[HOKUYO::turnOn] Using reduced FOV: ranges ["
        << m_firstRange << "-" << m_lastRange << "] for " << RAD2DEG(m_reduced_fov) << " deg. FOV");
  }
...
-------------------------------------------------------------*/
bool CHokuyoURG::ensureBufferHasBytes(size_t nDesiredBytes, bool additionalWaitForData)
{
  ...
  try
  {
    ...
    {
      if (auto sock = dynamic_cast<CClientTCPSocket*>(m_stream.get()); sock)
      {
        // socket:
        auto timeout = m_comms_timeout_ms;
        if (additionalWaitForData) mrpt::keep_max(timeout, 100);

        return sock->readAsync(&buf[0], to_read, timeout, m_comms_between_timeout_ms);
      }
      else
      {
        // serial port:
  ...
  try
  {
    ...
    for (;;)
    {
      ...
      else
      {
        ...
        MRPT_LOG_DEBUG_STREAM("[Hokuyo] parseResponse(): RX `" << m_rcv_data << "`");
...
void CHokuyoURG::setScanInterval(unsigned int skipScanCount) { m_scan_interval = skipScanCount; }
...
bool CHokuyoURG::setMotorSpeed(int motoSpeed_rpm)
{
  ...
  {
    MRPT_LOG_ERROR_STREAM(
        __CURRENT_FUNCTION_NAME__ << " Motorspeed must be in the range 540-600 rpm");
    return false;
  }
...
bool CHokuyoURG::startScanningMode()
{
  ...
  os::sprintf(
      cmd, 50, "M%c%04u%04u01%u00\x0A", m_intensity ? 'E' : 'D', m_firstRange, m_lastRange,
      scan_interval);
...
bool CHokuyoURG::ensureStreamIsOpen()
{
  ...
  if (m_stream)
  {
    ...
    else
    {
      ...
      if (COM != nullptr)
      {
        ...
        // It has been disconnected... try to reconnect:
        MRPT_LOG_ERROR_STREAM(
            __CURRENT_FUNCTION_NAME__ << ": Serial port connection lost! Trying to reconnect...");
  ...
  else
  {
    ...
    if (!m_ip_dir.empty())
    {
      ...
      {
        MRPT_LOG_ERROR_STREAM(
            __CURRENT_FUNCTION_NAME__ << " Cannot connect with the server '" << m_com_port << "'");
        return false;
      }
    ...
    else
    {
      ...
      {
        MRPT_LOG_ERROR_STREAM(
            __CURRENT_FUNCTION_NAME__ << " Cannot open serial port '" << m_com_port << "'");
        return false;
      }
