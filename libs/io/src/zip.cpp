...
void mrpt::io::zip::compress(
    const std::vector<unsigned char>& inData, std::vector<unsigned char>& outData)
{
  int ret = 0;
  MRPT_START
...
/*---------------------------------------------------------------
            compress
---------------------------------------------------------------*/
void mrpt::io::zip::compress(const std::vector<unsigned char>& inData, CStream& out)
{
  int ret = 0;
  MRPT_START
...
---------------------------------------------------------------*/
size_t mrpt::io::zip::decompress(
    CStream& inStream, size_t inDataSize, void* outData, size_t outDataBufferSize)
{
  int ret = 0;
  MRPT_START
  ...
  ret =
      ::uncompress((unsigned char*)outData, &actualOutSize, &inData[0], (unsigned long)inDataSize);
...
bool mrpt::io::zip::compress_gz_file(
    const std::string& file_path, const std::vector<uint8_t>& buffer, const int compress_level)
{
...
#ifdef _WIN32
  // Windows:
  pipe_file_name = mrpt::format("\\\\.\\pipe\\mrpt_compress_gz_data_block_%u", nPipeName);

  HANDLE hPipe = CreateNamedPipeA(
      pipe_file_name.c_str(), PIPE_ACCESS_DUPLEX | 0x00080000 /* FILE_FLAG_FIRST_PIPE_INSTANCE */,
      PIPE_TYPE_BYTE, PIPE_UNLIMITED_INSTANCES, in_data.size() + 1000, in_data.size() + 1000, 0,
      nullptr);
  if (hPipe == INVALID_HANDLE_VALUE)
    THROW_EXCEPTION("Error creating named pipe for gz-file compression");
#else
  // Unix:
  pipe_file_name = mrpt::format("/tmp/mrpt_compress_gz_data_block_%u", nPipeName);
#endif
