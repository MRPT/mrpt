...
void TCamera::saveToConfigFile(const std::string& section, mrpt::config::CConfigFileBase& cfg) const
{
  ...
  cfg.write(
      section, "resolution", mrpt::format("[%u %u]", (unsigned int)ncols, (unsigned int)nrows));
    ...
    const std::string& section, const mrpt::config::CConfigFileBase& cfg)
{
  *this = TCamera();  // reset to defaults
  ...
  cameraName = cfg.read_string(section, "camera_name", cameraName, false /* optional value */);
}
...
bool mrpt::img::operator==(const mrpt::img::TCamera& a, const mrpt::img::TCamera& b)
{
  return a.ncols == b.ncols && a.nrows == b.nrows && a.intrinsicParams == b.intrinsicParams &&
         a.dist == b.dist && a.focalLengthMeters == b.focalLengthMeters &&
...
mrpt::containers::yaml TCamera::asYAML() const
{
  ...
  p["rectification_matrix"] =
      mrpt::containers::yaml::FromMatrix(mrpt::math::CMatrixDouble33::Identity());
