...
class KDTreeCapable
{
 public:
      ...
      float x0, float y0, float& out_x, float& out_y, float& out_dist_sqr) const
  {
    ...
    const std::array<num_t, 2> query_point{
        {x0, y0}
    };
  ...
  inline std::vector<size_t> kdTreeNClosestPoint2D(
      ...
      const std::optional<float>& maximumSearchDistanceSqr = std::nullopt) const
  {
    MRPT_START
    rebuild_kdTree_2D();  // First: Create the 2D KD-Tree if required
    if (!m_kdtree2d_data.m_num_points) THROW_EXCEPTION("There are no points in the KD-tree.");
      ...
      const std::optional<float>& maximumSearchDistanceSqr = std::nullopt) const
  {
    ...
    std::vector<size_t> res = kdTreeNClosestPoint2D(
        d2f(p0.x), d2f(p0.y), N, dmy1, dmy2, outDistSqr, maximumSearchDistanceSqr);
  ...
  {
    return kdTreeNClosestPoint2DIdx(d2f(p0.x), d2f(p0.y), N, outIdx, outDistSqr);
  }
      ...
      const
  {
    ...
    const std::array<num_t, 3> query_point{
        {x0, y0, z0}
    };
  ...
  inline size_t kdTreeClosestPoint3D(const TPoint3D& p0, TPoint3D& pOut, float& outDistSqr) const
  {
    ...
    size_t res =
        kdTreeClosestPoint3D(d2f(p0.x), d2f(p0.y), d2f(p0.z), dmy1, dmy2, dmy3, outDistSqr);
      ...
      const std::optional<float>& maximumSearchDistanceSqr = std::nullopt) const
  {
    ...
    kdTreeNClosestPoint3D(
        d2f(p0.x), d2f(p0.y), d2f(p0.z), N, dmy1, dmy2, dmy3, outDistSqr, maximumSearchDistanceSqr);
    pOut.resize(dmy1.size());
    for (size_t i = 0; i < dmy1.size(); i++)
    {
      pOut[i].x = static_cast<double>(dmy1[i]);
  ...
  inline size_t kdTreeRadiusSearch3D(
      ...
      std::vector<nanoflann::ResultItem<size_t, num_t>>& out_indices_dist) const
  {
    MRPT_START
    rebuild_kdTree_3D();  // First: Create the 3D KD-Tree if required
    ...
    if (!m_kdtree2d_data.index)
    {
      ...
      if (N)
      {
        m_kdtree2d_data.index = std::make_unique<tree2d_t>(
            2, derived(),
            nanoflann::KDTreeSingleIndexAdaptorParams(kdtree_search_params.leaf_max_size));
