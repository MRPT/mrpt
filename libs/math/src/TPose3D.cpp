    ...
    mrpt::optional_ref<mrpt::math::CMatrixFixed<double, 4, 3>> out_dq_dr) const
{
  ...
  if (out_dq_dr)
  {
    alignas(MRPT_MAX_STATIC_ALIGN_BYTES) const double nums[4 * 3] = {
        -0.5 * q[3], 0.5 * (-csc + scs), -0.5 * q[1], -0.5 * q[2], 0.5 * (-ssc - ccs), 0.5 * q[0],
        0.5 * q[1],  0.5 * (ccc - sss),  0.5 * q[3],  0.5 * q[0],  0.5 * (-css - scc), -0.5 * q[2]};
...
{
  ASSERTDEBMSG_(
      std::abs(sqrt(square(R(0, 0)) + square(R(1, 0)) + square(R(2, 0))) - 1) < 3e-3,
      "Homogeneous matrix is not orthogonal & normalized!: " + R.inMatlabFormat());
  ASSERTDEBMSG_(
      std::abs(sqrt(square(R(0, 1)) + square(R(1, 1)) + square(R(2, 1))) - 1) < 3e-3,
      "Homogeneous matrix is not orthogonal & normalized!: " + R.inMatlabFormat());
  ASSERTDEBMSG_(
      std::abs(sqrt(square(R(0, 2)) + square(R(1, 2)) + square(R(2, 2))) - 1) < 3e-3,
      "Homogeneous matrix is not orthogonal & normalized!: " + R.inMatlabFormat());
...
{
  SO3_to_yaw_pitch_roll(CMatrixDouble33(HG.blockCopy<3, 3>(0, 0)), yaw, pitch, roll);
  x = HG(0, 3);
  y = HG(1, 3);
  z = HG(2, 3);
}
void TPose3D::composePose(const TPose3D other, TPose3D& result) const
{
  ...
  result.fromHomogeneousMatrix(CMatrixDouble44(me_H.asEigen() * o_H.asEigen()));
}
