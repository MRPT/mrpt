...
TLine2D TLine2D::FromCoefficientsABC(double A, double B, double C) { return TLine2D(A, B, C); }
...
{
  return std::abs(evaluatePoint(point)) / sqrt(coefs[0] * coefs[0] + coefs[1] * coefs[1]);
}
...
{
  return mrpt::format("[%10.05f, %10.05f, %10.05f]", coefs[0], coefs[1], coefs[2]);
}
