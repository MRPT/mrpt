...
void CSparseMatrix::cs2dense(const cs& SM, CMatrixDouble& d_M)
{
  ...
  {                // It's in triplet form.
    for (int idx = 0; idx < SM.nz; ++idx)
      d_M(SM.i[idx], SM.p[idx]) += SM.x[idx];  // += since the convention is that duplicate (i,j)
...
void CSparseMatrix::get_dense(CMatrixDouble& d_M) const { cs2dense(sparse_matrix, d_M); }
...
CSparseMatrix::CholeskyDecomp::CholeskyDecomp(const CSparseMatrix& SM) :
    m_symbolic_structure(nullptr), m_numeric_structure(nullptr), m_originalSM(&SM)
{
...
void CSparseMatrix::CholeskyDecomp::backsub(const CVectorDouble& b, CVectorDouble& sol) const
{
  ASSERT_(b.size() > 0);
  sol.resize(b.size());
...
/** Return the vector from a back-substitution step that solves: Ux=b   */
void CSparseMatrix::CholeskyDecomp::backsub(const double* b, double* sol, size_t N) const
{
  ASSERT_(N > 0);
