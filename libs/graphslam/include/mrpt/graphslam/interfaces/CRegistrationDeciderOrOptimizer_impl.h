...
void CRegistrationDeciderOrOptimizer<GRAPH_T>::initializeLoggers(const std::string& name)
{
...
template <class GRAPH_T>
void CRegistrationDeciderOrOptimizer<GRAPH_T>::setClassName(const std::string& name)
{
...
template <class GRAPH_T>
void CRegistrationDeciderOrOptimizer<GRAPH_T>::setCriticalSectionPtr(std::mutex* graph_section)
{
  m_graph_section = graph_section;
  this->logFmt(mrpt::system::LVL_DEBUG, "Fetched the CCRiticalSection successfully");
}
