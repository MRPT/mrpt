...
template <class GRAPH_T>
typename GRAPH_T::global_pose_t CGraphSlamEngine<GRAPH_T>::getCurrentRobotPosEstimation() const
{
...
template <class GRAPH_T>
typename GRAPH_T::global_poses_t CGraphSlamEngine<GRAPH_T>::getRobotEstimatedTrajectory() const
{
  std::lock_guard<std::mutex> graph_lock(m_graph_section);
...
void CGraphSlamEngine<GRAPH_T>::initClass()
{
  ...
  // Assert that the graph class used is supported.
  {
    ...
    bool found =
        (std::find(
             m_supported_constraint_types.begin(), m_supported_constraint_types.end(), c_str) !=
    ...
    else
    {
      MRPT_LOG_WARN_STREAM(
          "Given graph class " << c_str << " has not been tested consistently yet."
  ...
  if (m_enable_visuals)
  {
    m_win_manager->assignTextMessageParameters(&m_offset_y_timestamp, &m_text_index_timestamp);
  }
  ...
  if (mrpt::system::strCmpI(m_GT_file_format, "rgbd_tum"))
  {
    ...
    m_img_external_storage_dir = rawlog_dir + rawlog_fname_noext + "_Images/";
  ...
  if (m_enable_visuals)
  {
    ...
    m_win_observer->registerKeystroke(m_keystroke_pause_exec, "Pause/Resume program execution");
    m_win_observer->registerKeystroke(m_keystroke_odometry, "Toggle Odometry visualization");
    m_win_observer->registerKeystroke(m_keystroke_GT, "Toggle Ground-Truth visualization");
    m_win_observer->registerKeystroke(
        m_keystroke_estimated_trajectory, "Toggle Estimated trajectory visualization");
    m_win_observer->registerKeystroke(m_keystroke_map, "Toggle Map visualization");
  }
  ...
  if (m_enable_visuals)
  {
    ...
    m_win_manager->assignTextMessageParameters(&offset_y_total_edges, &text_index_total_edges);
    ...
    {
      m_win_manager->assignTextMessageParameters(&name_to_offset_y[*it], &name_to_text_index[*it]);
    }

    m_win_manager->assignTextMessageParameters(&offset_y_loop_closures, &text_index_loop_closures);

    // add all the parameters to the CEdgeCounter object
    m_edge_counter.setTextMessageParams(
        name_to_offset_y, name_to_text_index, offset_y_total_edges, text_index_total_edges,
  ...
  if (m_enable_visuals)
  {
    m_win_manager->assignTextMessageParameters(
        &m_offset_y_current_constraint_type, &m_text_index_current_constraint_type);
    m_win_manager->addTextMessage(
        m_offset_x_left, -m_offset_y_current_constraint_type, m_current_constraint_type,
        mrpt::img::TColorf(m_current_constraint_type_color), m_text_index_current_constraint_type);
  }
  ...
  {
    mrpt::maps::COccupancyGridMap2D::Ptr gridmap = mrpt::maps::COccupancyGridMap2D::Create();
    ...
    mrpt::obs::CObservation::Ptr& observation, size_t& rawlog_entry)
{
  ...
  return this->_execGraphSlamStep(action, observations, observation, rawlog_entry);
    ...
    size_t& rawlog_entry)
{
  ...
  ASSERTDEBMSG_(m_has_read_config, "\nConfig file is not read yet.\nExiting... \n");
  // good to go..
  ...
  bool registered_new_node;
  {
    ...
    registered_new_node = m_node_reg->updateState(action, observations, observation);
  ...
  if (registered_new_node)
  {
    ...
    if (m_is_first_time_node_reg)
    {
      ...
      m_nodes_to_laser_scans2D.insert(make_pair(m_nodeID_max, m_first_laser_scan2D));
  ...
  if (registered_new_node)
  {
    ...
    {
      for (auto it = edge_types_to_nums.begin(); it != edge_types_to_nums.end(); ++it)
      {
        // loop closure
  ...
  m_dataset_grab_time = mrpt::system::timeDifference(m_init_timestamp, m_curr_timestamp);
    ...
    bool registered /*=false*/, std::string class_name /*="Class"*/)
{
  ...
  std::lock_guard<std::mutex> graph_lock(m_graph_section);
  size_t listed_nodeCount = (m_nodeID_max == mrpt::graphs::INVALID_NODEID ? 0 : m_nodeID_max + 1);
...
void CGraphSlamEngine<GRAPH_T>::getMap(
    mrpt::maps::COccupancyGridMap2D::Ptr map, mrpt::system::TTimeStamp* acquisition_time) const
{
  MRPT_START
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::getMap(
    mrpt::maps::COctoMap::Ptr map, mrpt::system::TTimeStamp* acquisition_time) const
{
  MRPT_START
  THROW_EXCEPTION("Not Implemented Yet.");
...
inline void CGraphSlamEngine<GRAPH_T>::computeMap() const
{
  ...
  {  // 2D Poses
    ...
    // poses
    for (auto it = m_nodes_to_laser_scans2D.begin(); it != m_nodes_to_laser_scans2D.end(); ++it)
    {
...
void CGraphSlamEngine<GRAPH_T>::loadParams(const std::string& fname)
{
  ...
  m_user_decides_about_output_dir =
      cfg_file.read_bool("GeneralConfiguration", "user_decides_about_output_dir", false, false);
  m_GT_file_format =
      cfg_file.read_string("GeneralConfiguration", "ground_truth_file_format", "NavSimul", false);

  // Minimum verbosity level of the logger
  int min_verbosity_level = cfg_file.read_int("GeneralConfiguration", "class_verbosity", 1, false);
  this->setMinLoggingLevel(mrpt::system::VerbosityLevel(min_verbosity_level));

  // Section: VisualizationParameters

  // map visualization
  m_visualize_map = cfg_file.read_bool("VisualizationParameters", "visualize_map", true, false);

  // odometry-only visualization
  m_visualize_odometry_poses =
      cfg_file.read_bool("VisualizationParameters", "visualize_odometry_poses", true, false);
  m_visualize_estimated_trajectory =
      cfg_file.read_bool("VisualizationParameters", "visualize_estimated_trajectory", true, false);

  // GT configuration visualization
  m_visualize_GT =
      cfg_file.read_bool("VisualizationParameters", "visualize_ground_truth", true, false);
  // SLAM metric plot
  m_visualize_SLAM_metric =
      cfg_file.read_bool("VisualizationParameters", "visualize_SLAM_metric", true, false);

  // Viewports flags
  m_enable_curr_pos_viewport =
      cfg_file.read_bool("VisualizationParameters", "enable_curr_pos_viewport", true, false);
  m_enable_range_viewport =
      cfg_file.read_bool("VisualizationParameters", "enable_range_viewport", false, false);
  m_enable_intensity_viewport =
      cfg_file.read_bool("VisualizationParameters", "enable_intensity_viewport", false, false);
...
void CGraphSlamEngine<GRAPH_T>::updateRangeImageViewport()
{
  ...
  if (m_last_laser_scan3D->hasRangeImage)
  {
    ...
    img.setFromMatrix(m_last_laser_scan3D->rangeImage, false /* do normalize */);
    ...
    std::vector<mrpt::system::TTimeStamp>* gt_timestamps)
{
  ...
  for (size_t line_num = 0; file_GT.readLine(curr_line); line_num++)
  {
    ...
    if (gt_timestamps)
    {
      auto timestamp = mrpt::Clock::fromDouble(atof(curr_tokens[0].c_str()));
      gt_timestamps->push_back(timestamp);
    }

    // pose
    pose_t curr_pose(
        atof(curr_tokens[1].c_str()), atof(curr_tokens[2].c_str()), atof(curr_tokens[3].c_str()));
...
void CGraphSlamEngine<GRAPH_T>::alignOpticalWithMRPTFrame()
{
  ...
  const double tmpz[] = {cos(anglez), -sin(anglez), 0, sin(anglez), cos(anglez), 0, 0, 0, 1};
  CMatrixDouble rotz(3, 3, tmpz);

  // roty
  double angley = 0.0_deg;
  // double angley = 90.0_deg;
  const double tmpy[] = {cos(angley), 0, sin(angley), 0, 1, 0, -sin(angley), 0, cos(angley)};
  CMatrixDouble roty(3, 3, tmpy);

  // rotx
  // double anglex = -90.0_deg;
  double anglex = 0.0_deg;
  const double tmpx[] = {1, 0, 0, 0, cos(anglex), -sin(anglex), 0, sin(anglex), cos(anglex)};
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::dumpVisibilityErrorMsg(std::string viz_flag, int sleep_time)
{
  MRPT_START
    ...
    const mrpt::obs::CObservation::Ptr observation)
{
  ...
  ASSERTDEBMSG_(action || observation, "Neither action or observation contains valid data.");
  ...
  else
  {
    ...
    {
      for (auto sens_it = observations->begin(); sens_it != observations->end(); ++sens_it)
      {
...
void CGraphSlamEngine<GRAPH_T>::updateMapVisualization(
    const std::map<mrpt::graphs::TNodeID, mrpt::obs::CObservation2DRangeScan::Ptr>&
    ...
    const int keep_every_n_entries /*= 2*/)
{
  ...
  for (size_t i = 0; i != scan_size; i++)
  {
    if (i % keep_every_n_entries == 0)
    {
      ...
      new_validRange[new_scan_size] = laser_scan_in.getScanRangeValidity(i);
      new_scan_size++;
    }
  }
...
void CGraphSlamEngine<GRAPH_T>::initGTVisualization()
{
  ...
  // robot model
  CSetOfObjects::Ptr robot_model =
      this->setCurrentPositionModel("robot_GT", m_GT_color, m_robot_model_size);
  ...
  m_win_manager->addTextMessage(
      m_offset_x_left, -m_offset_y_GT, "Ground truth path", mrpt::img::TColorf(m_GT_color),
      m_text_index_GT);
  m_win->forceRepaint();
...
void CGraphSlamEngine<GRAPH_T>::initOdometryVisualization()
{
  ...
  // robot model
  CSetOfObjects::Ptr robot_model =
      this->setCurrentPositionModel("robot_odometry_poses", m_odometry_color, m_robot_model_size);
  ...
  m_win_manager->assignTextMessageParameters(&m_offset_y_odometry, &m_text_index_odometry);
  m_win_manager->addTextMessage(
      m_offset_x_left, -m_offset_y_odometry, "Odometry path", mrpt::img::TColorf(m_odometry_color),
...
void CGraphSlamEngine<GRAPH_T>::initEstimatedTrajectoryVisualization()
{
  ...
  if (m_visualize_estimated_trajectory)
  {
    ...
    m_win_manager->addTextMessage(
        ...
        mrpt::img::TColorf(m_estimated_traj_color), m_text_index_estimated_traj);
  }
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::updateEstimatedTrajectoryVisualization(bool full_update)
{
  MRPT_START
  ASSERTDEB_(m_enable_visuals);
  using namespace mrpt::opengl;
...
template <class GRAPH_T>
CGraphSlamEngine<GRAPH_T>::TRGBDInfoFileParams::TRGBDInfoFileParams(const std::string& rawlog_fname)
{
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::TRGBDInfoFileParams::setRawlogFile(const std::string& rawlog_fname)
{
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::computeSlamMetric(mrpt::graphs::TNodeID nodeID, size_t gt_index)
{
  MRPT_START
  using namespace mrpt::math;
  using namespace mrpt::poses;
  ...
  for (auto index_it = start_it; index_it != m_nodeID_to_gt_indices.end(); index_it++)
  {
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::generateReportFiles(const std::string& output_dir_fname)
{
  MRPT_START
  using namespace mrpt::system;
  using namespace mrpt;
  ...
  {  // slam evaluation metric
    ...
    m_out_streams[fname].printf("%s\n", desc.c_str());
    for (auto vec_it = m_deformation_energy_vec.begin(); vec_it != m_deformation_energy_vec.end();
         ++vec_it)
    {
...
template <class GRAPH_T>
void CGraphSlamEngine<GRAPH_T>::getDeformationEnergyVector(std::vector<double>* vec_out) const
{
