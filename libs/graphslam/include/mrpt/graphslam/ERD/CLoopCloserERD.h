    ...
    public virtual mrpt::graphslam::deciders::CRangeScanEdgeRegistrationDecider<GRAPH_T>
{
 public:
   ...
   */
  size_t getDijkstraExecutionThresh() const { return m_dijkstra_node_count_thresh; }
  void setDijkstraExecutionThresh(size_t new_thresh) { m_dijkstra_node_count_thresh = new_thresh; }
 ...
 protected:
  ...
  void dumpVisibilityErrorMsg(std::string viz_flag, int sleep_time = 500 /* ms */);
  ...
  void getMinUncertaintyPath(
      const mrpt::graphs::TNodeID from, const mrpt::graphs::TNodeID to, path_t* path) const;
