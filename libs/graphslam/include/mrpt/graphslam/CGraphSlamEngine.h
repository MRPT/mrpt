...
class CGraphSlamEngine : public mrpt::system::COutputLogger
{
 public:
  ...
  void getMap(
      mrpt::maps::COctoMap::Ptr map, mrpt::system::TTimeStamp* acquisition_time = nullptr) const;
  ...
  void resumeExec() const
  {
    ...
    if (m_enable_visuals)
    {
      this->m_win->addTextMessage(0.3, 0.8, "", m_text_index_paused_message);
    }
  }
  ...
  void pauseExec()
  {
    ...
    MRPT_LOG_WARN_STREAM(
        ...
        << system::upperCase(m_keystroke_pause_exec) << "\" in the dipslay window to resume");
    m_is_paused = true;
    if (m_enable_visuals)
    {
      this->m_win->addTextMessage(0.3, 0.8, m_paused_message, m_text_index_paused_message);
    }
