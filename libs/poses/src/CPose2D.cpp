...
CPose2D CPose2D::operator+(const CPose2D& D) const
{
  ...
  return CPose2D(
      ...
      m_coords[1] + D.m_coords[0] * m_sinphi + D.m_coords[1] * m_cosphi, m_phi + D.m_phi);
}
...
The operator a="this"+D is the pose compounding operator.
 ---------------------------------------------------------------*/
CPose3D CPose2D::operator+(const CPose3D& D) const { return CPose3D(*this) + D; }
...
void CPose2D::inverseComposePoint(const mrpt::math::TPoint2D& g, mrpt::math::TPoint2D& l) const
{
...
void CPose2D::inverseComposeFrom(const CPose2D& A, const CPose2D& B)
{
  ...
  m_coords[0] =
      (A.m_coords[0] - B.m_coords[0]) * B.m_cosphi + (A.m_coords[1] - B.m_coords[1]) * B.m_sinphi;
  m_coords[1] =
      -(A.m_coords[0] - B.m_coords[0]) * B.m_sinphi + (A.m_coords[1] - B.m_coords[1]) * B.m_cosphi;
  m_phi = math::wrapToPi(A.m_phi - B.m_phi);
  m_cossin_uptodate = false;
}
...
TPoint2D mrpt::poses::operator+(const CPose2D& pose, const TPoint2D& u)
{
  ...
  return TPoint2D(pose.x() + u.x * ccos - u.y * ssin, pose.y() + u.x * ssin + u.y * ccos);
}
...
void CPose2D::fromStringRaw(const std::string& s) { this->fromString("[" + s + "]"); }
...
CPose2D CPose2D::getOppositeScalar() const { return CPose2D(-m_coords[0], -m_coords[1], -m_phi); }
...
mrpt::math::TPose2D CPose2D::asTPose() const { return mrpt::math::TPose2D(x(), y(), phi()); }
