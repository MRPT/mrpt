...
{
  c3d.setZero();
  c3d.asEigen().template block<2, 2>(0, 0) = c2d.asEigen().template block<2, 2>(0, 0);
  c3d(3, 3) = c2d(2, 2);
...
void CPose3DPDF::jacobiansPoseComposition(
    const CPose3D& x, const CPose3D& u, CMatrixDouble66& df_dx, CMatrixDouble66& df_du)
{
  ...
  //  JACOB_dx = J_Q2E (6x7) * quat_df_dx (7x7) * J_E2Q_dx (7x6)
  //  JACOB_du = J_Q2E (6x7) * quat_df_du (7x7) * J_E2Q_du (7x6)
  df_dx.asEigen() = J_Q2E.asEigen() * quat_df_dx.asEigen() * J_E2Q_dx.asEigen();
  df_du.asEigen() = J_Q2E.asEigen() * quat_df_du.asEigen() * J_E2Q_du.asEigen();
}
