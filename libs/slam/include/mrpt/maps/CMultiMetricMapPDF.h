...
class CRBPFParticleData : public mrpt::serialization::CSerializable
{
 ...
 public:
  ...
  CRBPFParticleData(const TSetOfMetricMapInitializers& mapsInit) : mapTillNow(mapsInit), robotPath()
  {
  }
        ...
        mrpt::bayes::particle_storage_mode::POINTER>
{
 ...
 public:
  ...
  // see docs in base
  mrpt::math::TPose3D getLastPose(size_t i, bool& pose_is_valid) const override;

  void PF_SLAM_implementation_custom_update_particle_with_new_pose(
      CParticleDataContent* particleData, const mrpt::math::TPose3D& newPose) const override;
  ...
  bool PF_SLAM_implementation_doWeHaveValidObservations(
      const CParticleList& particles, const mrpt::obs::CSensoryFrame* sf) const override;
