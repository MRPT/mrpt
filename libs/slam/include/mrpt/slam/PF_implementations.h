...
bool PF_implementation<PARTICLE_TYPE, MYSELF, STORAGE>::
    PF_SLAM_implementation_gatherActionsCheckBothActObs(
        const mrpt::obs::CActionCollection* actions, const mrpt::obs::CSensoryFrame* sf)
{
...
template <class PARTICLE_TYPE, class MYSELF, mrpt::bayes::particle_storage_mode STORAGE>
template <class BINTYPE>
void PF_implementation<PARTICLE_TYPE, MYSELF, STORAGE>::PF_SLAM_implementation_pfAuxiliaryPFOptimal(
...
template <class PARTICLE_TYPE, class MYSELF, mrpt::bayes::particle_storage_mode STORAGE>
template <class BINTYPE>
void PF_implementation<PARTICLE_TYPE, MYSELF, STORAGE>::PF_SLAM_implementation_pfStandardProposal(
    const mrpt::obs::CActionCollection* actions,
    const mrpt::obs::CSensoryFrame* sf,
    const mrpt::bayes::CParticleFilter::TParticleFilterOptions& PF_options,
    const TKLDParams& KLD_options)
    ...
    if (!PF_options.adaptiveSampleSize)
    {
      ...
      for (size_t i = 0; i < M; i++)
      {
        ...
        const mrpt::poses::CPose3D finalPose =
            mrpt::poses::CPose3D(getLastPose(i, pose_is_valid)) + incrPose;
    ...
    else
    {
      ...
      do  // THE MAIN DRAW SAMPLING LOOP
      {
        ...
        const mrpt::poses::CPose3D newPose =
            mrpt::poses::CPose3D(getLastPose(drawn_idx, pose_is_valid)) + increment_i;
        ...
        BINTYPE p;
        KLF_loadBinFromParticle<PARTICLE_TYPE, BINTYPE>(p, KLD_options, part, &newPose_s);
      ...
      this->PF_SLAM_implementation_replaceByNewParticleSet(
          me->m_particles, newParticles, newParticlesWeight, newParticlesDerivedFromIdx);
...
template <class PARTICLE_TYPE, class MYSELF, mrpt::bayes::particle_storage_mode STORAGE>
template <class BINTYPE>
double PF_implementation<PARTICLE_TYPE, MYSELF, STORAGE>::PF_SLAM_particlesEvaluator_AuxPFOptimal(
    const mrpt::bayes::CParticleFilter::TParticleFilterOptions& PF_options,
    ...
    const void* observation)
{
  ...
  for (size_t q = 0; q < N; q++)
  {
    ...
    indivLik = me->PF_SLAM_computeObservationLikelihoodForParticle(
        PF_options, index, *static_cast<const mrpt::obs::CSensoryFrame*>(observation), x_predict);
...
template <class PARTICLE_TYPE, class MYSELF, mrpt::bayes::particle_storage_mode STORAGE>
template <class BINTYPE>
double PF_implementation<PARTICLE_TYPE, MYSELF, STORAGE>::PF_SLAM_particlesEvaluator_AuxPFStandard(
    const mrpt::bayes::CParticleFilter::TParticleFilterOptions& PF_options,
    const mrpt::bayes::CParticleFilterCapable* obj,
    size_t index,
    const void* action,
    const void* observation)
{
  ...
  if (!PF_options.pfAuxFilterStandard_FirstStageWeightsMonteCarlo)
  {
    ...
    myObj->m_pfAuxiliaryPFStandard_estimatedProb[index] =
        myObj->PF_SLAM_computeObservationLikelihoodForParticle(
            PF_options, index, *static_cast<const mrpt::obs::CSensoryFrame*>(observation),
            x_predict);
  ...
  else
  {
    ...
    if (PF_options.pfAuxFilterOptimal_MLE)
      myObj->m_pfAuxiliaryPFOptimal_maxLikDrawnMovement[index] = maxLikDraw.asTPose();
        ...
        const bool USE_OPTIMAL_SAMPLING)
{
  ...
  m_pfAuxiliaryPFOptimal_maxLikelihood.assign(M, mrpt::bayes::INVALID_LIKELIHOOD_VALUE);
  ...
  me->prepareFastDrawSample(
      PF_options, USE_OPTIMAL_SAMPLING ? funcOpt : funcStd, &meanRobotMovement, sf);

  // For USE_OPTIMAL_SAMPLING=1,  m_pfAuxiliaryPFOptimal_maxLikelihood is now
  // computed.

  if (USE_OPTIMAL_SAMPLING && me->isLoggingLevelVisible(mrpt::system::LVL_DEBUG))
  {
    me->logStr(
  ...
  if (!PF_options.adaptiveSampleSize)
  {
    ...
    for (size_t i = 0; i < M; i++)
    {
      ...
      PF_SLAM_aux_perform_one_rejection_sampling_step<BINTYPE>(
          USE_OPTIMAL_SAMPLING, doResample, maxMeanLik, k, sf, PF_options, newPose,
          newParticleLogWeight);
  ...
  else
  {
    ...
    for (partIt = me->m_particles.begin(), partIndex = 0; partIt != me->m_particles.end();
         ++partIt, ++partIndex)
    {
      ...
      BINTYPE p;
      KLF_loadBinFromParticle<PARTICLE_TYPE, BINTYPE>(p, KLD_options, part);
    ...
    mrpt::random::shuffle(permutationPathsAuxVector.begin(), permutationPathsAuxVector.end());
    ...
    do  // "N" is the index of the current "new particle":
    {
      ...
      else
      {
        ...
        if (permutationPathsAuxVector.size())
        {
          ...
          const size_t idx = mrpt::random::getRandomGenerator().drawUniform32bit() %
        ...
        else
        {
          ...
          if (oldPartIdxsStillNotPropragated.size())
          {
            ...
            auto it = oldPartIdxsStillNotPropragated.begin() + idx;  // advance(it,idx);
            k = *it;
          ...
          else
          {
            ...
            k = mrpt::random::getRandomGenerator().drawUniform32bit() % me->m_particles.size();
          }
        }
      }
    ...
    } while ((N < KLD_options.KLD_maxSampleSize && N < std::max(Nx, minNumSamples_KLD)) ||
        ...
        double& out_newParticleLogWeight)
{
  ...
  bool pose_is_valid;
  const mrpt::poses::CPose3D oldPose =
      mrpt::poses::CPose3D(getLastPose(k, pose_is_valid));  // current pose of the k'th particle
  ...
  else
  {
    ...
    {  // Optimal APF with rejection sampling:
      ...
      do
      {
        ...
        out_newPose.composeFrom(oldPose,
                                movementDraw);  // out_newPose = oldPose + movementDraw;

        // Compute acceptance probability:
        poseLogLik =
            PF_SLAM_computeObservationLikelihoodForParticle(PF_options, k, *sf, out_newPose);
    ...
    if (USE_OPTIMAL_SAMPLING)
    {                                                // Optimal PF
      if (doResample) out_newParticleLogWeight = 0;  // By definition of our optimal
    ...
    {  // APF:
      const double weightFact =
          (poseLogLik - m_pfAuxiliaryPFStandard_estimatedProb[k]) * PF_options.powFactor;
      if (doResample)
