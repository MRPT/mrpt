    ...
    mrpt::optional_ref<TMetricMapAlignmentResult> info)
{
  ...
  if (info)
    if (auto* o = dynamic_cast<TReturnInfo*>(&info.value().get()); o) *o = infoVal;
    ...
    TReturnInfo& outInfo)
{
  ...
  ASSERTMSG_(
      m1->getResolution() == m2->getResolution(), mrpt::format(
  ...
  // ------------------------------------------------------
  const size_t N1 = std::max(40, mrpt::round(m1->getArea() * options.featsPerSquareMeter));
  const size_t N2 = std::max(40, mrpt::round(m2->getArea() * options.featsPerSquareMeter));

  m_grid_feat_extr.extractFeatures(
      *m1, *lm1, N1, options.feature_descriptor, options.feature_detector_options);
  m_grid_feat_extr.extractFeatures(
      *m2, *lm2, N2, options.feature_descriptor, options.feature_detector_options);
  ...
  else
  {
    ...
    for (size_t idx1 = 0; idx1 < nLM1; idx1++)
    {
      ...
      {
        const float dist = lm1->landmarks.get(idx1)->features[0].descriptorDistanceTo(
    ...
    if (options.save_feat_coors)
    {
      ...
      for (size_t l = 0; l < idxs1.size(); l++) corrs_by_idx[idxs1[l]].insert(idxs2[l]);
    ...
    else
    {
      ...
      MRPT_LOG_INFO(mrpt::format(
          ...
          "%u): (%.03f,%.03f,%.03fdeg)\n",
          corrsCount, (unsigned)correspondences.size(), outInfo.noRobustEstimation.x(),
          outInfo.noRobustEstimation.y(), RAD2DEG(outInfo.noRobustEstimation.phi())));

      // The list of SOG modes & their corresponding sub-sets of
      // matchings:
      using TMapMatchingsToPoseMode =
          std::map<mrpt::tfest::TMatchingPairList, CPosePDFSOG::TGaussianMode>;
      ...
      if (options.methodSelection == CGridMapAligner::amRobustMatch)
      {
        ...
        mrpt::tfest::se2_l2_robust(
            correspondences, options.ransac_SOG_sigma_m, tfest_params, tfest_result);
        ...
        *pdf_SOG = tfest_result.transformation;
        ...
        MRPT_LOG_INFO_STREAM(
            "[CGridMapAligner] amRobustMatch: " << nB << " SOG modes reduced to " << pdf_SOG->size()
      ...
      else
      {
        ...
        COV_pnt(0, 0) = COV_pnt(1, 1) = square(options.ransac_SOG_sigma_m);
               ...
               trials < max_trials)  // ransac_nSimulations can be dynamic
        {
          ...
          const double corrs_dist1 = mrpt::math::distanceBetweenPoints(
              all_corrs[idx1].global.x, all_corrs[idx1].global.y, all_corrs[idx1].global.z,
              all_corrs[idx2].global.x, all_corrs[idx2].global.y, all_corrs[idx2].global.z);

          const double corrs_dist2 = mrpt::math::distanceBetweenPoints(
              all_corrs[idx1].local.x, all_corrs[idx1].local.y, all_corrs[idx1].local.z,
              all_corrs[idx2].local.x, all_corrs[idx2].local.y, all_corrs[idx2].local.z);

          // Is is a consistent possibility?
          //  We use a chi2 test (see paper for the derivation)
          const double corrs_dist_chi2 = square(square(corrs_dist1) - square(corrs_dist2)) /
          ...
          for (auto& sog_mode : sog_modes)
          {
            ...
            {
              // Increment weight:
              sog_mode.second.log_w = std::log(std::exp(sog_mode.second.log_w) + 1.0);
              is_new_hyp = false;
              break;
            }
          }
          ...
          {
            if (!mrpt::tfest::se2_l2(tentativeSubSet, temptPose)) continue;  // Invalid matching...
            ...
            CMatrixFixed<double, 2, 3> Hq;  // Jacobian wrt transformation q
            Hq(0, 0) = 1;
            Hq(1, 1) = 1;
...
#define GRIDMAP_USE_PROD_INTEGRAL
            ...
            for (size_t j = 0; j < nLM2; j++)
            {
              ...
              pdf_M2_j.mean = mrpt::poses::CPoint2D(
                  temptPose.mean + p2_j_local);  // In (temptative) global coords:
              pdf_M2_j.cov(0, 0) = pdf_M2_j.cov(1, 1) = square(options.ransac_SOG_sigma_m);
              ...
              lm1_pnts.kdTreeNClosestPoint2DIdx(
                  pdf_M2_j.mean.x(), pdf_M2_j.mean.y(), N_KDTREE_SEARCHED, matches_idx,
                  matches_dist);
              ...
              for (unsigned long u : matches_idx)
              {
                ...
                Hq(0, 2) = -p2_j_local.x * ssin - p2_j_local.y * ccos;
                Hq(1, 2) = p2_j_local.x * ccos - p2_j_local.y * ssin;

                // COV_j = Hq \Sigma_q Hq^t + Hc Cj Hc^t
                pdf_M1_i.cov = mrpt::math::multiply_HCHt(Hc, COV_pnt);
                pdf_M1_i.cov += mrpt::math::multiply_HCHt(Hq, temptPose.cov);
...
// And now compute the product integral:
          ...
          if (largestConsensusCorrs.size() < ninliers)
          {
            ...
            double pNoOutliers = 1 - pow(fracinliers,
        ...
        {
          MRPT_LOG_INFO_STREAM("SOG mode: " << s->second.mean << " inliers: " << s->first.size());
      ...
      // --------------------------------------------------------------------
      if (multimap1 && multimap2 && multimap1->countMapsByClass<CSimplePointsMap>() != 0 &&
          multimap2->countMapsByClass<CSimplePointsMap>() != 0)
      {
        ...
        {
          CPosePDF::Ptr icp_est = icp.Align(pnts1.get(), pnts2.get(), (i)->mean, icpInfo);
          ...
          MRPT_LOG_INFO_STREAM(
              "ICP " << cnt << " log-w: " << i->log_w << " Goodness: " << 100 * icpInfo.goodness
                     << "  D_M= " << icp_maha_dist);
          MRPT_LOG_INFO_STREAM("final pos: " << icp_est->getMeanVal());
        ...
        MRPT_LOG_DEBUG_STREAM(
            "[CGridMapAligner] " << pdf_SOG->size() << " SOG modes merged after ICP.");
    ...
    TReturnInfo& outInfo)
{
  ...
  for (phi = phiMin; phi < phiMax; phi += phiResolution)
  {
    ...
    // --------------------------------------------------------
    CPose2D v2(
        pivotPt_x - cos(phi) * map2width_2, pivotPt_y - sin(phi) * map2width_2,
...
{
  methodSelection = iniFile.read_enum(section, "methodSelection", methodSelection);
  ...
  feature_descriptor = iniFile.read_enum(section, "feature_descriptor", feature_descriptor, true);
