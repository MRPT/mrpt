...
void bind_mrpt_core_Clock(std::function< pybind11::module &(std::string const &namespace_) > &M)
{
	{ // mrpt::Clock file:mrpt/core/Clock.h line:21
		...
		cl.def_static("now", (struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > > (*)()) &mrpt::Clock::now, "Returns the current time using the currently selected Clock source.\n  (Performance: call typ. takes 33 nanoseconds)\n \n\n setActiveClock(), mrpt::Clock::Source, nowDouble()  \n\nC++: mrpt::Clock::now() --> struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > >");
		...
		cl.def_static("fromDouble", (struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > > (*)(const double)) &mrpt::Clock::fromDouble, "Create a timestamp from its double representation. \n toDouble \n\nC++: mrpt::Clock::fromDouble(const double) --> struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > >", pybind11::arg("t"));
		cl.def_static("toDouble", (double (*)(const struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > >)) &mrpt::Clock::toDouble, "Converts a timestamp to a UNIX time_t-like number, with fractional part.\n  If t is an invalid (default-contructed) time_point, `0.0` will be\n returned.\n \n\n fromDouble \n\nC++: mrpt::Clock::toDouble(const struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > >) --> double", pybind11::arg("t"));
		...
		cl.def_static("resetMonotonicToRealTimeEpoch", (long (*)()) &mrpt::Clock::resetMonotonicToRealTimeEpoch, "Monotonic clock *might* drift over time with respect to Realtime.\n The first time a time is requested to now() with Monotonic clock enabled,\n MRPT internally saves the current values of both, Realtime and Monotonic\n clocks, then use the difference in all subsequent calls to set Monotonic\n timepoints in the same epoch than Realtime.\n\n By explicitly calling this static method, a user is able to force a\n resynchrnization between the two clocks. Potentially useful for systems\n that run for very long periods of time without interruption.\n\n \n The mismatch between the former and the new estimations of the\n epochs differences between the two clocks, in units of nanoseconds.\n\n \n This method is ignored in non-Linux systems.\n\nC++: mrpt::Clock::resetMonotonicToRealTimeEpoch() --> long");
		cl.def_static("getMonotonicToRealtimeOffset", (unsigned long (*)()) &mrpt::Clock::getMonotonicToRealtimeOffset, "Returns the number of nanoseconds that are added to the output of the\n POSIX `CLOCK_MONOTONIC` to make timestamps match the epoch of POSIX\n `CLOCK_REALTIME`. \n\nC++: mrpt::Clock::getMonotonicToRealtimeOffset() --> unsigned long");
		cl.def_static("setSimulatedTime", (void (*)(const struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > > &)) &mrpt::Clock::setSimulatedTime, "When setActiveClock() is set to `Simulated`, sets the simulated time\n that will be returned in subsequent calls to now().\n [New in MRPT 2.1.1]\n\nC++: mrpt::Clock::setSimulatedTime(const struct std::chrono::time_point<class mrpt::Clock, struct std::chrono::duration<long, struct std::ratio<1, 10000000> > > &) --> void", pybind11::arg("t"));
	}
}
