...
for it in np.arange(0,n_iter):
    ...
    img_pts_[:,0:2]=img_pts + 0.05*np.random.randn(n,2)
...
err_algos=np.array(err_t_ppnp + err_t_posit + err_t_lhm)
#err_algos=err_algos.reshape(n_algos,n_iter)
err_algos=err_algos.reshape(3,n_iter)
...
display_comparison_plot(it, err_algos, names=['ppnp','posit','lhm'], title='Translation Error Plot', xtitle='Iteration', ytitle='e')
plt.legend()
plt.show()
