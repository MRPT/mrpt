...
[[nodiscard]] static mrpt::opengl::Scene::Ptr createSampleScene() {
  ...
  // Ground grid
  {
    auto grid = mrpt::opengl::CGridPlaneXY::Create(-20.0f, 20.0f, -20.0f, 20.0f,
                                                   0.0f, 1.0f);
  ...
  // Axes
  {
    auto axes = mrpt::opengl::CAxis::Create(-5.0f, -5.0f, -5.0f, 5.0f, 5.0f,
                                            5.0f, 0.5f, 2.0f);
  ...
  // A box
  {
    auto box = mrpt::opengl::CBox::Create(mrpt::math::TPoint3D(-1, -1, 0),
                                          mrpt::math::TPoint3D(1, 1, 2));
...
int main() {
  ...
  GLFWwindow *window =
      glfwCreateWindow(1280, 720, "MRPT + Dear ImGui Demo", nullptr, nullptr);
  if (window == nullptr) {
  ...
  while (glfwWindowShouldClose(window) == GLFW_FALSE) {
    ...
    if (auto vp = scene->getViewport("main"); vp) {
      ...
      if (auto sp = vp->getByClass<mrpt::opengl::CSphere>(); sp) {
        sp->setLocation(-3.0 + 2.0 * std::cos(static_cast<double>(sphereAngle)),
    ...
    if (ImGui::Begin("Controls")) {
      ...
      ImGui::Separator();
      ImGui::Text("Camera pointing at: (%.1f, %.1f, %.1f)",
                  static_cast<double>(cam.getPointingAtX()),
                  static_cast<double>(cam.getPointingAtY()),
                  static_cast<double>(cam.getPointingAtZ()));
