		   ...
		   if ( o ) //ASSERT_(o);
		   {
			   if (IS_CLASS(o,CObservationGasSensors))
			   {
				   ...
				   if (obs->sensorLabel == sensorLabel)
				   {
						...
						mrpt::poses::CPose3D MOXmodel_pose = mrpt::poses::CPose3D(obs->m_readings[enoseID].eNosePoseOnTheRobot);
						...
						if (have_estimation)
						{
							...
							mrpt::obs::CObservationGasSensors::Ptr obs_GDM = std::make_shared<CObservationGasSensors>();
