...
void AppData::obs_params_to_gui()
{
  ...
  for (int i = 0; i < 8; i++) edBoxes[4 + i]->setValue(mrpt::format("%.04e", p.dist[i]));

  // slider ranges:
  const double maxPixelRanges = std::max(std::max(p.cx(), p.cy()), std::max(p.fx(), p.fy()));
  const double minPixelRanges = std::min(std::min(p.cx(), p.cy()), std::min(p.fx(), p.fy()));
  for (int i = 0; i < 4; i++)
  {
  ...
  for (int i = 4; i < 12; i++)
  {
    const double absVal = std::max(1e-3, std::abs(std::stod(edBoxes[i]->value())));
...
void AppData::gui_params_to_obs()
{
  ...
  for (int i = 0; i < 8; i++) p.dist.at(i) = std::stod(edBoxes[4 + i]->value());
...
static void AppDepthCamDemo()
{
  ...
  w->setLayout(
      new nanogui::GridLayout(nanogui::Orientation::Horizontal, 1, nanogui::Alignment::Fill, 5, 0));
  w->setFixedWidth(350);
  ...
  w->add<nanogui::Label>("Extrinsics")->setFontSize(LARGE_FONT_SIZE);
  {
    ...
    std::vector<std::string> lb = {"x=", "y=", "z=", "yaw=", "pitch=", "roll="};
