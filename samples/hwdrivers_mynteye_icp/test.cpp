...
void thread_grabbing(TThreadParam& p)
{
  try
  {
    ...
    while (!p.quit)
    {
      ...
      auto obs = mrpt::ptr_cast<mrpt::obs::CObservation3DRangeScan>::from(obss.begin()->second);
...
void Test_3DCamICP()
{
    ...
    while (!thrPar.quit)
    {
      ...
      if (icp_out && icp_res.nIterations > 0)
      {
        ...
        ui_data.strStatuses[0] = mrpt::format(
            "ICP: %d iters, goodness: %.02f%%", int(icp_res.nIterations),
            icp_res.goodness * 100.0f),

        ui_data.strStatuses[1] = std::string("rel.pose:") + relativePose.asString();
        ui_data.strStatuses[2] = string(icp_res.goodness < 0.3 ? "LOST! Press restart" : "");
        ...
        if (icp_res.goodness > 0.65)
        {
               ...
               std::abs(relativePose.roll()) > KEYFRAMES_MIN_ANG))
          {
            ...
            // position:
            globalPtsMap.insertAnotherMap(cur_points.get(), new_keyframe_global);
  ...
  // Add UI controls:
  std::array<nanogui::TextBox*, 4> lbStatuses = {nullptr, nullptr, nullptr, nullptr};
...

  {
    ...
    subWin->add<nanogui::Label>("Visualization", "sans-bold");
    {
      ...
      cb->setCallback([&ui_data](bool checked) { ui_data.SHOW_FEAT_IDS = checked; });
      cb->setChecked(true);
    }

    {
      auto cb = subWin->add<nanogui::CheckBox>("Show keypoint responses");
      cb->setCallback([&ui_data](bool checked) { ui_data.SHOW_RESPONSES = checked; });
      cb->setChecked(true);
    }
    ...
    {
      auto cmb = subWin->add<nanogui::ComboBox>(
          std::vector<std::string>({"Hidden", "200px", "400px", "800px", "1000px"}));
    ...
    {
      nanogui::TextBox* slVal = subWin->add<nanogui::TextBox>("Point cloud decimation: 8");
      nanogui::Slider* sl = subWin->add<nanogui::Slider>();

      sl->setRange({2, 4});
      sl->setValue(3);
      sl->setCallback(
          [&ui_data, slVal](float v)
          {
            const unsigned int decim = mrpt::round(std::pow(2.0, mrpt::round(v)));
            ui_data.icpDecimation = decim;
            auto s =
                std::string("Point cloud decimation: ") + std::to_string(ui_data.icpDecimation);
            slVal->setValue(s);
          });
    }
    ...
    {
      auto btn = subWin->add<nanogui::Button>("Reset", ENTYPO_ICON_BACK_IN_TIME);
      btn->setCallback([&]() { ui_data.hasToReset = true; });
    }

    {
      auto btn = subWin->add<nanogui::Button>("Quit", ENTYPO_ICON_ARROW_LEFT);
