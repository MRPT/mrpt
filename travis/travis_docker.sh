...
if [ "$STAGE" == "1" ]; then
  ...
  export STAGE_OUTPUT_IMAGE=mrpt/mrpt-travis-archive:$DEPS-$CC-$TRAVIS_COMMIT_1
fi

if [ "$STAGE" == "2" ]; then
  export STAGE_INPUT_IMAGE=mrpt/mrpt-travis-archive:$DEPS-$CC-$TRAVIS_COMMIT_1
  export STAGE_OUTPUT_IMAGE=mrpt/mrpt-travis-archive:$DEPS-$CC-$TRAVIS_COMMIT_2
fi

if [ "$STAGE" == "3" ]; then
  export STAGE_INPUT_IMAGE=mrpt/mrpt-travis-archive:$DEPS-$CC-$TRAVIS_COMMIT_2
fi
