...
#include <mrpt/img/config.h>  // Control MRPT_HAS_... defines
#include <stdint.h>
...
#define MEXPLUS_CHECK_NOTNULL(pointer)                                                         \
  if (!(pointer))                                                                              \
  mexErrMsgIdAndTxt(                                                                           \
      "mexplus:error", "Null pointer exception: %s:%d:%s `" #pointer "`.", __FILE__, __LINE__, \
      __FUNCTION__)
...
mxArray* fromInternal(
    const typename std::enable_if<
        std::is_same<typename MxTypes<typename Container::value_type>::array_type, mxNumeric>::
                value &&
...
mxArray* fromInternal(
    const typename std::enable_if<
        std::is_same<typename MxTypes<typename Container::value_type>::array_type, mxChar>::value &&
            std::is_compound<Container>::value,
        Container>::type& value);
template <typename T>
mxArray* fromInternal(const typename std::enable_if<
...
mxArray* fromInternal(
    const typename std::enable_if<
        std::is_same<typename MxTypes<typename Container::value_type>::array_type, mxLogical>::
                value &&
            std::is_compound<Container>::value,
        Container>::type& value);
template <typename Container>
mxArray* fromInternal(
    const typename std::enable_if<
        std::is_same<typename MxTypes<typename Container::value_type>::array_type, mxCell>::value &&
            std::is_compound<Container>::value,
        Container>::type& value);
 ...
 */
const ConstMap<mxClassID, int> DepthOf = ConstMap<mxClassID, int>(mxDOUBLE_CLASS, CV_64F)(
    mxSINGLE_CLASS, CV_32F)(mxINT8_CLASS, CV_8S)(mxUINT8_CLASS, CV_8U)(mxINT16_CLASS, CV_16S)(
    mxUINT16_CLASS, CV_16U)(mxINT32_CLASS, CV_32S)(mxUINT32_CLASS, CV_32S)(mxLOGICAL_CLASS, CV_8U);
...
const ConstMap<int, mxClassID> ClassIDOf =
    ConstMap<int, mxClassID>(CV_64F, mxDOUBLE_CLASS)(CV_32F, mxSINGLE_CLASS)(CV_8S, mxINT8_CLASS)(
        CV_8U, mxUINT8_CLASS)(CV_16S, mxINT16_CLASS)(CV_16U, mxUINT16_CLASS)(CV_32S, mxINT32_CLASS);
...
inline mxArray* from(const cv::Mat& mat)
{
  ...
  for (int i = 0; i < nchannels; ++i)
  {
    ...
    void* ptr = reinterpret_cast<void*>(
        reinterpret_cast<size_t>(mxGetData(p_)) + mxGetElementSize(p_) * subs_si);
        ...
        Container>::type& value)
{
  ...
  mxArray* array = mxCreateNumericMatrix(
      1, value.size(), MxTypes<ValueType>::class_id, MxTypes<ValueType>::complexity);
  MEXPLUS_CHECK_NOTNULL(array);
...
class MxArray
{
 public:
  ...
  /** Move constructor.
   */
  MxArray(MxArray&& array) : array_(nullptr), owner_(false) { *this = std::move(array); }
  /** Move assignment.
   */
  ...
  static mxArray* Struct(
      int nfields = 0, const char** fields = nullptr, int rows = 1, int columns = 1)
  {
  ...
  static void set(mxArray* array, mwIndex index, mxArray* value)
  {
    ...
    MEXPLUS_ASSERT(mxIsCell(array), "Expected a cell array.");
    MEXPLUS_ASSERT(index < mxGetNumberOfElements(array), "Index out of range: %u.", index);
  ...
  static void set(mxArray* array, const std::string& field, mxArray* value, mwIndex index = 0)
  {
    ...
    if (!mxGetField(array, index, field.c_str()))
      MEXPLUS_ASSERT(
          mxAddField(array, field.c_str()) >= 0, "Failed to create a field '%s'", field.c_str());
  ...
  {
    return isStruct() && mxGetField(array_, index, field_name.c_str()) != nullptr;
  }
 ...
 private:
  ...
  static void toInternal(
      const mxArray* array, typename std::enable_if<std::is_arithmetic<T>::value, T>::type* value)
  {
  ...
  static void toInternal(
      ...
      typename std::enable_if<
          std::is_compound<T>::value && std::is_arithmetic<typename T::value_type>::value,
          T>::type* value);
  template <typename T>
  static void toInternal(
      const mxArray* array,
...
mxArray* MxArray::Numeric(int rows, int columns)
{
  typedef typename std::enable_if<
      std::is_same<typename MxTypes<T>::array_type, mxNumeric>::value, T>::type Scalar;
  mxArray* numeric =
