...
void CDifodoDatasets::initializeScene()
{
  ...
  // DifOdo camera
  CBox::Ptr camera_odo =
      std::make_shared<CBox>(math::TPoint3D(-0.02, -0.1, -0.01), math::TPoint3D(0.02, 0.1, 0.01));
...
void CDifodoDatasets::updateScene()
{
  ...
  for (unsigned int y = 0; y < cols; y++)
    for (unsigned int z = 0; z < rows; z++)
      cam_points->push_back(
          depth[repr_level](z, y), xx[repr_level](z, y), yy[repr_level](z, y),
  ...
  if ((first_pose == true) && ((gt_pose - gt_oldpose).norm() < 0.5))
  {
    ...
    traj_lines_odo->appendLine(
        cam_oldpose.x(), cam_oldpose.y(), cam_oldpose.z(), cam_pose.x(), cam_pose.y(),
        cam_pose.z());
    ...
    traj_lines_gt->appendLine(
        gt_oldpose.x(), gt_oldpose.y(), gt_oldpose.z(), gt_pose.x(), gt_pose.y(), gt_pose.z());
...
void CDifodoDatasets::loadFrame()
{
  ...
  for (unsigned int j = 0; j < cols; j++)
    ...
    {
      const float z = range(height - downsample * i - 1, width - downsample * j - 1) * rangeUnits;
      if (z < 4.5f)
        depth_wf(i, j) = z;
      else
        depth_wf(i, j) = 0.f;
  ...
  else
  {
    ...
    if (std::abs(qx0 + last_gt_data[3]) + std::abs(qy0 + last_gt_data[4]) +
            std::abs(qz0 + last_gt_data[5]) + std::abs(w0 + last_gt_data[6]) <
        0.05)
    {
      qx0 = -qx0;
      qy0 = -qy0;
