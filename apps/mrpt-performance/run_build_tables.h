...
int run_build_tables()
{
  ...
  for (const auto& P1 : lstConfigurations)
  {
    // class=\"my_rotated\"
    fo.printf(
        "<td align=\"center\" width=\"20em\"><b><div >%s</div></b></td>", P1.config_name.c_str());
  }
  fo.printf(" </tr>\n");
  fo.printf("</table></div>\n");
  ...
  fo.printf(
      ...
      mrpt::system::dateTimeLocalToString(mrpt::Clock::now()).c_str(), MRPT_getVersion().c_str());
  fo.printf("</body></html>\n");
  fo.close();
  ...
  for (const auto& P : lstConfigurations)
  {
    ...
    f.printf("<div align=\"center\"><h2>Results for: %s</h2></div>", P.config_name.c_str());
    f.printf("<hr> <br><br>\n");
  ...
  for (size_t i = 0; i < lstConfigurations.size(); i++)
  {
    ...
    for (size_t j = 0; j < lstConfigurations.size(); j++)
    {
      ...
      f.printf(
          "<div align=\"center\"><h2>Comparison: %s vs. %s</h2></div>", P1.config_name.c_str(),
          P2.config_name.c_str());
      f.printf("<hr> <br><br>\n");
      ...
      for (auto& k : P1.all_perf_data)  // vector<pair<string,double> >
      {
        ...
        f.printf(
            ...
            "<td align=\"center\">%s</td>"
            "</tr>\n",
            test_name.c_str(), str_secs.c_str(), str_secs_res.c_str(), str_Hz.c_str(),
            str_Hz_res.c_str());
      }
