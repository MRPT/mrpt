...
void navlog_viewer_GUI_designDialog::OnslidLogCmdScroll(wxScrollEvent& event)
{
	...
	// --------------------------------
	{
		...
		if (!win1)  {
			...
			win1->setCameraElevationDeg(90);
			{
				...
				// XY ground plane:
				mrpt::opengl::CGridPlaneXY::Ptr gl_grid = std::make_shared<mrpt::opengl::CGridPlaneXY>(-20,20, -20,20, 0, 1, 0.75f);
	...
	for (unsigned int nPTG=0;nPTG<log.infoPerPTG.size();nPTG++)  // log.infoPerPTG.size() may be != nPTGs in the last entry is used for "NOP cmdvel"
	{
		...
		if (!win)  {
			...
			win= std::make_shared<CDisplayWindow3D>(format("%u|TP-Obstacles",nPTG),W,H);
...

			{
				...
				scene->insert(std::make_shared<mrpt::opengl::CGridPlaneXY>(-1.0f, 1.0f, -1.0f, 1.0f, .0f, 1.0f));
				...
				{
					auto gl_obj = std::make_shared<mrpt::opengl::CMesh>(true /*transparency*/);
