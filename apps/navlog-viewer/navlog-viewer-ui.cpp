...
NavlogViewerApp::NavlogViewerApp()
{
...

  {
    ...
    panel->setLayout(new nanogui::BoxLayout(
        nanogui::Orientation::Horizontal, nanogui::Alignment::Fill, 5 /*margin*/));

    panel->add<nanogui::Button>("Load...", ENTYPO_ICON_UPLOAD)
        ->setCallback([this]() { this->OnbtnLoadClick(); });

    m_btnPlay = panel->add<nanogui::Button>("Play", ENTYPO_ICON_CONTROLLER_PLAY);
    m_btnStop = panel->add<nanogui::Button>("Stop", ENTYPO_ICON_CONTROLLER_STOP);

    panel->add<nanogui::Label>(" Animation delay (s):");
    nanogui::TextBox* edPlayInterval = panel->add<nanogui::TextBox>("0.100");
    ...
    m_btnPlay->setCallback(
        [edPlayInterval, this]()
        {
          ...
          if (std::abs(slidLog->value() - slidLog->range().second) < 1e-6) slidLog->setValue(0);
  ...
  // ===== TAB: Information
  {
    ...
    auto layout =
        new nanogui::GridLayout(nanogui::Orientation::Horizontal, 2, nanogui::Alignment::Fill, 5);
  ...
  // ===== TAB: View options
  {
    ...
    m_cbUseOdometryCoords = layer->add<nanogui::CheckBox>("Use raw odometry");
    m_cbUseOdometryCoords->setCallback([this](bool) { updateVisualization(); });

    m_cbGlobalFrame = layer->add<nanogui::CheckBox>("In global coordinates");
    m_cbGlobalFrame->setChecked(true);
    m_cbGlobalFrame->setCallback([this](bool) { updateVisualization(); });

    m_cbShowDelays = layer->add<nanogui::CheckBox>("Show delays model-based poses");
    m_cbShowDelays->setChecked(true);
    m_cbShowDelays->setCallback([this](bool) { updateVisualization(); });

    m_cbDrawShape = layer->add<nanogui::CheckBox>("Draw robot shape on trajectories");
    m_cbDrawShape->setChecked(true);
    m_cbDrawShape->setCallback([this](bool) { updateVisualization(); });

    m_cbShowAllDebugFields = layer->add<nanogui::CheckBox>("Show all debug fields");
    m_cbShowAllDebugFields->setCallback([this](bool) { updateVisualization(); });
    ...
    m_ClearanceOverPath = layer->add<nanogui::CheckBox>("Clearance: path/!pointwise");
    m_ClearanceOverPath->setCallback([this](bool) { updateVisualization(); });
  ...
  // ===== TAB: Advanced
  {
    ...
    auto layout =
        new nanogui::GridLayout(nanogui::Orientation::Horizontal, 2, nanogui::Alignment::Fill, 10);
    layer->setLayout(layout);

    auto lambdaAddSmallFontBtn = [](nanogui::Widget* parent, const std::string& caption,
    ...
    lambdaAddSmallFontBtn(layer, "See PTGs details", [this]() { OnmnuSeePTGParamsSelected(); });
    lambdaAddSmallFontBtn(
        layer, "Save current obstacle map...", [this]() { OnmnuSaveCurrentObstacles(); });
    lambdaAddSmallFontBtn(
        layer, "Export map plot to MATLAB...", [this]() { OnmnuMatlabPlotsSelected(); });
    lambdaAddSmallFontBtn(
        layer, "Export paths info to MATLAB...", [this]() { OnmnuMatlabExportPaths(); });
    lambdaAddSmallFontBtn(
        layer, "Save score matrices...", [this]() { OnmnuSaveScoreMatrixSelected(); });
    lambdaAddSmallFontBtn(
        layer, "Export current log entry...", [this]() { OnmnuExportSelected(); });
    lambdaAddSmallFontBtn(
  ...
  // ===== TAB: Manually pick trajectory
  {
    ...
    m_cbManualPickMode = layer->add<nanogui::CheckBox>("Enable manual pick");
    m_cbManualPickMode->setCallback([this](bool) { updateVisualization(); });

    layer->add<nanogui::Label>(" ");

    layer->add<nanogui::Label>("PTG index:");
    m_slidPtgIndex = layer->add<nanogui::Slider>();
    m_slidPtgIndex->setCallback([this](float /*v*/) { updateVisualization(); });

    layer->add<nanogui::Label>("Trajectory index:");
    m_slidTrajectoryIndex = layer->add<nanogui::Slider>();
    m_slidTrajectoryIndex->setCallback([this](float /*v*/) { updateVisualization(); });
    ...
    m_btnManualPickAppendYaml = layer->add<nanogui::Button>("Append to 'nav-selected.yaml'");
    m_btnManualPickAppendYaml->setCallback([this]() { OnManualPickAppendYaml(); });
  }
  ...
  m_win->addKeyboardCallback([this](int key, int scancode, int action, int modifiers)
...
void NavlogViewerApp::OnbtnLoadClick()
{
  ...
  const std::string fileName = nanogui::file_dialog(
      {
          {"reactivenavlog", "Navigation logs"},
          {        "navlog", "Navigation logs"}
  },
      false /*load*/);
      ...
      if (IS_CLASS(*obj, CLogFileRecord))
      {
        ...
        if (!logptr->infoPerPTG.empty())
        {
          ...
          if (nPTGs > m_logdata_ptg_paths.size())
          {
            ...
            {
              if (logptr->infoPerPTG[i].ptg) m_logdata_ptg_paths[i] = logptr->infoPerPTG[i].ptg;
            }
          }
          else
          {
            for (size_t i = 0; i < nPTGs; i++) logptr->infoPerPTG[i].ptg = m_logdata_ptg_paths[i];
          }
        }
      }
...
void NavlogViewerApp::updateInfoFromLoadedLog()
{
  ...
  {
    sDuration =
        mrpt::system::intervalFormat(mrpt::system::timeDifference(m_log_first_tim, m_log_last_tim));
  }
...
void NavlogViewerApp::updateVisualization()
{
  ...
  txtSelectedPTG->setValue(
      std::to_string(log.nSelectedPTG) + "from [0-"s + std::to_string(log.nPTGs - 1) + "]"s);

  {
    std::string timeIndexStr = "Time index: "s + std::to_string(log_idx) + " / "s +
                               std::to_string(std::max<size_t>(1U, m_logdata.size()) - 1);

    if (auto itTim = log.timestamps.find("tim_start_iteration"); itTim != log.timestamps.end())
    {
      timeIndexStr += " (tim_start_iteration="s +
                      mrpt::system::dateTimeLocalToString(itTim->second) + " [local time])"s;
    }
  ...
  // Update 3D view:
  {
    ...
    mrpt::opengl::CSetOfObjects::Ptr gl_robot_frame;
    {
      ...
      if (m_cbUseOdometryCoords->checked())
      {
        ...
        // frame:
        auto log0ptr = mrpt::ptr_cast<CLogFileRecord>::from(m_logdata[0]);
        ASSERT_(log0ptr.get());
        const auto curPose =
    ...
    // custom visuals:
    {
      mrpt::opengl::CSetOfObjects::Ptr gl_custom_vizs;
      {
        ...
        for (const auto& viz : log.visuals) gl_custom_vizs->insert(viz);
      }
    }
      ...
      if (sel_ptg_idx < int(m_logdata_ptg_paths.size()) && sel_ptg_idx >= 0)
      {
        ...
        if (ptg)
        {
          ...
          mrpt::math::TPose2D ptg_origin =
              (m_cbShowDelays->checked()) ? log.relPoseVelCmd : mrpt::math::TPose2D(0, 0, 0);
          ...
          {
            double min_shape_dists = std::max(0.01, std::stod(edShapeMinDist->value()));
...

  {
    ...
    {
      ADD_WIN_TEXTMSG(
          mrpt::format("rel_target[%u]=%s", i, log.WS_targets_relative[i].asString().c_str()));
    }
  }
...

  {
    ...
    for (size_t i = 0; i < log.infoPerPTG.size(); i++)
      ss << "PTG#" << i
         << mrpt::format(
                " TPObs:%ss HoloNav:%ss |",
                mrpt::system::unitsFormat(log.infoPerPTG[i].timeForTPObsTransformation).c_str(),
                mrpt::system::unitsFormat(log.infoPerPTG[i].timeForHolonomicMethod).c_str());
    ADD_WIN_TEXTMSG(ss.str());
  }
  ...
  for (unsigned int nPTG = 0; nPTG < log.infoPerPTG.size(); nPTG++)
  {
    ...
    ADD_WIN_TEXTMSG_COL(
        mrpt::format(
            ...
            mrpt::RAD2DEG(pI.desiredDirection), pI.desiredSpeed, pI.evaluation, sFactors.c_str()),
        col);
  }
  ...
  for (unsigned int nPTG = 0; nPTG < log.infoPerPTG.size(); nPTG++)
  {
    ...
    }
    {
      ...
      if (nAlphas > 2)
      {
        ...
        for (size_t i = 2; i < nAlphas; i++) gl_obj->appendLineStrip(xs[i], ys[i], 0);
      }
    }
  ...
  for (size_t i = 0; i < N; i++)
  {
    ...
    if (lastOdo && lastLocPose)
    {
      ...
      if (!log.timestamps.empty())
        f << mrpt::format("%.03f ", mrpt::Clock::toDouble(log.timestamps.begin()->second));
      else
        f << "0 ";

      for (int k = 0; k < 3; k++) f << locIncr[k] << " ";
      for (int k = 0; k < 3; k++) f << odoIncr[k] << " ";
      f << "\n";
    }
...
void NavlogViewerApp::OnmnuMatlabExportPaths()
{
  ...
  for (size_t i = 0; i < N; i++)
  {
    ...
    // timoff_obstacles;
    {
      ...
      if (it != logptr->values.end()) vals_timoff_obstacles[tim_start_iteration] = it->second;
    }
    // timoff_curPoseVelAge;
    {
      const auto it = logptr->values.find("timoff_curPoseVelAge");
      if (it != logptr->values.end()) vals_timoff_curPoseVelAge[tim_start_iteration] = it->second;
    }
    // timoff_sendVelCmd
    {
      const auto it = logptr->values.find("timoff_sendVelCmd");
      if (it != logptr->values.end()) vals_timoff_sendVelCmd[tim_start_iteration] = it->second;
    }
  ...
  for (const auto& e : global_local_vel)
  {
    f << (e.first - t_ref) << "," << e.second.pose.x << "," << e.second.pose.y << ","
      << e.second.pose.phi << ", " << e.second.poseOdom.x << "," << e.second.poseOdom.y << ","
  ...
  for (const auto& e : global_local_vel)
  {
    f << (e.first - t_ref) << "," << e.second.velGlobal.vx << "," << e.second.velGlobal.vy << ","
      << e.second.velGlobal.omega << " ; ";
  }
  f << "];\n"
       "figure(); "
  ...
  for (const auto& e : global_local_vel)
  {
    f << (e.first - t_ref) << "," << e.second.velLocal.vx << "," << e.second.velLocal.vy << ","
      << e.second.velLocal.omega << " ; ";
  }
  f << "];\n"
...
void NavlogViewerApp::OnManualPickAppendYaml()
{
  ...
  const std::string navLogSingleFile = mrpt::format("log_%.03f.navlog", m_manualPickTimestamp);
...
void NavlogViewerApp::updateManualPickControls()
{
  ...
  m_manualPickLabel->setCaption(
      mrpt::format("Selected PTG#%i trajectory #%i", manualPickedPTG, manualPickedTraj));

  if (manualPickEnabled)
  {
    if (!log.timestamps.empty())
      m_manualPickTimestamp = mrpt::Clock::toDouble(log.timestamps.begin()->second);
