        ...
        mrpt::bayes::CParticleFilterData<TExtendedCPose2D>::CParticleList>
{
 ...
 public:
  ...
  /** Copy constructor:
   */
  CPosePDFParticlesExtended(const CPosePDFParticlesExtended& obj) { copyFrom(obj); }
   ...
   */
  double evaluatePDF_parzen(float x, float y, float phi, float stdXY, float stdPhi) const;
...
struct lt_TPoseBin
{
  bool operator()(const TPoseBin& s1, const TPoseBin& s2) const { return s1.x < s2.x; }
};
