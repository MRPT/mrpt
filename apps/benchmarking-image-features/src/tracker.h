...
public:
    ...
    /**
     * trackThemAll this function tracks the features based on the parameters specified by the user, the tracking process starts after the first 2
     ...
     * @param tracking_image_counter reads the given image from the vector of strings specified in files_fullpath_tracking variable
     * @param remove_lost_feats variable to ask the user if wants to remove the lost features 1/0
     * @param add_new_feats asks the user if they want to add new features as the trackers performs the tracking process
     * @param max_feats specifies the maximum number of features to be detectd in a frame
     * @param patch_size the patch size for mathing the key-points
     * @param window_width width of the window
     * @param window_height height of the window
     * @return
     */
    cv::Mat trackThemAll(vector<string> files_fullpath_tracking,
                         int tracking_image_counter,
                         int remove_lost_feats,
                         int add_new_feats,
                         int max_feats,
                         int patch_size,
                         int window_width,
