...
Tracker::Tracker()
{
    ...
    save_tracked_history = true; // Dump feat_track_history to a file at the end
    curCamPoseId = 0;
...
************************************************************************************************/
cv::Mat Tracker::trackThemAll(vector<string> files_fullpath_tracking,
                               ...
                               )
{
    ...
    tracker->extra_params["remove_lost_features"]         = remove_lost_feats;//;1;   // automatically remove out-of-image and badly tracked features

    tracker->extra_params["add_new_features"]             = add_new_feats ;//1;   // track, AND ALSO, add new features
    ...
    if (SHOW_FEAT_TRACKS) {
        ...
        // Purge old data:
        for (std::map<TFeatureID, std::list<TPixelCoord> >::iterator it = feat_tracks.begin();
             it != feat_tracks.end();) {
