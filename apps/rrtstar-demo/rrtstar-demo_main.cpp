...
string defaultSimpleMap = mrpt::system::getShareMRPTDir() + string("/datasets/malaga-cs-fac-building.simplemap.gz");
string defaultCfgFile   = mrpt::system::getShareMRPTDir() + string("/config_files/navigation-ptgs/ptrrt_config_example1.ini");
...
void TestRRT1()
{
	...
	ASSERT_FILE_EXISTS_(defaultSimpleMap);

	cout << "Loading map...";
	CFileGZInputStream(defaultSimpleMap) >> simplemap;
	...
	// Parameters:
	planner.loadConfig( mrpt::utils::CConfigFile(defaultCfgFile) );
