...
void xRawLogViewerFrame::SelectObjectInTreeView(const CSerializable::Ptr& sel_obj)
{
  ...
  if (obsStamp.has_value() && obsStamp.value() != INVALID_TIMESTAMP)
  {
    auto s = wxString::Format(
        ...
        mrpt::system::dateTimeLocalToString(*obsStamp).c_str(), mrpt::Clock::toDouble(*obsStamp));

    if (m_treeView->getFirstTimestamp() != INVALID_TIMESTAMP)
    {
      const double posInSeconds =
          mrpt::system::timeDifference(m_treeView->getFirstTimestamp(), *obsStamp);

      s += wxString::Format("Since rawlog beginning: %.03f [s]\n", posInSeconds);
    }
  ...
  if (classID == CLASS_ID(CObservation2DRangeScan))
  {
    ...
    for (size_t i = 0; i < xs.size(); i++) cout << mrpt::format("%7.04f %7.04f;", xs[i], ys[i]);
    cout << "]\n\n";
  ...
  if (classID == CLASS_ID(CObservation3DRangeScan))
  {
    ...
    // Update depth image ======
    {
      ...
      if (obs->hasRangeImage)
        auxImg = obs->rangeImage_getAsImage(mrpt::img::TColormap::cmHOT);
      else
...
// Update 3D view ==========
  ...
  if (classID == CLASS_ID(CObservationRotatingScan))
  {
    ...
    int rangeHeight = 40;
    {
      ...
      bmp3Dobs_depth->setViewportPosition(50, -2 - rangeHeight, -30, rangeHeight);
    }
    ...
    if (!obs->intensityImage.empty())
    {
      ...
      bmp3Dobs_int->setViewportPosition(
          50, -2 - rangeHeight - 2 - intensityHeight, -30, intensityHeight);
    }

    bmp3Dobs_3dcloud->addTextMessage(2, -2 - rangeHeight / 2, "Range", 0);
    bmp3Dobs_3dcloud->addTextMessage(2, -2 - rangeHeight - 2 - intensityHeight / 2, "Intensity", 1);
