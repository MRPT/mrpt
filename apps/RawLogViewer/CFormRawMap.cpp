...
CFormRawMap::CFormRawMap(wxWindow* parent,wxWindowID id)
{
    ...
    boxResults->Add(FlexGridSizer9, 1, wxALL|wxEXPAND|wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL, 0);
    FlexGridSizer7->Add(boxResults, 1, wxALL|wxEXPAND|wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL, 0);
...
void CFormRawMap::OnbtnSavePathClick(wxCommandEvent& event)
{
    ...
    wxString defaultDir( _U( iniFile->read_string(iniFileSect,"LastDir",".").c_str() ) );
    {
		...
		for ( CPose3DInterpolator::const_iterator i=robot_path.begin();i!=robot_path.end();++i)
		{
			...
			std::map<mrpt::system::TTimeStamp, mrpt::math::CMatrixDouble66 >::const_iterator Wit = rtk_path_info.vehicle_uncertainty.find(i->first);
