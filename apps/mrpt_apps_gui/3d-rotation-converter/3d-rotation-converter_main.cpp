...
struct AppData
{
  ...
  mrpt::viz::CSetOfObjects::Ptr gl_corner_user;
  ...
  mrpt::math::CVectorFixed<double, 3> in_lie_log = mrpt::math::CVectorFixed<double, 3>::Zero();
  ...
  nanogui::TextBox *edOutMatrix = nullptr, *edOutQuat = nullptr, *edOutAxisAngle_Ax = nullptr,
                   *edOutAxisAngle_An = nullptr, *edOutLogSO3 = nullptr;
...
void recalcAll()
{
  ...
  {
    default:
      THROW_EXCEPTION("Invalid tab index");
      break;
    ...
    case 0:
    {
      ...
      userPose.setFromValues(0, 0, 0, app.in_ypr[0] * K, app.in_ypr[1] * K, app.in_ypr[2] * K);
    }
    break;
    // Rot matrix:
    case 1:
    {
      ...
      auto R = mrpt::math::CMatrixDouble33((svd.matrixU() * svd.matrixV().transpose()).eval());
      userPose.setRotationMatrix(R);
      for (int r = 0; r < 3; r++)
      {
        for (int c = 0; c < 3; c++)
        {
          app.ed_in_rot[r][c]->setValue(mrpt::format("%.05f", R(r, c)));
        }
      }
    }
    break;
    ...
    {
      const double K = app.units_radians ? 1.0 : (M_PI / 180.0);
      mrpt::math::TVector3D v = app.in_axisangle_ax * (app.in_axisangle_ang * K);
    ...
    case 4:
    {
      userPose.setRotationMatrix(mrpt::poses::Lie::SO<3>::exp(app.in_lie_log));
    }
    break;
  };
  ...
  app.edOutLogSO3->setValue(mrpt::format("[%.05f %.05f %.05f]", log_R[0], log_R[1], log_R[2]));
...
void AppRotationConverter()
{
  ...
  app.gl_corner_user = mrpt::viz::stock_objects::CornerXYZ(1.0f);
  app.gl_corner_reference =
      mrpt::viz::CAxis::Create(-1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f, 0.2f, 1.0f, true);
  app.gl_corner_reference->setTextScale(0.04f);

  // Create GUI:
  app.win = mrpt::gui::CDisplayWindowGUI::Create("3D rotation converter", 900, 700, cp);

  // Add INPUT window:
  // -----------------------------
  nanogui::Window* winInput = new nanogui::Window(&(*app.win), "Rotation input");
  ...
  {  // Yaw/pitch/roll  tab:
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      auto* ed = layer->add<nanogui::TextBox>(mrpt::format("%.2f", app.in_quat[i]));
      ed->setEditable(true);
      ed->setFormat("[-+]?[0-9.e+-]*");
      ed->setCallback(
          [i](const std::string& s)
          {
            ...
            app.sl_in_ypr[i]->setValue(
                static_cast<float>(app.in_ypr[i] / (app.units_radians ? M_PI : 180.0f)));
            recalcAll();
            return true;
          });
      auto* sl = layer->add<nanogui::Slider>();
      app.sl_in_ypr[i] = sl;
  ...
  {  // rotation matrix tab:
    ...
    for (int r = 0; r < 3; r++)
    {
      for (int c = 0; c < 3; c++)
      {
        auto* ed = layer->add<nanogui::TextBox>(mrpt::format("%.04f", app.in_rot(r, c)));
        ed->setEditable(true);
        ed->setFormat("[-+]?[0-9.e+-]*");
        ed->setCallback(
            [r, c](const std::string& s)
            {
  ...
  {  // Quaternion tab:
    ...
    auto* layout = new nanogui::GridLayout(
        nanogui::Orientation::Horizontal, 2, nanogui::Alignment::Fill, 5, 0);
    layer->setLayout(layout);
  ...
  {  // Axis-angle tab:
...

    {
      ...
      for (int r = 0; r < 3; r++)
      {
        auto* ed = panel->add<nanogui::TextBox>(mrpt::format("%.04f", app.in_axisangle_ax[r]));
        ed->setEditable(true);
        ed->setFormat("[-+]?[0-9.e+-]*");
        ed->setCallback(
    ...
    }
    {
      ...
      auto* ed = layer->add<nanogui::TextBox>(mrpt::format("%.04f", app.in_axisangle_ang));
      ed->setEditable(true);
      ed->setFormat("[-+]?[0-9.e+-]*");
      ed->setCallback(
          [](const std::string& s)
  ...
  {  // axis with magnitude = log(R) in Lie group
    ...
    layer->add<nanogui::Label>("(i.e. vee(log(R)) in SO(3))");
    {
      ...
      for (int r = 0; r < 3; r++)
      {
        auto* ed = layer->add<nanogui::TextBox>(mrpt::format("%.04f", app.in_lie_log[r]));
        ed->setEditable(true);
        ed->setFormat("[-+]?[0-9.e+-]*");
        ed->setCallback(
            [r](const std::string& s)
  ...
  // Apply button:
  winInput->add<nanogui::Button>("Apply", ENTYPO_ICON_CALCULATOR)->setCallback(recalcAll);

  // Add top menu subwindow:
  // -----------------------------
  {
    nanogui::ref<nanogui::Window> winMenu = new nanogui::Window(&(*app.win), "");
    winMenu->setPosition(nanogui::Vector2i(0, 0));
    winMenu->setLayout(
        new nanogui::BoxLayout(nanogui::Orientation::Horizontal, nanogui::Alignment::Middle, 5));
    nanogui::Theme* modTheme = new nanogui::Theme(app.win->screen()->nvgContext());
    ...
    winMenu
        ->add<nanogui::CheckBox>(
            "Show reference frame", [&](bool b) { app.gl_corner_reference->setVisibility(b); })
        ->setChecked(true);

    winMenu
        ->add<nanogui::CheckBox>(
            "Show rotated frame", [&](bool b) { app.gl_corner_user->setVisibility(b); })
        ->setChecked(true);
    winMenu
        ->add<nanogui::CheckBox>(
    ...
    winMenu->add<nanogui::ComboBox>(std::vector<std::string>({"radians", "degrees"}))
        ->setCallback(
            [&](int index)
            {
              // On units update:
  ...
  // -----------------------------
  {
    nanogui::Window* winOutput = new nanogui::Window(&(*app.win), "Rotation output");
    ...
    {
      auto* panel = winOutput->add<nanogui::Widget>();
      panel->setLayout(new nanogui::GridLayout(
          nanogui::Orientation::Horizontal, 3, nanogui::Alignment::Fill, 5, 0));
      for (int r = 0; r < 3; r++)
      {
