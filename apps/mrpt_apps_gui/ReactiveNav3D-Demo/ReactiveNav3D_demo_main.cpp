...
int main(int num_arg, char* argv[])
{
  try
  {
    ...
    {
      int pushed_key = 0;
      if (ReactInterface.window.keyHit()) pushed_key = ReactInterface.window.getPushedKey();
      ...
      if (observer.mouse_click == 1)
      {
        ...
        {
          moving_target = false;
          const CAbstractNavigator::TNavigationParams nav_params = ReactInterface.createNewTarget(
              static_cast<float>(last_Target_Pos.x), static_cast<float>(last_Target_Pos.y), 0.3f,
              false);
        ...
        if (mousePos)
        {
          ...
          if (inters.getPoint(last_Target_Pos))
          {
            ...
            ReactInterface.scene->getByClass<CDisk>(0)->setLocation(
                last_Target_Pos.x, last_Target_Pos.y, last_Target_Pos.z + 0.01);
          }
        }
      }
    }
