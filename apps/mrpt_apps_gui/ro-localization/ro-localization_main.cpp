...
void TestParticlesLocalization()
{
  ...
  std::string OUT_DIR_PREFIX =
      iniFile->read_string("ro-localization", "logOutput_dir", "RO-LOCALIZATION_OUT");
  ...
  for (float range_Pc = Pc_range_ini; range_Pc <= Pc_range_end; range_Pc += Pc_range_step)
  {
    ...
    for (int repetition = 0; repetition < NUM_REPS; repetition++)
    {
      ...
      // saveToMATLABScript2D was removed in MRPT 3.x
...
// 3D World
      ...
      // World Axis
      {
        viz::CAxis::Ptr obj = std::make_shared<viz::CAxis>(-20, -10, -1, 20, 10, 4, 1);
        obj->enableTickMarks();
        obj->setColor(0, 0, 0);
#ifdef SHOW_REAL_TIME_3D
        sceneTR->insert(obj);
      ...
      while (rawlogEntry < (rawlogEntries - 1))
      {
        ...
        // ----------------------------------------
        odometryEstimation =
            odometryEstimation + action->getBestMovementEstimation()->poseChange->getMeanVal();
        ...
        if (groundTruth.rows() > step)
        {
          ...
          if (sensorPoseOnRobot_valid)
          {
            ...
            {
              mrpt::maps::CLandmark* lm = beacMap->landmarks.get(q);
              // Real range
        ...
        std::cout << "         PDF est: " << pdfEstimation
                  << ", ESS (B.R.)= " << PF_stats.ESS_beforeResample << " Cov:\n";
...
// The particles:
#ifdef SHOW_REAL_TIME_3D
          viz::CPointCloud::Ptr parts;

          {
            auto obj = sceneTR->getByName("part");
            if (!obj)
            {
              parts = viz::CPointCloud::Create();
              scene->insert(parts);
#ifdef SHOW_REAL_TIME_3D
              sceneTR->insert(parts);
#endif
...
#else
          viz::CPointCloud::Ptr parts = std::make_shared<viz::CPointCloud>();
          viz::CVisualObject::Ptr obj;
#endif
...
// The particles' cov:
#ifdef SHOW_REAL_TIME_3D
          viz::CEllipsoid2D::Ptr ellip;
          {
            auto obj = sceneTR->getByName("cov");
            if (!obj)
            {
              ellip = std::make_shared<viz::CEllipsoid2D>();
              scene->insert(ellip);
#ifdef SHOW_REAL_TIME_3D
...
#ifdef SHOW_REAL_TIME_3D
          // The laser scan:
          viz::CPointCloud::Ptr scanPts;
          {
            auto obj = sceneTR->getByName("laser");
            if (!obj)
            {
              scanPts = viz::CPointCloud::Create();
          ...
          if (auto dist = observations->getObservationByClass<CObservationBeaconRanges>();
              beacMap && dist && !dist->sensedData.empty())
          {
            for (auto& k : dist->sensedData)
            {
              ...
              {
#ifdef SHOW_REAL_TIME_3D
                viz::CDisk::Ptr sphere;
                {
                  auto obj = sceneTR->getByName(beacon_name);
                  if (!obj)
                  {
                    sphere = std::make_shared<viz::CDisk>();
                    scene->insert(sphere);
#ifdef SHOW_REAL_TIME_3D
...
#else
                viz::CSphere::Ptr sphere = std::make_shared<viz::CSphere>();
                viz::CVisualObject::Ptr obj;
#endif
                ...
                float R = square(k.sensedDistance) -
                          square(k.sensorLocationOnRobot.z() - lm->pose_mean.z);
          ...
          {
            viz::CVisualObject::Ptr obj = sceneTR->getByName("franc");
            viz::CSphere::Ptr sphere;

            if (!obj)
              sphere = std::make_shared<viz::CSphere>();
            else
              sphere = std::dynamic_pointer_cast<viz::CSphere>(obj);
            sphere->setColor(0, 1, 0);
            sphere->setRadius(0.05f);
            sphere->setName("franc");

            if (auto dist = observations->getObservationByClass<CObservationBeaconRanges>(); dist)
            {
              sphere->setLocation(dist->auxEstimatePose.x(), dist->auxEstimatePose.y(), 0.05f);
            }
          ...
          // GPS pose
          {
            auto o = observations->getObservationByClass<CObservationGPS>();
            if (o && beacMap)
            {
              viz::CEllipsoid2D::Ptr sphere;
              double x, y;

              {
                ...
                if (!obj)
                {
                  sphere = std::make_shared<viz::CEllipsoid2D>();
                  scene->insert(sphere);
                  sceneTR->insert(sphere);
                }
                else
                  sphere = std::dynamic_pointer_cast<viz::CEllipsoid2D>(obj);
              }
              ...
              if (o->hasMsgClass<mrpt::obs::gnss::Message_NMEA_GGA>())
              {
                ...
                x = DEG2RAD(
                        (gga.fields.longitude_degrees -
                         beacMap->likelihoodOptions.GPSOrigin.longitude)) *
                    6371000 * 1.03;
                y = DEG2RAD(
                        (gga.fields.latitude_degrees -
                         beacMap->likelihoodOptions.GPSOrigin.latitude)) *
                    6371000 * 1.15;
                sphere->setLocation(
                    (x * cos(beacMap->likelihoodOptions.GPSOrigin.ang) +
                     y * sin(beacMap->likelihoodOptions.GPSOrigin.ang) +
                     beacMap->likelihoodOptions.GPSOrigin.x_shift),
                    (-x * sin(beacMap->likelihoodOptions.GPSOrigin.ang) +
                     y * cos(beacMap->likelihoodOptions.GPSOrigin.ang) +
                     beacMap->likelihoodOptions.GPSOrigin.y_shift),
                    0);
              }
