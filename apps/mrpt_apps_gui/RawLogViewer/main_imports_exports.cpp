...
void parseGeneralVector(const std::string& str, size_t idx, vector<float>& outVector);
...
void xRawLogViewerFrame::OnImportCARMEN([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString caption = "Import a CARMEN log...";
  wxString wildcard = "CARMEN log files (*.log)|*.log|All files (*.*)|*.*";
  ...
  wxString strMaxLaserRange =
      wxGetTextFromUser(_("Maximum valid laser range:"), _("Enter parameter:"), _("30"));
...
void xRawLogViewerFrame::OnImportSequenceOfImages([[maybe_unused]] wxCommandEvent& event)
{
  ...
  string inDir = dirDialog.GetPath().ToStdString();
  ...
  wxString strFPS = wxGetTextFromUser(
      _("Provide frame rate (fps, or Hz):"), _("To generate fake timestamps:"), _("10"));
  double FPS;
  ...
  for (int countLoop = 0; countLoop < nEntries; countLoop++)
  {
    if (countLoop % 5 == 0)
    {
      auxStr.sprintf("Parsed %u files", countLoop);
    ...
    try
    {
      ...
      if (!os::_strcmpi("pgm", filExt.c_str()) || !os::_strcmpi("jpg", filExt.c_str()) ||
          !os::_strcmpi("bmp", filExt.c_str()) || !os::_strcmpi("png", filExt.c_str()) ||
          !os::_strcmpi("jpeg", filExt.c_str()) || !os::_strcmpi("tif", filExt.c_str()) ||
          !os::_strcmpi("tiff", filExt.c_str()) || !os::_strcmpi("dib", filExt.c_str()) ||
          !os::_strcmpi("jpe", filExt.c_str()) || !os::_strcmpi("pbm", filExt.c_str()) ||
          !os::_strcmpi("ppm", filExt.c_str()) || !os::_strcmpi("sr", filExt.c_str()) ||
...
{
  wxString caption = "Export as ALOG...";
  wxString wildcard = "ALOG files (*.alog)|*.alog|All files (*.*)|*.*";
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
  ...
  if (dialog.ShowModal() == wxID_OK)
  {
    ...
    wxString strRobot = wxGetTextFromUser(
        _("Enter the name of the robot to use in ALOG:"), _("Robot name:"), _("ROBOT"));

    string robot_name = string(strRobot.ToStdString());
    ...
    // Save the file:
    string fileName = string(fil.ToStdString());
    ...
    wxProgressDialog progDia(
        "Exporting rawlog to ALOG", "Saving...",
        n,     // range
        this,  // parent
        wxPD_CAN_ABORT | wxPD_APP_MODAL | wxPD_SMOOTH | wxPD_AUTO_HIDE | wxPD_ELAPSED_TIME |
            wxPD_ESTIMATED_TIME | wxPD_REMAINING_TIME);
          ...
          lastTime != INVALID_TIMESTAMP)
      {
        ...
        if (act)
        {
          ...
          ::fprintf(
              ...
              time_odo, robot_name.c_str(), time_odo, MOOS_odometry.x(), MOOS_odometry.y(),
              MOOS_odometry.phi());
        }
      }
      else if (rawlog.getType(i) == CRawlog::TEntryType::etSensoryFrame)
      {
        ...
        if (obs)
        {
          ...
          ::fprintf(
              ...
              time_obs, time_obs, RAD2DEG(-obs->aperture / 2), RAD2DEG(obs->aperture / 2), 181);
          ...
          if (M_real == 361)
          {
            for (unsigned int j = 0; j < 181; j++)
            {
              ...
              float val = obs->getScanRangeValidity(idx) ? obs->getScanRange(idx) : 0;
              if (j < (181 - 1))
                ::fprintf(f, "%.03f,", val);
              else
                ::fprintf(f, "%.03f}\n", val);
        ...
        THROW_EXCEPTION("Export to ALOG only implemented for SF-based rawlogs.");
...
void xRawLogViewerFrame::OnImportRTL([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString caption = "Import a RTL log file...";
  wxString wildcard = "RTL files (*.rtl)|*.rtl|All files (*.*)|*.*";
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
      ...
      if (strLine.size() > 3)
      {
        ...
        else if (strLine[0] == 'L')
        {
          ...
          if (rawdata.size() > 10)
          {
            ...
            {
              float rng = sqrt(
                  square(rawdata[1 + i * 4 + 2] - rawdata[1 + i * 4 + 0]) +
  ...
  string target_filename;
  {
    ...
    wildcard =
        ...
        "files (*.*)|*.*";
    defaultDir = (iniFile->read_string(iniFileSect, "LastDir", ".").c_str());
...
// [N]={X1,X2,...}
void parseMOOSVector(const std::string& str, size_t idx, vector<float>& outVector)
{
  ASSERT_(str[idx] == '[');
...
void xRawLogViewerFrame::OnMenuImportALOG([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString caption = "Import a MOOS alog file...";
  wxString wildcard = "MOOS alog files (*.alog)|*.alog|All files (*.*)|*.*";
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
      ...
      if (idx != string::npos)
      {
        ...
        if (string::npos != (idx = strLine.find("_ODOMETRY")))
        {
          if (string::npos != (idx = strLine.find("Pose=[3x1]{")))
          {
            newRecord.data.resize(6);
            if (3 == sscanf(
                         strLine.c_str() + idx + 11, "%f,%f,%f", &newRecord.data[0],
                         &newRecord.data[1], &newRecord.data[2]))
            {
              if (string::npos != (idx = strLine.find("Vel=[3x1]{")))
              {
                if (3 == sscanf(
                             strLine.c_str() + idx + 10, "%f,%f,%f", &newRecord.data[3],
                             &newRecord.data[4], &newRecord.data[5]))
                {
                  newRecord.type = 0;  // OK, valid odometry
                }
              }
  ...
  if (nImgs)
  {
    ...
    dir_for_images = string(dirDialog.GetPath().ToStdString());
  }
  else
  {
    ...
    const std::string& dir_for_images)
{
  ...
  for (progress_i = 0, it = theAlog.begin(); it != theAlog.end(); it++)
  {
    ...
    if (it->second.type != 0)
    {
      ...
      switch (it->second.type)
      {
        ...
        case 2:
        {
          ...
          obs->sensorPose.setFromValues(0, 0, 0, 0, -DEG2RAD(it->second.startElev), 0);
        }
        break;
      ...
      if (newObs)
      {
        ...
        if (firstSFtime != INVALID_TIMESTAMP && timeDifference(firstSFtime, tim) > 0.3)
        {
    ...
    else
    {
      ...
      CPose2D curOdo(it->second.data[0], it->second.data[1], it->second.data[2] + M_PI / 2);
...
void xRawLogViewerFrame::OnGenGasTxt([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
  wxString defaultFilename = (loadedFileName + string("_gasSensors.txt")).c_str();
  wxFileDialog dialog(
      this, caption, defaultDir, defaultFilename, wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
    ...
    for (i = 0; i < n; i++)
    {
      ...
      if (obs)
      {
        ...
        for (auto& m_reading : obs->m_readings)
        {
          ...
          for (auto it = m_reading.readingsVoltage.begin(); it != m_reading.readingsVoltage.end();
               ++it)
          {
            ::fprintf(f, "%f ", *it);
            lineCount++;
...
void xRawLogViewerFrame::OnGenWifiTxt([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString wildcard = "Text files (*.txt)|*.txt|All files (*.*)|*.*";
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
  wxString defaultFilename = (loadedFileName + string("_wifiSensors.txt")).c_str();
  wxFileDialog dialog(
      this, caption, defaultDir, defaultFilename, wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
    ...
    for (i = 0; i < n; i++)
    {
      ...
      switch (rawlog.getType(i))
      {
        ...
        case CRawlog::TEntryType::etObservation:
        {
          ...
          {
            obs =
                std::dynamic_pointer_cast<CObservationWirelessPower>(o);  // Get the GAS observation
          }
        }
        break;
...
void xRawLogViewerFrame::OnGenRFIDTxt([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString wildcard = "Text files (*.txt)|*.txt|All files (*.*)|*.*";
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
  wxString defaultFilename = (loadedFileName + string("_RFIDSensors.txt")).c_str();
  wxFileDialog dialog(
      this, caption, defaultDir, defaultFilename, wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
...
void xRawLogViewerFrame::OnMenuItemImportBremenDLRLog([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString caption = "Import a uni-bremen DLR dataset...";
  wxString wildcard =
      "Uni-bremen DLR circles dataset (*.circles)|*.circles|All files "
      "(*.*)|*.*";
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
  ...
  */
  ...
  int32_t next_outlier_ID = 10000;  // In DLR datasets no real landmark has
  // such a high ID... use these numbers for
  // outliers.
  ...
  while (fileParser.getNextLine(line))
  {
    ...
    if (words[0] == "STEP")
    {
      ...
      if (set_of_obs.empty())
      {
        ...
        // images without any detected landmark
      ...
      odoParams.gaussianModel.a1 = odoParams.gaussianModel.a2 = odoParams.gaussianModel.a3 =
      ...
      else
      {
        ...
        newRawlog.insert(std::make_shared<CObservationOdometry>(obs_odo));
      }
    }
    else if (words[0] == "LANDMARK_C")
    {
      ...
      const double lm_x = -atof(words[2].c_str());  // Ey! Yes, I flipped the coord.
      ...
      const double dat_C_xy[3 * 3] = {Cxy_xx, Cxy_xy, 0, Cxy_xy, Cxy_yy, 0, 0, 0, 0};
      const CMatrixDouble33 C_xy(dat_C_xy);

      // Jacobian:
      const double dat_H[3 * 3] = {lm_x / r, lm_y / r, 0, -lm_y / (r * r), lm_x / (r * r), 0,
                                   0,        0,        0};
...
void xRawLogViewerFrame::OnGenerateIMUTextFile([[maybe_unused]] wxCommandEvent& event)
{
  ...
  if (dialog.ShowModal() == wxID_OK)
  {
    ...
    ::fprintf(
        ...
        "IMU_PITCH IMU_ROLL IMU_X IMU_Y IMU_Z MAG_X MAG_Y MAG_Z PRESS "
        "ALTIT TEMP\n"
        "%% "
        "------------------------------------------------------------------"
        "------------------------------------------------------------------"
          ...
          do
          {
            ...
            if (obs)
            {
              ...
              if (obs->rawMeasurements.size() > 0)
              {
                ...
                // timestamp and save all 15 values:
        ...
        case CRawlog::TEntryType::etObservation:
        {
          ...
          if (oo->GetRuntimeClass() == CLASS_ID(CObservationIMU))
          {
            ...
            if (obs->rawMeasurements.size() > 0)
            {
              ...
              double sampleTime =
                  mrpt::Clock::toDouble(t);  // timeDifference(rawlog_first_timestamp,t);
...
void xRawLogViewerFrame::OnGenerateTextFileRangeBearing([[maybe_unused]] wxCommandEvent& event)
{
  ...
  wxString wildcard = "Text files (*.txt)|*.txt|All files (*.*)|*.*";
  wxString defaultDir((iniFile->read_string(iniFileSect, "LastDir", ".").c_str()));
  wxString defaultFilename = (loadedFileName + string("_RANGE_BEARING.txt")).c_str();
  wxFileDialog dialog(
      this, caption, defaultDir, defaultFilename, wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
    ...
    for (i = 0; i < n; i++)
    {
      if (rawlog.getType(i) == CRawlog::TEntryType::etSensoryFrame)
      {
        ...
        if (obs)
        {
          ...
          {
            M++;
            ::fprintf(f, "%u %i %f %f %f\n", i, q.landmarkID, q.range, q.yaw, q.pitch);
          }
        }
      }
