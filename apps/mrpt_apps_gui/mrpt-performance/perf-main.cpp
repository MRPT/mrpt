...
int main(int argc, char** argv)
{
  try
  {
    CLI::App app("mrpt-performance");
    app.set_version_flag("--version", MRPT_getVersion());

    std::string match_contains;
    app.add_option(
        "-c,--match-contains", match_contains, "Run only the tests containing the given substring");

    bool build_tables = false;
    app.add_flag(
        "-t,--tables", build_tables,
        "Don't run any test, instead build the tables of compared performances in SOURCE_DIR/doc/");

    bool release = false;
    app.add_flag(
        "-r,--release", release, "Don't use the postfix 'dev' in the performance stats file");

    // Parse arguments:
    CLI11_PARSE(app, argc, argv);

    if (build_tables) return run_build_tables();
    ...
    {
      const char* version_postfix = release ? "" : "dev";
...
#define COMP_VER(NAME, MAJ, MIN, PATCH) NAME ___STR1__(MAJ) ___STR1__(MIN) ___STR1__(PATCH)
  ...
  catch (const std::exception& e)
  {
    if (::strlen(e.what()))
    {
      ...
      std::cerr << "Program finished for an exception!!"
                << "\n";
