...
{
  wxCHECK_MSG(optValue.Ok(), false, "Invalid wxOptionValue");
...
bool wxOptionValue::Create(const wxString &string)
{
  ...
  {
    int i;
    wxPrintf("wxOptionValue::wxOptionValue( const wxString &string BUSTED\n");
  ...
  return (
      (M_OPTVALUDATA->m_optionValues.GetCount() > 0) &&
...
bool wxOptionValue::Ok() const { return M_OPTVALUDATA != nullptr; }
...
{
  wxCHECK_MSG(Ok(), wxEmptyString, "Invalid wxOptionValue");
  return M_OPTVALUDATA->m_type;
}

void wxOptionValue::SetType(const wxString &type)
{
  wxCHECK_RET(Ok(), "Invalid wxOptionValue");
...
{
  wxCHECK_MSG(Ok(), wxArrayString(), "Invalid wxOptionValue");
...
{
  wxCHECK_MSG(
      Ok() && (n < M_OPTVALUDATA->m_optionNames.GetCount()), wxEmptyString,
...
{
  wxCHECK_MSG(
      Ok() && (n < M_OPTVALUDATA->m_optionValues.GetCount()), wxEmptyString,
      "Invalid wxOptionValue");
...
{
  wxCHECK_MSG(Ok(), wxNOT_FOUND, "Invalid wxOptionValue");
...
{
  wxCHECK_MSG(Ok(), false, "Invalid wxOptionValue");
  int index = M_OPTVALUDATA->m_optionNames.Index(name, false);
...
{
  wxCHECK_MSG(Ok(), false, "Invalid wxOptionValue");
  wxCHECK_MSG(n < M_OPTVALUDATA->m_optionValues.GetCount(), false, "invalid index");
...
{
  wxCHECK_RET(Ok() && (name.Length() > 0), "Invalid wxOptionValue or option");
...
int wxOptionValue::GetOption(
    const wxString &name, wxArrayInt &values, int count, const wxString &delims) const
{
...
bool wxOptionValue::GetOption(
    const wxString &name, unsigned char *value, int count, const wxString &delims) const
{
  wxArrayInt intArr;
  intArr.Alloc(count);
...
bool wxOptionValue::GetOption(
    const wxString &name, int *value, int count, const wxString &delims) const
{
  wxArrayInt intArr;
  intArr.Alloc(count);
  if (GetOption(name, intArr, count, delims) != count) return false;
...
bool wxOptionValue::GetOption(
    const wxString &name, long *value, int count, const wxString &delims) const
{
  wxArrayInt intArr;
  intArr.Alloc(count);
  if (GetOption(name, intArr, count, delims) != count) return false;
...
bool wxOptionValue::GetOption(
    const wxString &name, float *value, int count, const wxString &delims) const
{
...
bool wxOptionValue::GetOption(
    const wxString &name, double *value, int count, const wxString &delims) const
{
...
bool wxOptionValue::GetOption(
    const wxString &name, int *v1, int *v2, int *v3, const wxString &delims) const
{
  wxArrayInt intArr;
...
bool wxOptionValue::GetOption(
    const wxString &name, float *v1, float *v2, float *v3, const wxString &delims) const
{
  double nums[3];
