...
wxBmpComboPopupChild::wxBmpComboPopupChild(wxWindow *parent, wxBmpComboBox *owner) :
    wxScrolledWindow(
        parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxSUNKEN_BORDER | wxHSCROLL | wxVSCROLL)
{
  m_bmpCombo = owner;
  m_last_selection = -1;
...
bool wxBmpComboBox::SetButtonStyle(long style)
{
  ...
  wxCHECK_MSG(n_styles < 2, false, "Only one wxBmpComboBox label position allowed");
...
void wxBmpComboBox::CalcLabelBitmapPos(
    int n, const wxSize &area, wxPoint &labelPos, wxPoint &bitmapPos) const
{
  ...
  else  // if ((m_label_style & wxBMPCOMBO_LEFT) != 0)
  {
    labelPos = wxPoint(BORDER, (area.y - lh) / 2);
    bitmapPos = wxPoint(
        BORDER * 2 + m_labelSize.x + (area.x - BORDER * 2 - m_labelSize.x - bw) / 2,
        (area.y - bh) / 2);
  }
}
...
{
  wxCHECK_MSG(int(n) < GetCount(), wxNOT_FOUND, "invalid index");
...
{
  wxCHECK_RET(int(n + count) <= GetCount(), "invalid index");
...
{
  wxCHECK_MSG((n >= 0) && (n < GetCount()), wxEmptyString, "invalid index");
  return m_labels[n];
}

wxBitmap wxBmpComboBox::GetItemBitmap(int n) const
{
  wxCHECK_MSG((n >= 0) && (n < GetCount()), wxNullBitmap, "invalid index");
  return *(wxBitmap *)m_bitmaps.Item(n);
}

void wxBmpComboBox::SetSelection(int n, bool send_event)
{
  wxCHECK_RET((n >= 0) && (n < GetCount()), "invalid index");
  m_selection = n;
  m_labelWin->Refresh(true);
