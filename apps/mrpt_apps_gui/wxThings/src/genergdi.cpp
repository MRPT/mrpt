...
class wxGenericPenRefData : public wxObjectRefData
{
 public:
  wxGenericPenRefData(
      int width = 1, int style = wxSOLID, int cap = wxCAP_ROUND, int join = wxJOIN_ROUND) :
      wxObjectRefData(),
      m_width(width),
      m_style(style),
      m_cap(cap),
...
void wxGenericPen::Create(const wxGenericColour &colour, int width, int style, int cap, int join)
{
  UnRef();
  m_refData = new wxGenericPenRefData(width, style, cap, join);
  M_GPENDATA->m_colour = colour;
}
void wxGenericPen::Create(const wxColour &colour, int width, int style, int cap, int join)
{
...
void wxGenericPen::SetDashes(int number_of_dashes, const wxDash *dash)
{
  wxCHECK_RET(Ok(), "Invalid generic pen");
  wxCHECK_RET(
      ((number_of_dashes == 0) && !dash) || ((number_of_dashes > 0) && dash),
      "Invalid dashes for pen");
...
{
  wxCHECK_MSG(Ok(), nullptr, "Invalid generic pen");
...
{
  wxCHECK_MSG(Ok(), nullptr, "Invalid generic brush");
