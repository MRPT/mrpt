...
bool wxMatrix2D::Copy(const wxMatrix2D &matrix)
{
  ...
  memcpy(
      M_MATRIXDATA->m_data, matrix.GetData(),
      M_MATRIXDATA->m_width * M_MATRIXDATA->m_height * sizeof(double));

  return true;
}

void wxMatrix2D::Destroy() { UnRef(); }

bool wxMatrix2D::Ok() const { return M_MATRIXDATA != nullptr; }
...
wxMatrix2D wxMatrix2D::InsertRows(int row, const wxMatrix2D &matrix) const
{
  ...
  else  // insert the matrix in the middle
  {
    ...
    memcpy(
        &temp_data[width * row + matrix_size], &data[width * row],
        sizeof(double) * (size - width * row));
  }
...
wxMatrix2D wxMatrix2D::InsertCols(int col, const wxMatrix2D &matrix) const
{
  ...
  for (j = 0; j < height; j++)
  {
    ...
    {
      memcpy(
          &temp_data[j * temp_width], &matrix_data[j * matrix_width],
          sizeof(double) * matrix_width);
    ...
    else if (col >= width)  // append the matrix to the end
    {
      ...
      memcpy(
          &temp_data[j * temp_width + width], &matrix_data[j * matrix_width],
          sizeof(double) * matrix_width);
    }
    else  // insert the matrix in the middle
    {
      ...
      memcpy(
          &temp_data[j * temp_width + col], &matrix_data[j * matrix_width],
          sizeof(double) * matrix_width);
      memcpy(
          &temp_data[j * temp_width + col + matrix_width], &data[j * width + col],
    ...
    else
    {
      ...
      if (num_cols < 1)
      {
        for (i = 0; i < 10000; ++i)
        {
          ...
          {
            printf(
                "i%d pts%d cols%d rows%d pos%d pt%lf \n", i, num_points, num_cols, num_rows, pos,
                point);
            fflush(stdout);
            // data[i] = point;
            ++num_cols;
