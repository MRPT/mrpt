...
class WXDLLIMPEXP_THINGS wxGenericColour : public wxObject
{
 public:
  ...
  // bool ReadString(const wxString& str, const wxString& format =
  // wxT("%d,%d,%d,%d"));
...
class WXDLLIMPEXP_THINGS wxGenericPen : public wxObject
{
 public:
  ...
  wxGenericPen(
      const wxGenericColour& colour,
      int width = 1,
      int style = wxSOLID,
      int cap = wxCAP_ROUND,
      int join = wxJOIN_ROUND) :
      wxObject()
  {
...
class WXDLLIMPEXP_THINGS wxGenericBrush : public wxObject
{
 public:
  ...
  bool IsHatch() const { return (GetStyle() >= wxFIRST_HATCH) && (GetStyle() <= wxLAST_HATCH); }
