...
void Test_Kinect()
{
  ...
  mrpt::viz::CSetOfObjects::Ptr gl_curFeats = mrpt::viz::CSetOfObjects::Create();
  mrpt::viz::CSetOfObjects::Ptr gl_keyframes = mrpt::viz::CSetOfObjects::Create();

  mrpt::viz::CPointCloudColoured::Ptr gl_points_map = mrpt::viz::CPointCloudColoured::Create();
  gl_points_map->setPointSize(2.0);

  const double aspect_ratio = 480.0 / 640.0;  // kinect.rows() / double( kinect.cols() );

  mrpt::viz::CSetOfObjects::Ptr gl_cur_cam_corner =
      mrpt::viz::stock_objects::CornerXYZSimple(0.4f, 4);
  ...
  while (win3D.isOpen() && !thrPar.quit)
  {
        ...
        (!last_obs || possiblyNewObs->timestamp != last_obs->timestamp))
    {
      ...
      if (step_num > 1)  // we need "previous_image" to be valid.
      {
        ...
        for (auto itFeat = trackedFeats.begin(); itFeat != trackedFeats.end();)
        {
      ...
      for (auto& trackedFeat : trackedFeats)
      {
        ...
        const float d = last_obs->rangeImage(y, x) * last_obs->rangeUnits;
        if (d > 0.05 && d < 10.0)
        {
          ASSERT_(
              size_t(last_obs->rangeImage.cols() * last_obs->rangeImage.rows()) ==
              last_obs->points3D_x.size());
          const size_t nPt = last_obs->rangeImage.cols() * y + x;
          curVisibleFeats[trackedFeat.ID] = TPoint3D(
              last_obs->points3D_x[nPt], last_obs->points3D_y[nPt], last_obs->points3D_z[nPt]);
        }
      }
      ...
      if (!lastVisibleFeats.empty())
      {
        ...
        for (auto itCur = curVisibleFeats.begin(); itCur != curVisibleFeats.end(); ++itCur)
        {
          auto itFound = lastVisibleFeats.find(itCur->first);
          if (itFound != lastVisibleFeats.end())
          {
            corrs.push_back(TMatchingPair(
                itFound->first, itCur->first, itFound->second.x, itFound->second.y,
                itFound->second.z, itCur->second.x, itCur->second.y, itCur->second.z));
          }
        }
        ...
        if (corrs.size() >= 3)
        {
          ...
          str_status2 =
              string(results.inliers_idx.size() == 0 ? "LOST! Please, press 'r' to restart" : "");
          ...
          if (register_ok && std::abs(results.scale - 1.0) < 0.1)
          {
                 ...
                 std::abs(relativePose.roll()) > KEYFRAMES_MIN_ANG))
            {
              ...
              currentCamPose_wrt_last = CPose3D();  // It's (0,0,0) since the last
              ...
              // position:
              globalPtsMap.insertObservation(*last_obs, new_keyframe_global);
      ...
      if (last_obs->hasPoints3D)
      {
        ...
        gl_curFeats->clear();
        for (auto it = curVisibleFeats.begin(); it != curVisibleFeats.end(); ++it)
        {
          static double D = 0.02;
          mrpt::viz::CBox::Ptr box =
              mrpt::viz::CBox::Create(TPoint3D(-D, -D, -D), TPoint3D(D, D, D));
