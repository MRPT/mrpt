	...
	, m_ui(std::make_unique<Ui::CLandmarksConfig>())
{
	...
	m_ui->implementation->addItem("VedaldiBinary", CFeatureExtraction::VedaldiBinary);
...
void CLandmarksConfig::updateConfiguration(mrpt::maps::TMetricMapInitializer *options)
{
	...
	mapDefination->insertionOpts.insert_SIFTs_from_monocular_images = m_ui->insert_SIFTs_from_monocular_images->isChecked();
	mapDefination->insertionOpts.insert_SIFTs_from_stereo_images = m_ui->insert_SIFTs_from_stereo_images->isChecked();
	mapDefination->insertionOpts.insert_Landmarks_from_range_scans = m_ui->insert_Landmarks_from_range_scans->isChecked();

	mapDefination->insertionOpts.SiftCorrRatioThreshold = m_ui->SiftCorrRatioThreshold->value();
	mapDefination->insertionOpts.SiftLikelihoodThreshold = m_ui->SiftLikelihoodThreshold->value();
	mapDefination->insertionOpts.SiftEDDThreshold = m_ui->SiftEDDThreshold->value();
	mapDefination->insertionOpts.SIFTMatching3DMethod = m_ui->SIFTMatching3DMethod->value();

	mapDefination->insertionOpts.SIFTsLoadDistanceOfTheMean = m_ui->SIFTsLoadDistanceOfTheMean->value();
	mapDefination->insertionOpts.SIFTsLoadEllipsoidWidth = m_ui->SIFTsLoadEllipsoidWidth->value();
	mapDefination->insertionOpts.SIFTs_stdXY = m_ui->SIFTs_stdXY->value();
	mapDefination->insertionOpts.SIFTs_stdDisparity = m_ui->SIFTs_stdDisparity->value();
	mapDefination->insertionOpts.SIFTs_numberOfKLTKeypoints = m_ui->SIFTs_numberOfKLTKeypoints->value();
	mapDefination->insertionOpts.SIFTs_stereo_maxDepth = m_ui->SIFTs_stereo_maxDepth->value();
	mapDefination->insertionOpts.SIFTs_epipolar_TH = m_ui->SIFTs_epipolar_TH->value();
	mapDefination->insertionOpts.PLOT_IMAGES = m_ui->PLOT_IMAGES->isChecked();


	mapDefination->likelihoodOpts.rangeScan2D_decimation = m_ui->rangeScan2D_decimation->value();
	mapDefination->likelihoodOpts.SIFTs_sigma_euclidean_dist = m_ui->SIFTs_sigma_euclidean_dist->value();
	mapDefination->likelihoodOpts.SIFTs_sigma_descriptor_dist = m_ui->SIFTs_sigma_descriptor_dist->value();
	mapDefination->likelihoodOpts.SIFTs_mahaDist_std = m_ui->SIFTs_mahaDist_std->value();
	mapDefination->likelihoodOpts.SIFTnullCorrespondenceDistance = m_ui->SIFTnullCorrespondenceDistance->value();
	mapDefination->likelihoodOpts.SIFTs_decimation = m_ui->SIFTs_decimation->value();//int

	mapDefination->likelihoodOpts.beaconRangesStd = m_ui->beaconRangesStd->value();
	mapDefination->likelihoodOpts.beaconRangesUseObservationStd = m_ui->beaconRangesUseObservationStd->isChecked();
	mapDefination->likelihoodOpts.extRobotPoseStd = m_ui->extRobotPoseStd->value();
	mapDefination->likelihoodOpts.GPS_sigma = m_ui->GPS_sigma->value();

	mapDefination->likelihoodOpts.GPSOrigin.longitude = m_ui->longitude->value();
	...
	mapDefination->likelihoodOpts.SIFT_feat_options.featsType = static_cast<TFeatureType>(m_ui->TFeatureType->currentData().toInt());
	mapDefination->likelihoodOpts.SIFT_feat_options.patchSize = m_ui->patchSize->value();

	mapDefination->likelihoodOpts.SIFT_feat_options.useMask = m_ui->useMask->isChecked();
	mapDefination->likelihoodOpts.SIFT_feat_options.addNewFeatures = m_ui->addNewFeatures->isChecked();
	mapDefination->likelihoodOpts.SIFT_feat_options.FIND_SUBPIXEL = m_ui->FIND_SUBPIXEL->isChecked();

	mapDefination->likelihoodOpts.SIFT_feat_options.KLTOptions.radius = m_ui->radiusKLTOptions->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.KLTOptions.threshold = m_ui->thresholdKLTOptions->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.KLTOptions.min_distance = m_ui->min_distanceKLTOptions->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.KLTOptions.tile_image = m_ui->tile_imageKLTOptions->isChecked();

	mapDefination->likelihoodOpts.SIFT_feat_options.harrisOptions.threshold = m_ui->thresholdHarris->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.harrisOptions.k = m_ui->k->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.harrisOptions.sigma = m_ui->sigma->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.harrisOptions.radius = m_ui->radiusHarris->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.harrisOptions.min_distance = m_ui->min_distanceHarris->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.harrisOptions.tile_image = m_ui->tile_imageHarris->isChecked();

	mapDefination->likelihoodOpts.SIFT_feat_options.FASTOptions.threshold = m_ui->thresholdFASTOptions->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.FASTOptions.min_distance = m_ui->min_distanceFASTOptions->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.FASTOptions.nonmax_suppression = m_ui->nonmax_suppression->isChecked();
	mapDefination->likelihoodOpts.SIFT_feat_options.FASTOptions.use_KLT_response = m_ui->use_KLT_response->isChecked();

	mapDefination->likelihoodOpts.SIFT_feat_options.ORBOptions.n_levels = m_ui->n_levels->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.ORBOptions.min_distance = m_ui->min_distanceORBOptions->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.ORBOptions.scale_factor = m_ui->scale_factor->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.ORBOptions.extract_patch = m_ui->extract_patch->isChecked();

	mapDefination->likelihoodOpts.SIFT_feat_options.SIFTOptions.implementation = static_cast<CFeatureExtraction::TSIFTImplementation>(m_ui->implementation->currentData().toInt());
	mapDefination->likelihoodOpts.SIFT_feat_options.SIFTOptions.threshold = m_ui->thresholdSIFTOptions->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.SIFTOptions.edgeThreshold = m_ui->edgeThreshold->value();

	mapDefination->likelihoodOpts.SIFT_feat_options.SURFOptions.rotation_invariant = m_ui->rotation_invariant->isChecked();
	mapDefination->likelihoodOpts.SIFT_feat_options.SURFOptions.hessianThreshold = m_ui->hessianThreshold->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.SURFOptions.nOctaves = m_ui->nOctaves->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.SURFOptions.nLayersPerOctave = m_ui->nLayersPerOctave->value();

	mapDefination->likelihoodOpts.SIFT_feat_options.SpinImagesOptions.hist_size_intensity = m_ui->hist_size_intensity->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.SpinImagesOptions.hist_size_distance = m_ui->hist_size_distance->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.SpinImagesOptions.std_dist = m_ui->std_dist->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.SpinImagesOptions.std_intensity = m_ui->std_intensity->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.SpinImagesOptions.radius = m_ui->radiusSpinImagesOptions->value();

	mapDefination->likelihoodOpts.SIFT_feat_options.PolarImagesOptions.bins_angle = m_ui->bins_angle->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.PolarImagesOptions.bins_distance = m_ui->bins_distance->value();
	mapDefination->likelihoodOpts.SIFT_feat_options.PolarImagesOptions.radius = m_ui->radiusPolarImagesOptions->value();

	mapDefination->likelihoodOpts.SIFT_feat_options.LogPolarImagesOptions.radius = m_ui->radiusLogPolarImagesOptions->value();
