...
COccupancyConfig::COccupancyConfig() : CBaseConfig(), m_ui(std::make_unique<Ui::COccupancyConfig>())
{
  m_ui->setupUi(this);
  ...
  m_ui->likelihoodMethod->addItem(
      "lmLikelihoodField_Thrun", COccupancyGridMap2D::lmLikelihoodField_Thrun);
  m_ui->likelihoodMethod->addItem(
...
{
  auto* mapDefination = dynamic_cast<COccupancyGridMap2D::TMapDefinition*>(options);
