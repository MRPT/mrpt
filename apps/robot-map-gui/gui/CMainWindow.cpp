    ...
    QMainWindow(parent), m_ui(std::make_unique<Ui::CMainWindow>())
{
  ...
  connect(m_ui->m_configWidget, &CConfigWidget::openedConfig, this, &CMainWindow::openConfig);
  connect(
      m_ui->m_configWidget, &CConfigWidget::applyConfigurationForCurrentMaps, this,
      &CMainWindow::applyConfigurationForCurrentMaps);
  connect(m_ui->m_configWidget, &CConfigWidget::addedMap, this, &CMainWindow::updateConfig);
  connect(m_ui->m_configWidget, &CConfigWidget::removedMap, this, &CMainWindow::updateConfig);
  connect(m_ui->m_configWidget, &CConfigWidget::updatedConfig, this, &CMainWindow::updateConfig);

  connect(m_ui->m_observationsTree, &CObservationTree::clicked, this, &CMainWindow::itemClicked);
  connect(m_ui->m_actionAbout, &QAction::triggered, this, &CMainWindow::about);
  connect(m_ui->m_actionOpen, &QAction::triggered, this, &CMainWindow::openMap);
  connect(m_ui->m_actionSave, &QAction::triggered, this, &CMainWindow::saveMap);
  connect(m_ui->m_actionSaveAsText, &QAction::triggered, this, &CMainWindow::saveAsText);
  connect(m_ui->m_actionSaveAsPNG, &QAction::triggered, this, &CMainWindow::saveAsPNG);
  ...
  connect(
      m_ui->m_poseDirection, &CPoseDirection::updateDirection, this, &CMainWindow::updateDirection);
  ...
  connect(m_ui->m_viewer, &CViewerContainer::selectedChanged, this, &CMainWindow::selectedChanged);
  connect(
      m_ui->m_viewer, &CViewerContainer::deleteRobotPoses, this, &CMainWindow::deleteRobotPoses);
  connect(m_ui->m_viewer, &CViewerContainer::moveRobotPoses, this, &CMainWindow::moveRobotPoses);
  connect(m_ui->m_viewer, &CViewerContainer::selectedChanged, this, &CMainWindow::selectedChanged);
  connect(
      m_ui->m_viewer, &CViewerContainer::showPoseDirection, this, &CMainWindow::showPoseDirection);
...
void CMainWindow::about() { mrpt::gui::show_mrpt_about_box_Qt("Robot-made maps viewer"); }
...
void CMainWindow::moveRobotPosesOnMap(const std::vector<size_t>& idx, const QPointF& dist)
{
  ...
  for (auto& poseSf : posesObsPairs)
  {
    ...
    pose.setFromValues(pose[0], pose[1], pose[2], pose.yaw(), pose.pitch(), pose.roll());
    posePDF->changeCoordinatesReference(mrpt::poses::CPose3D(dist.x(), dist.y(), 0.0));
  }
...
void CMainWindow::updateDirection(size_t index, double yaw, double pitch, double roll)
{
  ...
  pose.setFromValues(pose[0], pose[1], pose[2], DEG2RAD(yaw), DEG2RAD(pitch), DEG2RAD(roll));
  auto newPosePDF = mrpt::poses::CPose3DPDFGaussian::Create(pose, posePDF->getCovariance());
...
void CMainWindow::showPoseDirection(size_t idx, double yaw, double pitch, double roll)
{
  m_ui->m_dockWidgetDirection->setVisible(true);
  m_ui->m_poseDirection->setIndex(idx);
  m_ui->m_poseDirection->setDirection(RAD2DEG(yaw), RAD2DEG(pitch), RAD2DEG(roll));
}

void CMainWindow::hidePoseDirection() { m_ui->m_dockWidgetDirection->setVisible(false); }
...
void CMainWindow::updateRenderMapFromConfig()
{
  ...
  {
    QString name =
        QString::fromStdString(typeToName(it.first.type)) + QString::number(it.first.index);

    auto action = m_ui->m_saveMetricMapRepresentation->addAction(name);
    connect(action, &QAction::triggered, this, &CMainWindow::saveMetricMapRepresentation);

    auto actionBinary = m_ui->m_saveMetricmapInBinaryFormat->addAction(name);
    connect(actionBinary, &QAction::triggered, this, &CMainWindow::saveMetricmapInBinaryFormat);
  }
}
