    ...
    const mrpt::config::CConfigFileBase& cfg, const std::string& sSection)
{
  ...
  mrpt::keep_max(
      m_clearance_num_points, static_cast<decltype(m_clearance_num_points)>(refDistance / 1.0));
  ...
  MRPT_LOAD_HERE_CONFIG_VAR(
      target_rel_speed, double, m_nav_dyn_state.targetRelSpeed, cfg, sSection);
}
    ...
    mrpt::config::CConfigFileBase& cfg, const std::string& sSection) const
{
  ...
  cfg.write(
      ...
      "(Default=5).");
  cfg.write(
      sSection, "clearance_decimated_paths", m_clearance_decimated_paths, WN, WV,
  ...
  cfg.write(
      sSection, "wi", mrpt::RAD2DEG(m_nav_dyn_state.curVelLocal.omega), WN, WV,
  ...
  cfg.write(
      sSection, "reltrg_phi", mrpt::RAD2DEG(m_nav_dyn_state.relTarget.phi), WN, WV,
    ...
    const double max_path_distance) const
{
  ...
  for (size_t n = 0; n < nPointsInPath; n++)
  {
    ...
    if (d < last_added_dist + decimate_distance && n != 0) continue;  // skip: decimation
...
bool CParameterizedTrajectoryGenerator::isInitialized() const { return m_is_initialized; }

void CParameterizedTrajectoryGenerator::updateNavDynamicState(
    const CParameterizedTrajectoryGenerator::TNavDynamicState& newState, const bool force_update)
{
...
void CParameterizedTrajectoryGenerator::internal_TPObsDistancePostprocess(
    const double ox, const double oy, const double new_tp_obs_dist, double& inout_tp_obs) const
{
  ...
  switch (getCollisionBehavior())
  {
    ...
    case PTGCollisionBehavior::BACK_AWAY:
    {
    ...
    bool treat_as_obstacle) const
{
  ...
  for (auto& e : inout_realdist2clearance)
  {
    ...
    const double this_clearance =
        treat_as_obstacle ? this->evalClearanceToRobotShape(ol.x, ol.y) : ol.norm();
