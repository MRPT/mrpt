...
TWaypoint::TWaypoint(
    ...
    bool allow_skip_,
    std::optional<double> target_heading_,
    double speed_ratio_) :
...
std::string TWaypoint::getAsText() const
{
  ...
  if (target_heading.has_value())
    s += mrpt::format("phi=%8.03f deg ", mrpt::RAD2DEG(target_heading.value()));
  else
    s += " (heading: any) ";
  ...
  for (const auto& p : waypoints)
  {
    ...
    if (p.target_heading.has_value())
    {
      auto o = mrpt::viz::CArrow::Create(
          ...
          mrpt::math::TPoint3Df(static_cast<float>(params.heading_arrow_len), 0, 0));
      o->setPose(
    ...
    for (const auto& p : waypoints)
    {
      ...
      gl_pt->setColor_u8(
          is_cur_goal ? params.color_current_goal
                      : (p.reached ? params.color_reached : params.color_regular));
      obj.insert(gl_pt);
...
void TWaypointSequence::load(const mrpt::config::CConfigFileBase& c, const std::string& s)
{
  this->clear();
  ...
  for (unsigned int i = 0; i < N; i++)
  {
    ...
    if (c.keyExists(s, sectHeading))
      wp.target_heading = c.read_double(s, sectHeading, mrpt::nav::TWaypoint::INVALID_NUM);
