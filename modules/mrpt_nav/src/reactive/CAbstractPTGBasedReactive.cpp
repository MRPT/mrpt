    ...
    const CAbstractNavigator::TNavigationParamsBase& rhs) const
{
  ...
  return o != nullptr && CWaypointsNavigator::TNavigationParamsWaypoints::isEqual(rhs) &&
...
void CAbstractPTGBasedReactive::performNavigationStep()
{
  ...
  // which PTGs are we using:
  {
    if (m_navLogger.isNewLogFile())  // Only the first time
    {
      ...
      for (size_t i = 0; i < nPTGs; i++)
      {
        ...
        arch << *this->getPTG(i);
        buf.Seek(0);
        newLogRec.infoPerPTG[i].ptg =
            std::dynamic_pointer_cast<mrpt::nav::CParameterizedTrajectoryGenerator>(
                arch.ReadObject());
      }
    }
  }
  ...
  try
  {
    ...
    const bool target_changed_since_last_iteration =
        (!m_copy_prev_navParams) || !(*m_copy_prev_navParams == *m_navigationParams);
    if (target_changed_since_last_iteration) m_copy_prev_navParams = m_navigationParams->clone();
    ...
    {
      auto p = dynamic_cast<const CWaypointsNavigator::TNavigationParamsWaypoints*>(
    ...
    initPTGs();  // Will only recompute if
    // "m_PTGsMustBeReInitialized==true"

    // Load the obstacles and sort them in height bands.
    // -----------------------------------------------------------------------------
    bool sense_ok;
    {
    ...
    mrpt::math::TPose2D rel_pose_PTG_origin_wrt_sense(0, 0, 0), relPoseSense(0, 0, 0),
        relPoseVelCmd(0, 0, 0);
    if (params_abstract_ptg_navigator.use_delays_model)
    {
       ...
       * timoff_pose2VelCmd
       */
      const double timoff_obstacles =
          mrpt::system::timeDifference(tim_start_iteration, m_WS_Obstacles_timestamp);
      ...
      const double timoff_curPoseVelAge =
          mrpt::system::timeDifference(tim_start_iteration, m_curPoseVel.timestamp);
      ...
      timoff_pose2VelCmd = timoff_sendVelCmd_avr.getLastOutput() +
                           0.5 * tim_changeSpeed_avr.getLastOutput() - timoff_curPoseVelAge;
    ...
    for (auto& t : targets)
    {
      if (t.target_frame_id != m_curPoseVel.pose_frame_id)
      {
        ...
        frame_tf->lookupTransform(
            t.target_frame_id, m_curPoseVel.pose_frame_id, robot_frame2map_frame);
        ...
        t.target_frame_id = m_curPoseVel.pose_frame_id;  // Now the coordinates are in
...

    {
      ...
      for (size_t i = 0; i < nPTGs; i++) getPTG(i)->updateNavDynamicState(ptg_dynState);
    }
    ...
    for (size_t indexPTG = 0; indexPTG < nPTGs; indexPTG++)
    {
      ...
      build_movement_candidate(
          ptg, indexPTG, relTargets, rel_pose_PTG_origin_wrt_sense, ipf, cm, newLogRec,
          false /* this is a regular PTG reactive case */, *holoMethod, tim_start_iteration,
          *m_navigationParams);
    }  // end for each PTG
    ...
    {
      m_pending_events.emplace_back(
          std::bind(&CRobot2NavInterface::sendApparentCollisionEvent, std::ref(m_robot)));
    }
    ...
    const bool can_do_nop_motion =
        (m_lastSentVelCmd.isValid() && !target_changed_since_last_iteration && last_sent_ptg &&
        ...
        (NOP_not_too_close_and_have_to_slowdown =
             (last_sent_ptg->supportSpeedAtTarget() ||
              (relTargetDist > (slowdowndist = m_holonomicMethod[m_lastSentVelCmd.ptg_index]
    ...
    {
      mrpt::system::CTimeLoggerEntry tle2(
          m_navProfiler, "CAbstractPTGBasedReactive::performNavigationStep().eval_NOP");

      // Add the estimation of how long it takes to run the changeSpeeds()
      // callback (usually a tiny period):
      const mrpt::system::TTimeStamp tim_send_cmd_vel_corrected = mrpt::system::timestampAdd(
          m_lastSentVelCmd.tim_send_cmd_vel, tim_changeSpeed_avr.getLastOutput());
      ...
      if (valid_odom && valid_pose)
      {
        ...
        if (fill_log_record)
        {
          newLogRec.additional_debug_msgs["rel_cur_pose_wrt_last_vel_cmd_NOP(interp)"] =
        ...
        build_movement_candidate(
            ...
            rel_pose_PTG_origin_wrt_sense_NOP, m_infoPerPTG[nPTGs], candidate_movs[nPTGs],
            newLogRec, true /* this is the PTG continuation (NOP) choice */,
            *getHoloMethod(m_lastSentVelCmd.ptg_index), tim_start_iteration, *m_navigationParams,
    ...
    ASSERT_(m_multiobjopt);
    const auto best_ptg_idx = m_multiobjopt->decide(candidate_movs, mo_info);
    ...
    const bool best_is_NOP_cmdvel = (best_ptg_idx.has_value() && *best_ptg_idx == nPTGs);

    // Convert to int for downstream code that still uses int indexing
    const int nSelectedPTG = best_ptg_idx.has_value() ? static_cast<int>(*best_ptg_idx) : -1;
    ...
    else
    {
      ...
      // Get the non-holonomic movement command.
      // ---------------------------------------------------------------------
      double cmd_vel_speed_ratio = 1.0;
      if (selectedHolonomicMovement)
      {
        CTimeLoggerEntry tle2(m_timelogger, "navigationStep.selectedHolonomicMovement");
      ...
      else
      {
        ...
        {
          mrpt::system::CTimeLoggerEntry tle2(m_timlog_delays, "changeSpeeds()");
        ...
        m_lastSentVelCmd.colfreedist_move_k =
            nSelectedPTG >= 0
                ? m_infoPerPTG[nSelectedPTG].TP_Obstacles[m_lastSentVelCmd.ptg_alpha_index]
                : .0;
        m_lastSentVelCmd.was_slowdown = (selectedHolonomicMovement->props.at("is_slowdown") != 0.0);
    ...
    const double period_tim = tNow - m_lastExecutionTimestamp;
    m_lastExecutionTimestamp = tNow;
    if (period_tim > 1.5 * meanExecutionPeriod.getLastOutput() &&
        period_tim < 2.0 /* Ignore long periods, likely nav pauses */
    )
    {
      MRPT_LOG_WARN_FMT(
          "Timing warning: Suspicious executionPeriod=%.03f ms is >150%% "
    ...
    if (m_enableConsoleOutput)
    {
      MRPT_LOG_DEBUG(mrpt::format(
          ...
          1000.0 * meanExecutionPeriod.getLastOutput(), 1000.0 * meanExecutionTime.getLastOutput(),
          1000.0 * meanTotalExecutionTime.getLastOutput(), nSelectedPTG));
    }
    if (fill_log_record)
    {
      generateLogRecord(
...
/** \callergraph */
void CAbstractPTGBasedReactive::generateLogRecord(
    CLogFileRecord& newLogRec,
    ...
    const mrpt::system::TTimeStamp& tim_start_iteration)
{
  ...
  // ---------------------------------------
  mrpt::system::CTimeLoggerEntry tle(
      m_navProfiler, "CAbstractPTGBasedReactive::generateLogRecord()");
    ...
    const mrpt::nav::CHolonomicLogFileRecord::Ptr& hlfr)
{
  MRPT_START
  mrpt::system::CTimeLoggerEntry tle(
      m_navProfiler, "CAbstractPTGBasedReactive::calc_move_candidate_scores()");
  ...
  size_t selected_trg_idx = 0;
  {
    ...
    {
      const double angdist =
          std::abs(mrpt::math::angDistance(TP_Targets[i].target_alpha, cm.direction));
  ...
  if (m_navigationParams && m_navigationParams->target.targetDesiredRelSpeed < 1.0 &&
  ...
  cm.props["holo_stage_eval"] = this_is_PTG_continuation ? m_lastSentVelCmd.original_holo_eval
  ...
  if (this_is_PTG_continuation)
  {
    ...
    else
    {
      ...
      const double NOP_At =
          ...
          mrpt::system::timeDifference(m_lastSentVelCmd.tim_send_cmd_vel, tim_start_iteration);
    ...
    if (!is_time_based)
    {
      ...
      if (ok1)
      {
        ...
        newLogRec.additional_debug_msgs["PTG_eval.PTGcont"] =
            mrpt::format("mismatchDistance=%.03f cm", 1e2 * predicted2real_dist);

        if (predicted2real_dist >
                params_abstract_ptg_navigator.max_distance_predicted_actual_path &&
    ...
    colfree = WS_point_is_unique ? std::min(in_TPObstacles[move_k], in_TPObstacles[cur_k])
  ...
  cm.props["dist_eucl_final"] = mrpt::hypot_fast(WS_Target.x - pose.x, WS_Target.y - pose.y);
...
bool CAbstractPTGBasedReactive::impl_waypoint_is_reachable(const mrpt::math::TPoint2D& wp) const
{
  ...
  if (m_infoPerPTG.size() < N || m_infoPerPTG_timestamp == INVALID_TIMESTAMP ||
      mrpt::system::timeDifference(m_infoPerPTG_timestamp, mrpt::Clock::now()) > 0.5)
  ...
  for (size_t i = 0; i < N; i++)
  {
    ...
    if (collision_free_dist > 1.01 * wp_norm_d) return true;  // free path found to target
  }
...
/** \callergraph */
bool CAbstractPTGBasedReactive::senseObstacles()
{
    ...
    const mrpt::math::TPose2D& rel_cur_pose_wrt_last_vel_cmd_NOP)
{
  ...
  bool use_this_ptg = true;
  {
    ...
    if (navpPTG && !navpPTG->restrict_PTG_indices.empty())
    {
      ...
      {
        if (navpPTG->restrict_PTG_indices[i] == indexPTG) use_this_ptg = true;
      }
    }
  }
  ...
  else
  {
    ...
    // -----------------------------------------------------------------------------
    {
      ...
      if (m_timelogger.isEnabled())
        m_timelogger.registerUserMeasure(
            "navigationStep.transformToTPSpace", timeForTPObsTransformation);
    }

    //  Holonomic navigation method
    // -----------------------------------------------------------------------------
    if (!this_is_PTG_continuation)
    {
      tictac.Tic();
      ...
      const auto no = holoMethod.navigate(ni);
      ...
      {
        const double v = mrpt::hypot_fast(m_curPoseVel.velLocal.vx, m_curPoseVel.velLocal.vy);
      ...
      if (obsFreeNormalizedDistance < params_abstract_ptg_navigator.secure_distance_end)
      {
        if (obsFreeNormalizedDistance <= params_abstract_ptg_navigator.secure_distance_start)
          ...
          velScale =
              (obsFreeNormalizedDistance - params_abstract_ptg_navigator.secure_distance_start) /
              (params_abstract_ptg_navigator.secure_distance_end -
               params_abstract_ptg_navigator.secure_distance_start);
      }
      ...
      if (m_timelogger.isEnabled())
        m_timelogger.registerUserMeasure("navigationStep.holonomicMethod", timeForHolonomicMethod);
    }
    else
    {
    ...
    // Evaluate each movement to assign them a "evaluation" value.
    // ---------------------------------------------------------------------
    {
  ...
  if (fill_log_record)
  {
    ...
    if (!this_is_PTG_continuation)
      ...
      ipp.PTG_desc = mrpt::format(
          "NOP cmdvel (prev PTG idx=%u)", static_cast<unsigned int>(m_lastSentVelCmd.ptg_index));

    mrpt::containers::copy_container_typecasting(ipf.TP_Obstacles, ipp.TP_Obstacles);
    ...
    mrpt::config::CConfigFileBase& c, const std::string& s) const
{
  ...
  {
    const auto lst =
        mrpt::rtti::getAllRegisteredClassesChildrenOf(CLASS_ID(CAbstractHolonomicReactiveMethod));
    for (const auto& cl : lst)
      lstHoloStr += string("# - `") + string(cl->className) + string("`\n");
  }
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      holonomic_method, string("C++ class name of the holonomic navigation method to run in "
  ...
  {
    const auto lst =
        mrpt::rtti::getAllRegisteredClassesChildrenOf(CLASS_ID(CMultiObjectiveMotionOptimizerBase));
    for (const auto& cl : lst)
      lstDecidersStr += string("# - `") + string(cl->className) + string("`\n");
  }
  MRPT_SAVE_CONFIG_VAR_COMMENT(
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      ...
      "allow it.");
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      enable_obstacle_filtering, "Enabled obstacle filtering (params in its own section)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      evaluate_clearance, "Enable exact computation of clearance (default=false)");
  MRPT_SAVE_CONFIG_VAR_COMMENT(
...
void CAbstractPTGBasedReactive::loadConfigFile(const mrpt::config::CConfigFileBase& c)
{
  MRPT_START
  m_PTGsMustBeReInitialized = true;
...
void CAbstractPTGBasedReactive::saveConfigFile(mrpt::config::CConfigFileBase& c) const
{
  ...
  else
  {
    ...
    for (const auto& cl : lst)
    {
      ...
      auto* holo = dynamic_cast<CAbstractHolonomicReactiveMethod*>(obj.get());
      if (holo)
      {
  ...
  else
  {
    ...
    {
      mrpt::rtti::CObject::Ptr obj = cl->createObject();
      auto* momo = dynamic_cast<CMultiObjectiveMotionOptimizerBase*>(obj.get());
      if (momo)
      {
