...
CAbstractNavigator::TargetInfo::TargetInfo() : target_coords(0, 0, 0), target_frame_id("map") {}
...
{
  return target_coords == o.target_coords && target_frame_id == o.target_frame_id &&
...
void CAbstractNavigator::resume()
{
  ...
  if (m_navigationState == TState::SUSPENDED) m_navigationState = TState::NAVIGATING;
}
...
void CAbstractNavigator::resetNavError()
{
  ...
  if (m_navigationState == TState::NAV_ERROR)
  {
    m_navigationState = TState::IDLE;
...
void CAbstractNavigator::loadConfigFile(const mrpt::config::CConfigFileBase& c)
{
  ...
  m_navProfiler.enable(
      c.read_bool("CAbstractNavigator", "enable_time_profiler", m_navProfiler.isEnabled()));
...
void CAbstractNavigator::navigationStep()
{
  ...
  mrpt::system::CTimeLoggerEntry tle(m_navProfiler, "CAbstractNavigator::navigationStep()");

  const TState prevState = m_navigationState;
  switch (m_navigationState)
  {
    case TState::IDLE:
    case TState::SUSPENDED:
      try
      {
        // If we just arrived at this state, stop robot:
        if (m_lastNavigationState == TState::NAVIGATING)
        {
          MRPT_LOG_INFO(
    ...
    case TState::NAV_ERROR:
      try
      {
        // Send end-of-navigation event:
        if (m_lastNavigationState == TState::NAVIGATING && m_navigationState == TState::NAV_ERROR)
        {
          m_pending_events.emplace_back(
              std::bind(&CRobot2NavInterface::sendNavigationEndDueToErrorEvent, std::ref(m_robot)));
        }
...
void CAbstractNavigator::processNavigateCommand(const TNavigationParams* params)
{
  ...
  ASSERT_(
      params->target.targetDesiredRelSpeed >= .0 && params->target.targetDesiredRelSpeed <= 1.0);
  ...
  // new state:
  m_navigationState = TState::NAVIGATING;
  m_navErrorReason = TErrorReason();
...
void CAbstractNavigator::updateCurrentPoseAndSpeeds()
{
  ...
  while (m_latestPoses.size() > 1 &&
         mrpt::system::timeDifference(m_latestPoses.begin()->first, m_latestPoses.rbegin()->first) >
  ...
  while (m_latestOdomPoses.size() > 1 &&
         mrpt::system::timeDifference(
             m_latestOdomPoses.begin()->first, m_latestOdomPoses.rbegin()->first) >
             PREVIOUS_POSES_MAX_AGE)
  {
    ...
    mrpt::config::CConfigFileBase& c, const std::string& s) const
{
  ...
  MRPT_SAVE_CONFIG_VAR_COMMENT(
      alarm_seems_not_approaching_target_timeout, "navigator timeout (seconds) [Default=30 sec]");
  MRPT_SAVE_CONFIG_VAR_COMMENT(
...
void CAbstractNavigator::performNavigationStepNavigating(bool call_virtual_nav_method)
{
  ...
  try
  {
    ...
    if (m_lastNavigationState == TState::IDLE)
    {
      m_pending_events.emplace_back(
          std::bind(&CRobot2NavInterface::sendNavigationStartEvent, std::ref(m_robot)));
    }
    ...
    const mrpt::math::TSegment2D seg_robot_mov = mrpt::math::TSegment2D(
        ...
        m_latestPoses.size() > 1 ? mrpt::math::TPoint2D((++m_latestPoses.rbegin())->second)
                                 : mrpt::math::TPoint2D((m_latestPoses.rbegin())->second));

    if (m_navigationParams)
    {
      const double targetDist =
          seg_robot_mov.distance(mrpt::math::TPoint2D(m_navigationParams->target.target_coords));

      // Should "End of navigation" event be sent??
      if (!m_navigationParams->target.targetIsIntermediaryWaypoint && !m_navigationEndEventSent &&
          targetDist < params_abstract_navigator.dist_to_target_for_sending_event)
      {
        m_navigationEndEventSent = true;
        m_pending_events.emplace_back(
            std::bind(&CRobot2NavInterface::sendNavigationEndEvent, std::ref(m_robot)));
      }
      ...
      if (checkHasReachedTarget(targetDist))
      {
        m_navigationState = TState::IDLE;
        logFmt(
            mrpt::system::LVL_WARN, "Navigation target (%.03f,%.03f) was reached\n",
      ...
      else
      {
        ...
        if (mrpt::system::timeDifference(m_badNavAlarm_lastMinDistTime, mrpt::Clock::now()) >
            params_abstract_navigator.alarm_seems_not_approaching_target_timeout)
        {
          ...
          m_pending_events.emplace_back(
              std::bind(&CRobot2NavInterface::sendWaySeemsBlockedEvent, std::ref(m_robot)));
          return;
        }
      }
        ...
        if (is_col)
        {
          ...
          if (send_event)
          {
            ...
            m_pending_events.emplace_back(std::bind(
                &CRobot2NavInterface::sendCannotGetCloserToBlockedTargetEvent, std::ref(m_robot)));
