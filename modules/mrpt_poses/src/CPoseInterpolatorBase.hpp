...
{
template <size_t DIM>
CPoseInterpolatorBase<DIM>::CPoseInterpolatorBase() : maxTimeInterpolation(std::chrono::seconds(-1))
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
template <size_t DIM>
typename CPoseInterpolatorBase<DIM>::cpose_t& CPoseInterpolatorBase<DIM>::interpolate(
    const mrpt::Clock::time_point& t, cpose_t& out_interp, bool& out_valid_interp) const
{
  pose_t p;
...
template <size_t DIM>
typename CPoseInterpolatorBase<DIM>::pose_t& CPoseInterpolatorBase<DIM>::interpolate(
    const mrpt::Clock::time_point& t, pose_t& out_interp, bool& out_valid_interp) const
{
  ...
  const mrpt::Clock::duration dt12 =
      interp_method_requires_4pts ? (p2.first - p1.first) : mrpt::Clock::duration(0);
  const mrpt::Clock::duration dt23 = (p3.first - p2.first);
  const mrpt::Clock::duration dt34 =
      interp_method_requires_4pts ? (p4.first - p3.first) : mrpt::Clock::duration(0);

  if (maxTimeInterpolation.count() > 0 &&
      (dt12 > maxTimeInterpolation || dt23 > maxTimeInterpolation || dt34 > maxTimeInterpolation))
  {
    out_valid_interp = false;
    return out_interp;
  }
...
bool CPoseInterpolatorBase<DIM>::saveToTextFile(const std::string& s) const
{
  try
  {
    ...
    for (auto i = m_path.begin(); i != m_path.end(); ++i)
    {
      ...
      for (unsigned int k = 0; k < p.size(); k++) str += mrpt::format("%.06f ", p[k]);
      str += std::string("\n");
...
bool CPoseInterpolatorBase<DIM>::saveToTextFile_TUM(const std::string& s) const
{
  try
  {
    ...
    {
      const double t = mrpt::Clock::toDouble(i->first);
      const auto p = mrpt::poses::CPose3DQuat(mrpt::poses::CPose3D(i->second));

      // TUM format: timestamp x y z q_x q_y q_z q_w
      str = mrpt::format("%.06f", t);
      constexpr std::array<int, 7> idxs = {0, 1, 2, 4, 5, 6, 3};
      for (unsigned int k = 0; k < 7; k++) str += mrpt::format(" %.06f", p[idxs[k]]);
      str += std::string("\n");
      f << str;
    }
    return true;
...
bool CPoseInterpolatorBase<DIM>::loadFromTextFile(const std::string& s)
{
  ...
  {
    for (unsigned int k = 0; k < pose_t::static_size; k++) p[k] = M(i, k + 1);
...
bool CPoseInterpolatorBase<DIM>::loadFromTextFile_TUM(const std::string& s)
{
  ...
  for (size_t i = 0; i < N; i++)
  {
    for (unsigned int k = 0; k < 7; k++) p[idxs[k]] = M(i, k + 1);
    insert(mrpt::Clock::fromDouble(M(i, 0)), pose_t(mrpt::poses::CPose3D(p).asTPose()));
  }
  return true;
  MRPT_END
}
