...
/*---------------------------------------------------------------
  Constructor
  ---------------------------------------------------------------*/
CPointPDFGaussian::CPointPDFGaussian(const CPoint3D& init_Mean, const CMatrixDouble33& init_Cov) :
...
/*---------------------------------------------------------------
  Constructor
  ---------------------------------------------------------------*/
CPointPDFGaussian::CPointPDFGaussian(const CPoint3D& init_Mean) : mean(init_Mean), cov()
{
  cov.setZero();
}
...
void CPointPDFGaussian::serializeTo(mrpt::serialization::CSchemeArchiveBase& out) const
{
  SCHEMA_SERIALIZE_DATATYPE_VERSION(1);
  out["mean"] = mean;
  out["cov"] = CMatrixD(cov);
}
void CPointPDFGaussian::serializeFrom(mrpt::serialization::CSchemeArchiveBase& in)
{
  int version = 0;
...
void CPointPDFGaussian::bayesianFusion(const CPointPDFGaussian& p1, const CPointPDFGaussian& p2)
{
  ...
  auto x = cov.asEigen() * (C1_inv.asEigen() * x1.asEigen() + C2_inv.asEigen() * x2.asEigen());
...
double CPointPDFGaussian::productIntegralWith(const CPointPDFGaussian& p) const
{
  ...
  const Eigen::Vector3d MU(mean.x() - p.mean.x(), mean.y() - p.mean.y(), mean.z() - p.mean.z());
...
double CPointPDFGaussian::productIntegralWith2D(const CPointPDFGaussian& p) const
{
  ...
  return std::pow(M_2PI, -0.5 * (state_length - 1)) * (1.0 / std::sqrt(C.det())) *
         exp(-0.5 * (MU.transpose() * C_inv.asEigen() * MU)(0, 0));
    ...
    [[maybe_unused]] const double minMahalanobisDistToDrop)
{
  ...
  const auto* p1 = dynamic_cast<const CPointPDFGaussian*>(&p1_);
  const auto* p2 = dynamic_cast<const CPointPDFGaussian*>(&p2_);

  const CMatrixDouble33 C1_inv = p1->cov.inverse_LLt();
  const CMatrixDouble33 C2_inv = p2->cov.inverse_LLt();
