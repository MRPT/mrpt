...
/*---------------------------------------------------------------
  Constructor
  ---------------------------------------------------------------*/
CPose3DPDFGaussianInf::CPose3DPDFGaussianInf(const CPose3D& init_Mean) : mean(init_Mean), cov_inv()
{
}
...
void CPose3DPDFGaussianInf::copyFrom(const CPosePDF& o)
{
  ...
  {  // cov is already inverted, but it's
     // a 2D pose:
...
bool CPose3DPDFGaussianInf::saveToTextFile(const string& file) const
{
  ...
  for (unsigned int i = 0; i < 6; i++)
    os::fprintf(
        f, "%e %e %e %e %e %e\n", cov_inv(i, 0), cov_inv(i, 1), cov_inv(i, 2), cov_inv(i, 3),
        cov_inv(i, 4), cov_inv(i, 5));
...
/*---------------------------------------------------------------
          drawSingleSample
 ---------------------------------------------------------------*/
void CPose3DPDFGaussianInf::drawSingleSample([[maybe_unused]] CPose3D& outPart) const
{
  MRPT_START
  ...
  outPart.setFromValues(
      mean.x() + v[0], mean.y() + v[1], mean.z() + v[2], mean.yaw() + v[3], mean.pitch() + v[4],
      mean.roll() + v[5]);
...
/*---------------------------------------------------------------
          drawManySamples
 ---------------------------------------------------------------*/
void CPose3DPDFGaussianInf::drawManySamples(size_t N, vector<CVectorDouble>& outSamples) const
{
  MRPT_START
...
void CPose3DPDFGaussianInf::enforceCovSymmetry()
{
  ...
  for (int i = 0; i < cov_inv.rows() - 1; i++)
    for (int j = i + 1; j < cov_inv.rows(); j++) cov_inv(i, j) = cov_inv(j, i);
}
...
void CPose3DPDFGaussianInf::printTo(std::ostream& out) const { out << *this; }
