...
void CPose2DInterpolator::serializeTo(mrpt::serialization::CArchive& out) const { out << m_path; }
    ...
    pose_t& out_interp) const
{
  ...
  switch (method)
  {
    ...
    case imLinear2Neig:
    {
      ...
      out_interp.phi =
          math::interpolate2points(td, ts[1], yaw[1], ts[2], yaw[2], true);  // Wrap 2pi
    }
    break;
    ...
    case imLinear4Neig:
    {
      ...
      out_interp.phi =
          math::leastSquareLinearFit<double, decltype(ts), 4>(td, ts, yaw, true);  // Wrap 2pi
    }
    break;
