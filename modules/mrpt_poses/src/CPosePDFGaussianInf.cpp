...
void CPosePDFGaussianInf::serializeTo(mrpt::serialization::CSchemeArchiveBase& out) const
{
  SCHEMA_SERIALIZE_DATATYPE_VERSION(1);
  out["mean"] = mean;
  out["cov_inv"] = CMatrixD(cov_inv);
}
void CPosePDFGaussianInf::serializeFrom(mrpt::serialization::CSchemeArchiveBase& in)
{
  int version = 0;
...
void CPosePDFGaussianInf::drawSingleSample(CPose2D& outPart) const
{
  ...
  MRPT_END_WITH_CLEAN_UP(cov_inv.saveToTextFile("__DEBUG_EXC_DUMP_drawSingleSample_COV_INV.txt"););
}

void CPosePDFGaussianInf::drawManySamples(size_t N, std::vector<CVectorDouble>& outSamples) const
{
  MRPT_START
...
void CPosePDFGaussianInf::inverse(CPosePDF& o) const
{
  ...
  out->cov_inv.asEigen().noalias() = (H.asEigen() * cov_inv.asEigen() * H.transpose()).eval();
}
...
void CPosePDFGaussianInf::printTo(std::ostream& out) const { out << *this; }
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
void CPosePDFGaussianInf::inverseComposition(
    const CPosePDFGaussianInf& x1, const CPosePDFGaussianInf& x0, const CMatrixDouble33& COV_01)
{
  // Use implementation in CPosePDFGaussian:
...
void CPosePDFGaussianInf::operator+=(const CPosePDFGaussianInf& Ap)
{
  ...
  this->cov_inv = (multiply_HCHt(df_dx, OLD_COV) + multiply_HCHt(df_du, Ap_cov)).inverse_LLt();
