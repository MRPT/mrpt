...
/*---------------------------------------------------------------
          Constructor
 ---------------------------------------------------------------*/
CConfigFile::CConfigFile(const std::string& fileName) : m_impl(mrpt::make_impl<CConfigFile::Impl>())
{
  MRPT_START
...
void CConfigFile::writeNow()
{
  ...
  if (m_modified && !m_file.empty())
  {
    ...
    {
      THROW_EXCEPTION_FMT(
          "SimpleIni error %i for file='%s'", static_cast<int>(err), m_file.c_str());
    }
    m_modified = false;
  }
  MRPT_END
