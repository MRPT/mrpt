...
void parse_process_var_define(
    ParseContext& pc, const std::string& var_name, const std::string& var_value);
...
std::string mci::parse_process_var_eval(const mci::ParseContext& pc, std::string expr)
{
...
void mci::parse_process_var_define(
    mci::ParseContext& pc, const std::string& var_name, const std::string& var_value)
{
  if (!var_name.empty())
  {
  ...
  while (i < in_len)
  {
    ...
    else
    {
      ...
      if (in_len > i + 4 && in_str[i] == '$' && in_str[i + 1] == '{')
      {
        ...
        if (!end_ok)
        {
          throw std::runtime_error(mrpt::format(
              "Line %u: Expected closing `}` near: `%s`", pc.line_count, varname.c_str()));
        }
      ...
      if (in_len > i + 7 && !strncmp(in_str + i, "$eval{", 6))
      {
        ...
        {
          throw std::runtime_error(mrpt::format(
              "Line %u: Expected closing `}` near: `%s`", pc.line_count, expr.c_str()));
        }
