    ...
    public mrpt::config::OptionsCapable
{
 ...
 protected:
  ...
  double computeObservationLikelihood_Consensus(
      const mrpt::obs::CObservation& obs, const mrpt::poses::CPose2D& takenFrom) const;
  /** One of the methods that can be selected for implementing
 ...
 public:
  ...
  bool saveAsBitmapFileWithLandmarks(
      ...
      const mrpt::img::TColor& marks_color = mrpt::img::TColor(0, 0, 255)) const
  {
    MRPT_START
    mrpt::img::CImage img;
    ...
    for (unsigned int i = 0; i < landmarks->landmarks.size(); i++)
    {
      ...
      if (addTextLabels)
        img.textOut({px, py - 8}, mrpt::format("%u", i), mrpt::img::TColor::black());
    }
  ...
  static COccupancyGridMap2D FromROSMapServerYAML(const std::string& yamlFilePath)
  {
    ...
    if (!grid.loadFromROSMapServerYAML(yamlFilePath))
      THROW_EXCEPTION_FMT("Error loading gridmap from '%s'", yamlFilePath.c_str());
    return grid;
  }
  ...
  {
    return mrpt::format(
        "2D gridmap, extending from (%f,%f) to (%f,%f), cell size=%f", getXMin(), getYMin(),
        getXMax(), getYMax(), getResolution());
  }
 ...
 private:
  ...
  float min_x{-10.0f}, max_x{10.0f}, min_y{-10.0f}, max_y{10.0f}, resolution{0.10f};
