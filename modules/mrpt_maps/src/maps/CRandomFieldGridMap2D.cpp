      ...
      for (size_t i = 0; i < m_cov.rows(); i++)
      {
        ...
        for (size_t j = i; j < m_cov.cols(); j++)
        {
          ...
          else
          {
            m_cov(i, j) =
                std0sqr *
                exp(-0.5 * (res2 * static_cast<double>(square(cx1 - cx2) + square(cy1 - cy2))) /
                    KF_covSigma2);
            m_cov(j, i) = m_cov(i, j);
          }
        }  // for j
      ...
      for (size_t i = 0; i < N; i++)
      {
        ...
        if (i == 0)
        {
          ...
          *ptr++ = std0sqr;
          ...
          Acy = 0;
          for (Acx = 1; Acx <= W; Acx++)
            *ptr++ = std0sqr * exp(-0.5 * (res2 * static_cast<double>(square(Acx) + square(Acy))) /
                                   KF_covSigma2);
          ...
          for (Acy = 1; Acy <= W; Acy++)
            for (Acx = -W; Acx <= W; Acx++)
              *ptr++ =
                  std0sqr * exp(-0.5 * (res2 * static_cast<double>(square(Acx) + square(Acy))) /
                                KF_covSigma2);
        }
        else
        {
    ...
    case mrGMRF_SD:
    {
      ...
      {  // Load Occupancy Gridmap and
        ...
        MRPT_LOG_DEBUG_FMT(
            ...
            static_cast<unsigned int>(m_Ocgridmap.getSizeX() * m_Ocgridmap.getSizeY()),
        ...
        MRPT_LOG_DEBUG_FMT(
            ...
            static_cast<unsigned int>(getSizeX()), static_cast<unsigned int>(getSizeY()));
      ...
      const size_t nPriorFactors =
          ...
          this->getSizeX() * (this->getSizeY() - 1);  // L = (Nr-1)*Nc + Nr*(Nc-1); Full connected
      ...
      if (!m_gmrf_connectivity && this->m_insertOptions_common->GMRF_use_occupancy_information)
      {
        ...
        std::multimap<size_t, size_t> cell_interconnections;  // Store the interconnections
        ...
        for (size_t j = 0; j < nodeCount; j++)  // For each cell in the map
        {
          ...
          for (int neighbor = 0; neighbor < 2; neighbor++)
          {
            ...
            if (exist_relation_between2cells(
                    &m_Ocgridmap, cxo_min, cxo_max, cyo_min, cyo_max, seed_cxo, seed_cyo,
          ...
          for (int dir = 0; dir < 2; dir++)
          {
            ...
            if (custom_connectivity != nullptr)
            {
              const bool is_connected = custom_connectivity->getEdgeInformation(
                  this, cx, cy, cx + (dir == 0 ? 1 : 0), cy + (dir == 1 ? 1 : 0), edge_lamdba);
    ...
    double normReading, const mrpt::math::TPoint2D& point, bool is_DMV)
{
  ...
  double minWinValueAtCutOff =
      exp(-square(m_insertOptions_common->cutoffRadius / m_insertOptions_common->sigma));
      ...
      m_DM_gaussWindow.size() != square(Ac_all))
  {
    ...
    for (unsigned cx = 0; cx < Ac_all; cx++)
    {
      ...
      {
        dist = m_resolution *
               sqrt(static_cast<double>(square(Ac_cutoff + 1 - cx) + square(Ac_cutoff + 1 - cy)));
        *(it++) = static_cast<float>(std::exp(-square(dist / std)));
      }
    }
...
CRandomFieldGridMap2D::TInsertionOptionsCommon::TInsertionOptionsCommon() :
    cutoffRadius(static_cast<float>(sigma) * 3.0f),
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
void CRandomFieldGridMap2D::TInsertionOptionsCommon::internal_dumpToTextStream_common(
    std::ostream& out) const
{
  ...
  out << mrpt::format("dm_sigma_omega                          = %f\n", dm_sigma_omega);
  ...
  out << mrpt::format("GMRF_lambdaObsLoss                      = %f\n", GMRF_lambdaObs);
  ...
  out << mrpt::format("GMRF_gridmap_image_res                  = %f\n", GMRF_gridmap_image_res);
  out << mrpt::format(
    ...
    const mrpt::config::CConfigFileBase& iniFile, const std::string& section)
{
  ...
  KF_observationModelNoise =
      iniFile.read_float(section.c_str(), "KF_observationModelNoise", KF_observationModelNoise);
  KF_defaultCellMeanValue =
...
void CRandomFieldGridMap2D::getAsMatrix(mrpt::math::CMatrixDouble& cells_mat) const
{
  ...
  for (unsigned int y = 0; y < m_size_y; y++)
  {
    for (unsigned int x = 0; x < m_size_x; x++)
    {
      ...
      mrpt::saturate(
          c, m_insertOptions_common->GMRF_saturate_min, m_insertOptions_common->GMRF_saturate_max);
    ...
    double additionalMarginMeters)
{
  ...
  CDynamicGrid<TRandomFieldCell>::resize(
      new_x_min, new_x_max, new_y_min, new_y_max, defaultValueNewCells, additionalMarginMeters);
  ...
  if (m_size_x != old_sizeX || m_size_y != old_sizeY)
  {
        ...
        if (C1_isFromOldMap)
        {
          for (j = i; j < N; j++)
          {
            ...
            if (C1_isFromOldMap && C2_isFromOldMap)
            {
              ...
              MRPT_END_WITH_CLEAN_UP(printf("cx1=%u\n", static_cast<unsigned>(cx1));
              ...
              if (i == j && m_cov(i, i) < 0)
              {
                printf(
                    ...
                    "\nAcx_left=%u \nAcy_bottom=%u "
                    "\nold_sizeX=%u \n",
                    static_cast<unsigned>(i), static_cast<unsigned>(j), static_cast<unsigned>(cx1),
                    static_cast<unsigned>(cy1), static_cast<unsigned>(cx2),
                    static_cast<unsigned>(cy2), static_cast<unsigned>(idx_c1),
                    static_cast<unsigned>(idx_c2), static_cast<unsigned>(Acx_left),
                    static_cast<unsigned>(Acy_bottom), static_cast<unsigned>(old_sizeX));
              }
            }
      ...
      for (i = 0; i < N; i++)
      {
        ...
        for (j = i; j < N; j++)
        {
          ...
          if (!C1_isFromOldMap || !C2_isFromOldMap)
          {
            ...
            {
              dist =
                  m_resolution * sqrt(static_cast<double>(square(cx1 - cx2) + square(cy1 - cy2)));
    ...
    else if (m_mapType == mrKalmanApproximate)
    {
      ...
      MRPT_LOG_DEBUG_FMT(
          ...
          static_cast<unsigned>(old_sizeY), static_cast<unsigned>(m_size_x),
          static_cast<unsigned>(m_size_y), static_cast<unsigned>(m_size_x * m_size_y));
      ...
      for (size_t i = N - 1; i < N; i--)
      {
        ...
        const int old_idx_of_i = static_cast<int>((cx - Acx_left) + (cy - Acy_bottom) * old_sizeX);
    ...
    double normReading, const mrpt::math::TPoint2D& point)
{
  ...
  double sk =
      ...
      square(m_insertOptions_common->KF_observationModelNoise);  // Predicted observation variance
  double sk_1 = 1.0 / sk;
  ...
  for (i = 0; i < N; i++)
  {
    ...
    for (j = i; j < N; j++)
    {
      ...
      if (i == j)
      {
        ...
        {
          printf(
              "Wrong insertion in KF! m_cov(%u,%u) = %.5f", static_cast<unsigned int>(i),
    ...
    const std::string& filNamePrefix) const
{
  ...
  DIMs.saveToTextFile(
      ...
      false /* add mrpt header */, "% Grid limits: [x_min x_max y_min y_max]\n");
  ...
  switch (m_mapType)
  {
    ...
    case mrKernelDMV:
    {
      ...
      if (m_mapType == mrKernelDMV)
      {
        ...
        all_confs.saveToTextFile(
            filNamePrefix + std::string("_confidence.txt"), MATRIX_FORMAT_FIXED);
      }
    }
    break;
    ...
    case mrKalmanApproximate:
    {
      ...
      for (size_t i = 0; i < m_size_y; i++)
        ...
        {
          MEAN(static_cast<int>(i), static_cast<int>(j)) = static_cast<float>(
              cellByIndex(static_cast<unsigned>(j), static_cast<unsigned>(i))->kf_mean());
          STDs(static_cast<int>(i), static_cast<int>(j)) = static_cast<float>(
      ...
      if (m_mapType == mrKalmanApproximate)
      {
        m_stackedCov.saveToTextFile(
            filNamePrefix + std::string("_mean_compressed_cov.txt"), MATRIX_FORMAT_FIXED);
      }
      if (m_mapType == mrKalmanFilter)
      {
      ...
      bool savedOk = img_cov.saveToFile(
          filNamePrefix + std::string("_cells_std.png"), true /* vertical flip */);
      ASSERT_(savedOk);
    ...
    case mrGMRF_SD:
    {
      ...
      for (size_t i = 0; i < m_size_y; ++i)
      {
        for (size_t j = 0; j < m_size_x; ++j, ++idx)
        {
          ...
          XYZ(static_cast<int>(idx), 0) = static_cast<float>(idx2x(static_cast<int>(j)));
      ...
      STDs.saveToTextFile(filNamePrefix + std::string("_cells_std.txt"), MATRIX_FORMAT_FIXED);
      XYZ.saveToTextFile(
          filNamePrefix + std::string("_xyz_and_std.txt"), MATRIX_FORMAT_FIXED, false,
...
void CRandomFieldGridMap2D::saveAsMatlab3DGraph(const std::string& filName) const
{
  ...
  ASSERT_(
      m_mapType == mrKalmanFilter || m_mapType == mrKalmanApproximate || m_mapType == mrGMRF_SD);
    ...
    case mrGMRF_SD:
    {
      ...
      for (cy = 1; cy < m_size_y; cy++)
      {
        for (cx = 1; cx < m_size_x; cx++)
        {
          ...
          triag.vertices[0].setColor(colormap(cmJET, static_cast<float>(col_xy)).asTColor());
          triag.vertices[1].setColor(colormap(cmJET, static_cast<float>(col_xy_1)).asTColor());
          triag.vertices[2].setColor(colormap(cmJET, static_cast<float>(col_x_1y_1)).asTColor());
          ...
          triag.vertices[0].setColor(colormap(cmJET, static_cast<float>(col_xy)).asTColor());
          triag.vertices[1].setColor(colormap(cmJET, static_cast<float>(col_x_1y_1)).asTColor());
          triag.vertices[2].setColor(colormap(cmJET, static_cast<float>(col_x_1y)).asTColor());
          obj_m->insertTriangle(triag);
          ...
          triag.x(0) = static_cast<float>(xs[cx]);
          triag.y(0) = static_cast<float>(ys[cy]);
          triag.z(0) = static_cast<float>(c_xy + v_xy);
          triag.x(1) = static_cast<float>(xs[cx]);
          triag.y(1) = static_cast<float>(ys[cy - 1]);
          triag.z(1) = static_cast<float>(c_xy_1 + v_xy_1);
          triag.x(2) = static_cast<float>(xs[cx - 1]);
          triag.y(2) = static_cast<float>(ys[cy - 1]);
          triag.z(2) = static_cast<float>(c_x_1y_1 + v_x_1y_1);
          ...
          triag.x(1) = static_cast<float>(xs[cx - 1]);
          triag.y(1) = static_cast<float>(ys[cy - 1]);
          triag.z(1) = static_cast<float>(c_x_1y_1 + v_x_1y_1);
          triag.x(2) = static_cast<float>(xs[cx - 1]);
          triag.y(2) = static_cast<float>(ys[cy]);
          triag.z(2) = static_cast<float>(c_x_1y + v_x_1y);
    ...
    case mrKernelDMV:
    {
      ...
      for (cy = 1; cy < m_size_y; cy++)
      {
        for (cx = 1; cx < m_size_x; cx++)
        {
          ...
          //-----------------
          double c_xy = mrpt::saturate_val(
              computeMeanCellValue_DM_DMV(cell_xy), m_insertOptions_common->GMRF_saturate_min,
              m_insertOptions_common->GMRF_saturate_max);
          double c_x_1y = mrpt::saturate_val(
              computeMeanCellValue_DM_DMV(cell_x_1y), m_insertOptions_common->GMRF_saturate_min,
              m_insertOptions_common->GMRF_saturate_max);
          double c_xy_1 = mrpt::saturate_val(
              computeMeanCellValue_DM_DMV(cell_xy_1), m_insertOptions_common->GMRF_saturate_min,
              m_insertOptions_common->GMRF_saturate_max);
          double c_x_1y_1 = mrpt::saturate_val(
              computeMeanCellValue_DM_DMV(cell_x_1y_1), m_insertOptions_common->GMRF_saturate_min,
              m_insertOptions_common->GMRF_saturate_max);
          ...
          triag.x(0) = static_cast<float>(xs[cx]);
          triag.y(0) = static_cast<float>(ys[cy]);
          triag.z(0) = static_cast<float>(v_xy);
          triag.x(1) = static_cast<float>(xs[cx]);
          triag.y(1) = static_cast<float>(ys[cy - 1]);
          triag.z(1) = static_cast<float>(v_xy_1);
          triag.x(2) = static_cast<float>(xs[cx - 1]);
          triag.y(2) = static_cast<float>(ys[cy - 1]);
          triag.z(2) = static_cast<float>(v_x_1y_1);
          ...
          triag.x(1) = static_cast<float>(xs[cx - 1]);
          triag.y(1) = static_cast<float>(ys[cy - 1]);
          triag.z(1) = static_cast<float>(v_x_1y_1);
          triag.x(2) = static_cast<float>(xs[cx - 1]);
          triag.y(2) = static_cast<float>(ys[cy]);
          triag.z(2) = static_cast<float>(v_x_1y);
...
double CRandomFieldGridMap2D::computeMeanCellValue_DM_DMV(const TRandomFieldCell* cell) const
{
  // A confidence measure:
  const double alpha =
      1.0 - std::exp(-square(cell->dm_mean_w() / m_insertOptions_common->dm_sigma_omega));
    ...
    const TGridInterpolationMethod interp_method)
{
  ...
  switch (interp_method)
  {
    ...
    case gimBilinear:
      if (x <= m_x_min + m_resolution * 0.5 || y <= m_y_min + m_resolution * 0.5 ||
          x >= m_x_max - m_resolution * 0.5 || x >= m_x_max - m_resolution * 0.5)
      {
  ...
  if (do_sensor_normalization)
    out_predict_response =
        ...
        out_predict_response * (m_insertOptions_common->R_max - m_insertOptions_common->R_min);
    ...
    double normReading, const mrpt::math::TPoint2D& point)
{
  ...
  resize(point.x - Aspace, point.x + Aspace, point.y - Aspace, point.y + Aspace, defCell, Aspace);
  ...
  for (int Acy = Acy0; Acy <= 0; Acy++)
  {
    ...
    int idx_c_in_idx = -Acy * static_cast<int>(W21) - Acx0;

    int window_idx = Acx0 + W + (Acy + W) * static_cast<int>(W21);

    for (int Acx = Acx0; Acx <= limit_cx; Acx++)
    {
      ASSERT_(idx_c_in_idx > 0);
      const double cov_i_c = m_stackedCov(static_cast<int>(idx), idx_c_in_idx);
      ...
      window_idxs[window_idx++] = static_cast<int>(idx);
  ...
  for (int Acy = 0; Acy <= Acy1; Acy++)
  {
    ...
    int idx_i_in_c;
    if (Acy > 0)
      idx_i_in_c = (W + 1) + (Acy - 1) * static_cast<int>(W21) + (start_cx + W);  // Rest of rows
    else
      idx_i_in_c = 0;  // First row.

    int window_idx = start_cx + W + (Acy + W) * static_cast<int>(W21);
  ...
  for (size_t i = 0; i < W21sqr; i++)
  {
    ...
    for (size_t j = i; j < W21sqr; j++)
    {
      ...
      if (Ax > W) continue;  // Next cell (may be more rows after the current
      // one...)
      ...
      int idx_j_in_i;
      if (Ay > 0)
        idx_j_in_i = Ax + W + (Ay - 1) * static_cast<int>(W21) + W + 1;
      else
...
void CRandomFieldGridMap2D::recoverMeanAndCov() const
{
  ...
  const size_t N = m_map.size();
  for (size_t i = 0; i < N; i++) m_map_castaway_const()[i].kf_std() = sqrt(m_stackedCov(i, 0));
}
...
CRandomFieldGridMap2D::TMapRepresentation CRandomFieldGridMap2D::getMapType() { return m_mapType; }
  ...
  switch (m_mapType)
  {
    ...
    case mrGMRF_SD:
      insertObservation_GMRF(
          ...
          reading_stddev == .0 ? m_insertOptions_common->GMRF_lambdaObs  // default information
...
void CRandomFieldGridMap2D::updateMapEstimation_GMRF()
{
  ...
  m_gmrf.updateEstimation(x_incr, m_insertOptions_common->GMRF_skip_variance ? nullptr : &x_var);

  ASSERT_(size_t(m_map.size()) == size_t(x_incr.size()));
  ASSERT_(
      m_insertOptions_common->GMRF_skip_variance || size_t(m_map.size()) == size_t(x_var.size()));
    ...
    size_t objective_cyo)
{
  ...
  cxo_min = max(cxo_min, static_cast<size_t>(0));
  cxo_max = min(cxo_max, static_cast<size_t>(m_Ocgridmap->getSizeX()) - 1);
  cyo_min = max(cyo_min, static_cast<size_t>(0));
  cyo_max = min(cyo_max, static_cast<size_t>(m_Ocgridmap->getSizeY()) - 1);
...
double CRandomFieldGridMap2D::TObservationGMRF::getInformation() const { return this->Lambda; }
void CRandomFieldGridMap2D::TObservationGMRF::evalJacobian(double& dr_dx) const { dr_dx = 1.0; }
// ============ TPriorFactorGMRF ===========
...
double CRandomFieldGridMap2D::TPriorFactorGMRF::getInformation() const { return this->Lambda; }
