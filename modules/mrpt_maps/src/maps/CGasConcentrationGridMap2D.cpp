...
MAP_DEFINITION_REGISTER(
    "mrpt::maps::CGasConcentrationGridMap2D,gasGrid", mrpt::maps::CGasConcentrationGridMap2D)

CGasConcentrationGridMap2D::TMapDefinition::TMapDefinition()

    = default;

void CGasConcentrationGridMap2D::TMapDefinition::loadFromConfigFile_map_specific(
    ...
    std::ostream& out) const
{
  out << mrpt::format(
      "MAP TYPE                                  = %s\n",
      mrpt::typemeta::TEnumType<CGasConcentrationGridMap2D::TMapRepresentation>::value2name(mapType)
          .c_str());
...
mrpt::maps::CMetricMap::Ptr CGasConcentrationGridMap2D::internal_CreateFromMapDefinition(
...
CGasConcentrationGridMap2D::CGasConcentrationGridMap2D(
    ...
    float y_max,
    float resolution) :
    CRandomFieldGridMap2D(mapType, x_min, x_max, y_min, y_max, resolution), insertionOptions()
{
  // Override defaults:
    ...
    if (o.sensorLabel.compare(insertionOptions.gasSensorLabel) == 0)
    {
      ...
      if (o.sensorLabel.compare("MCEnose") == 0 || o.sensorLabel.compare("Full_MCEnose") == 0)
      {
        ...
        else
        {
          ...
          {
            if (it->sensorTypes.at(i) == int(insertionOptions.gasSensorType)) break;
          }
          ...
          {
            std::cout << "Sensor especified not found, compute default "
                         "mean value"
                      << "\n";
            sensorReading = d2f(math::mean(it->readingsVoltage));
          }
...
double CGasConcentrationGridMap2D::internal_computeObservationLikelihood(
    [[maybe_unused]] const CObservation& obs, [[maybe_unused]] const CPose3D& takenFrom) const
{
...
// Save the "m_map": This requires special handling for big endian systems:
  ...
  out << insertionOptions.sigma << insertionOptions.cutoffRadius << insertionOptions.R_min
      << insertionOptions.R_max << insertionOptions.KF_covSigma
      << insertionOptions.KF_initialCellStd << insertionOptions.KF_observationModelNoise
      << insertionOptions.KF_defaultCellMeanValue << insertionOptions.KF_W_size;
...
// Read the note in writeToStream()
      ...
      if (version >= 1)
      {
        ...
        in >> insertionOptions.sigma >> insertionOptions.cutoffRadius >> insertionOptions.R_min >>
            insertionOptions.R_max >> insertionOptions.KF_covSigma >>
            insertionOptions.KF_initialCellStd >> insertionOptions.KF_observationModelNoise >>
            insertionOptions.KF_defaultCellMeanValue >> insertionOptions.KF_W_size;
      }
...
void CGasConcentrationGridMap2D::TInsertionOptions::dumpToTextStream(std::ostream& out) const
{
  ...
  out << mrpt::format("default_wind_direction					= %f\n", default_wind_direction);
  out << mrpt::format("default_wind_speed						= %f\n", default_wind_speed);
  out << mrpt::format("std_windNoise_phi						= %f\n", std_windNoise_phi);
  out << mrpt::format("std_windNoise_mod						= %f\n", std_windNoise_mod);
...
void CGasConcentrationGridMap2D::getWindAs3DObject(mrpt::viz::CSetOfObjects::Ptr& windObj) const
{
  ...
  {
    std::cout << " GAS MAP DIMENSIONS DO NOT MATCH WIND MAP "
              << "\n";
    // mrpt::system::pause();
  }
...
bool CGasConcentrationGridMap2D::simulateAdvection(double STD_increase_value)
{
  ...
  try
  {
    ...
    if (N != wind_map_size)
    {
      std::cout << " GAS MAP DIMENSIONS DO NOT MATCH WIND INFORMATION "
                << "\n";
      // mrpt::system::pause();
    }
    ...
    std::cout << " - SA matrix computed in " << tictac.Tac() << "s" << endl << "\n";
  }
  catch (const std::exception& e)
  {
    std::cout << " #########  EXCEPTION computing Transition Matrix (A) "
                 "##########\n: "
              << e.what() << "\n";
    std::cout << "on cell i= " << i << "  c=" << c << endl << "\n";
    return false;
  }
  ...
  try
  {
        ...
        if (A(it_i, it_j) != 0)
        {
          if (row_sum[it_i] >= 1)
            new_variances[it_i] +=
                ...
                (m_stackedCov(it_j, 0) + square(m_map[it_j].kf_mean() - new_means[it_i]));
          else
            new_variances[it_i] +=
                A(it_i, it_j) *
                (m_stackedCov(it_j, 0) + square(m_map[it_j].kf_mean() - new_means[it_i]));
    ...
    std::cout << " - Mean&Var updated in " << tictac.Tac() << "s"
              << "\n";
  ...
  {
    std::cout << " #########  EXCEPTION Updating Covariances ##########\n: " << e.what() << "\n";
    std::cout << "on row i= " << i << "  column c=" << c << endl << "\n";
    return false;
  }

  // std::cout << " Increasing general STD..." << "\n";
...

{
  std::cout << endl
            << "---------------------------------"
            << "\n";
  std::cout << " BUILDING GAUSSIAN WIND WEIGHTS "
            << "\n";
  std::cout << "---------------------------------" << endl << "\n";
  ...
  LUT.table = new vector<vector<vector<TGaussianCell>>>(
      LUT.phi_count, vector<vector<TGaussianCell>>(LUT.r_count, vector<TGaussianCell>()));
  ...
  std::cout << "Looking for file: " << filename.c_str() << "\n";

  if (mrpt::system::fileExists(filename.c_str()))
  {
    // file exists. Load lookUptable from file
    std::cout << "LookUp table found for this configuration. Loading..."
              << "\n";
  ...
  else
  {
    ...
    std::cout << "LookUp table NOT found. Generating table..."
              << "\n";
          ...
          for (it = w_values.begin(); it != w_values.end(); it++)
          {
            ...
            if (w_final >= 0.001)
            {
              ...
              if (debug)
              {
                // Save to file (debug)
                fprintf(
                    debug_file, "(%d,%d)=%.6f    ", gauss_info.cx, gauss_info.cy, gauss_info.value);
              }
            }
          }
...
bool CGasConcentrationGridMap2D::save_Gaussian_Wind_Grid_To_File()
{
  ...
  {
    return false;
    std::cout << "WARNING: Gaussian_Wind_Weights file NOT SAVED"
              << "\n";
  }
  ...
  catch (const std::exception& e)
  {
    std::cout << endl
              << "------------------------------------------------------------"
              << "\n";
    std::cout << "EXCEPTION WHILE SAVING LUT TO FILE"
              << "\n";
...
bool CGasConcentrationGridMap2D::load_Gaussian_Wind_Grid_From_File()
{
  ...
  try
  {
    ...
    {
      std::cout << "WARNING WHILE READING FROM: Gaussian_Wind_Weights"
                << "\n";
      return false;
    }
  ...
  catch (const std::exception& e)
  {
    std::cout << endl
              << "------------------------------------------------------------"
              << "\n";
    std::cout << "EXCEPTION WHILE LOADING LUT FROM FILE"
              << "\n";
    std::cout << "Exception = " << e.what() << "\n";
    return false;
  }
