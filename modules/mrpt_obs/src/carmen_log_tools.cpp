    ...
    const mrpt::system::TTimeStamp& time_start_log)
{
  ...
  if (strStartsI(line, "ROBOTLASER"))
  {
    ...
    if (!(S >> obsLaser->sensorLabel >> laser_type >> start_angle >> obsLaser->aperture >>
          angular_resolution >> obsLaser->maxRange >> accuracy >> remission_mode))
    ...
    for (size_t i = 0; i < nRanges; i++)
    {
      ...
      // Valid value?
      obsLaser->setScanRangeValidity(i, !(range >= obsLaser->maxRange || range <= 0));
    }
    ...
    if (!(S >> globalLaserPose.x >> globalLaserPose.y >> globalLaserPose.phi >> globalRobotPose.x >>
          globalRobotPose.y >> globalRobotPose.phi))
  ...
  else if (strStartsI(line, "FLASER") || strStartsI(line, "RLASER"))
  {
    ...
    // Params:
    {
      ...
      if (line[0] == 'F')
      {  // front:
        maxRange = global_log_params.getOrDefault<double>("robot_front_laser_max", 81.0);
        resolutionDeg = global_log_params.getOrDefault<double>("laser_front_laser_resolution", 0.5);
      }
      else if (line[0] == 'R')
      {  // rear:
        maxRange = global_log_params.getOrDefault<double>("robot_rear_laser_max", 81.0);
        resolutionDeg = global_log_params.getOrDefault<double>("laser_rear_laser_resolution", 0.5);
      }
      obsLaser->maxRange = d2f(maxRange);
      obsLaser->aperture = d2f(DEG2RAD(resolutionDeg) * static_cast<double>(nRanges));
    }
  ...
  else if (strStartsI(line, "PARAM "))
  {
    ...
    if (!key.empty() && !val.empty()) global_log_params[lowerCase(key)] = val;
