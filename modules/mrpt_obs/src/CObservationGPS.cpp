        ...
        if (has_PZS_datum_)
        {
          ...
          if (version >= 6)
          {
            ...
            in.ReadBufferFixEndianness(
                &PZS_datum.pos_covariance(0, 0), PZS_datum.pos_covariance.size());
            MRPT_READ_POD(in, PZS_datum.hasVelCov);
            in.ReadBufferFixEndianness(
                &PZS_datum.vel_covariance(0, 0), PZS_datum.vel_covariance.size());
          ...
          {
            PZS_datum.hasCartesianPosVel = PZS_datum.hasPosCov = PZS_datum.hasVelCov =
...
void CObservationGPS::dumpToConsole(std::ostream& o) const { this->dumpToStream(o); }
...
void CObservationGPS::getDescriptionAsText(std::ostream& o) const
{
  ...
  o << "  (as time_t): " << std::fixed << std::setprecision(5)
    << mrpt::Clock::toDouble(originalReceivedTimestamp) << "\n";
  o << "  (as TTimestamp): " << originalReceivedTimestamp << "\n";

  o << "Sensor position on the robot/vehicle: " << sensorPose << "\n";
