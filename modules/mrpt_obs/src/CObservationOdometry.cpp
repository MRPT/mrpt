...
void CObservationOdometry::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  switch (version)
  {
    ...
    case 2:
    {
      ...
      {
        in >> hasEncodersInfo;
        if (hasEncodersInfo || version < 2) in >> encoderLeftTicks >> encoderRightTicks;
...
std::string CObservationOdometry::exportTxtHeader() const
{
  return mrpt::format(
      ...
      ,
      "GLOBAL_ODO_X", "GLOBAL_ODO_Y", "GLOBAL_ODO_PHI_RAD", "HAS_ENCODERS", "LEFT_ENC_INCR_TICKS",
      "RIGHT_ENC_INCR_TICKS", "HAS_VELOCITIES", "VEL_VX_MetPerSec", "VEL_VY_MetPerSec",
      "ANG_SPEED_RadPerSec");
}
std::string CObservationOdometry::exportTxtDataRow() const
{
  return mrpt::format(
      ...
      ,
      odometry.x(), odometry.y(), odometry.phi(), static_cast<int>(hasEncodersInfo ? 1 : 0),
      static_cast<int>(encoderLeftTicks), static_cast<int>(encoderRightTicks),
      static_cast<int>(hasVelocities ? 1 : 0), velocityLocal.vx, velocityLocal.vy,
      velocityLocal.omega);
}
