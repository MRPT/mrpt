...
TEST(Vision, checkerBoardStereoCalibration)
{
  ...
  const auto dir = mrpt::mrpt_data_dir() + "/datasets/stereo-calib/"s;
  ...
  for (unsigned int i = 0; i < NUM_IMGS; i++) EXPECT_TRUE(out.image_pair_was_used[i]) << "i=" << i;
...
TEST(Vision, checkerBoardStereoCalibration_empty)
{
  ...
  const bool ok = mrpt::vision::checkerBoardStereoCalibration(images, params, out);
  EXPECT_FALSE(ok);
}
