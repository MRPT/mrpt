...
bool mrpt::vision::checkerBoardStereoCalibration(
    TCalibrationStereoImageList& images, const TStereoCalibParams& p, TStereoCalibResults& out)
{
  try
  {
    ...
    for (size_t i = 0; i < images.size(); i++)
    {
      ...
      if (corners_found[0] && corners_found[1])
      {
        ...
        if (Al.x * Ar.x + Al.y * Ar.y < 0)
        {
          ...
          // Invert all corners:
          std::reverse(
              images[i].right.detected_corners.begin(), images[i].right.detected_corners.end());
        }

        if (has_to_redraw_corners)
        {
          // Checkboad as color image:
          images[i].right.img_checkboard = images[i].right.img_original.colorImage();
    ...
    if (p.verbose) std::cout << valid_image_pair_indices.size() << " valid image pairs.\n";
    ...
    for (unsigned int y = 0; y < p.check_size_y; y++)
      for (unsigned int x = 0; x < p.check_size_x; x++)
        obj_points.emplace_back(
            p.check_squares_length_X_meters * x, p.check_squares_length_Y_meters * y, 0);
    ...
    for (int calibRound = 0; calibRound < 2; calibRound++)
    {
      ...
      while (iter < p.maxIters && !done)
      {
        ...
        Eigen::LLT<Eigen::MatrixXd> llt(HH.asEigen().selfadjointView<Eigen::Lower>());
        ...
        if (err_new < err)
        {
          ...
          if (numItersImproving++ > 5) use_robust = user_wants_use_robust;
    ...
    // R2L pose:
    out.right2left_camera_pose = mrpt::poses::CPose3D(lm_stat.right2left_pose);
    ...
    for (size_t i = 0; i < nUnknownsCamParams; i++)
    {
      ...
      out.right_params_inv_variance[vars_to_optimize[i]] =
          H(base_idx_H_CPs + nUnknownsCamParams + i, base_idx_H_CPs + nUnknownsCamParams + i);
    }
    ...
    for (unsigned long idx : valid_image_pair_indices)
    {
      ...
      // Camera poses:
      dat_l.reconstructed_camera_pose = -mrpt::poses::CPose3D(lm_stat.left_cam_poses[idx]);
      ...
      mrpt::img::camera_geometry::projectPoints_with_distortion(
          obj_points, out.cam_params.leftCamera, dat_l.reconstructed_camera_pose.asTPose(),
          dat_l.projectedPoints_distorted, true);
      mrpt::img::camera_geometry::projectPoints_with_distortion(
          obj_points, out.cam_params.rightCamera, dat_r.reconstructed_camera_pose.asTPose(),
    ...
    mrpt::math::CMatrixFixed<double, 2, 9>& Hc)
{
  ...
  // Hb = dh(b,c)/db  (2x2)
  Hb(0, 0) = fx * (k2 * r2 + k3 * r6 + 6 * t2 * x + 2 * t1 * y +
  ...
  Hb(1, 1) = fy * (k2 * r2 + k3 * r6 + 2 * t2 * x + 6 * t1 * y +
    ...
    mrpt::img::TPixelCoordf& px)
{
  ...
  px.x = params.cx() +
         params.fx() * (x * A + params.dist[2] * B + params.dist[3] * (r2 + 2 * square(x)));
  px.y = params.cy() +
         params.fy() * (y * A + params.dist[3] * B + params.dist[2] * (r2 + 2 * square(y)));
}
    ...
    mrpt::math::CMatrixDouble& H)
{
  ...
  // Reset to zeros:
  auto minus_g_tot = mrpt::math::CVectorDynamic<double>::Zero(nMaxUnknowns, 1);
  ...
  minus_g.asEigen().block(0, 0, nUnkPoses, 1) = minus_g_tot.asEigen().block(0, 0, nUnkPoses, 1);
  H.asEigen().block(0, 0, nUnkPoses, nUnkPoses) = H_tot.asEigen().block(0, 0, nUnkPoses, nUnkPoses);
    ...
    lm_stat_t& lm_stat)
{
  ...
  for (size_t i = 0; i < N; i++)
  {
    auto& cam_pose = lm_stat.left_cam_poses[lm_stat.valid_image_pair_indices[i]];
  ...
  // Increment of the right-left pose:
  {
    ...
    // new_pose =  old_pose  [+] delta
    //         = exp(delta) (+) old_pose
    lm_stat.right2left_pose = (incrPose + CPose3D(lm_stat.right2left_pose)).asTPose();
  }
...
void eval_h_b(const CVectorFixedDouble<2>& X, const TCamera& params, CVectorFixedDouble<2>& out)
{
  // Radial distortion:
...
void eval_b_p(const CVectorFixedDouble<3>& P, int dummy, CVectorFixedDouble<2>& b)
{
  // Radial distortion:
  b[0] = P[0] / P[2];
  b[1] = P[1] / P[2];
...
void eval_dA_eps_D_p(
    const CVectorFixedDouble<6>& eps, const TEvalData_A_eps_D_p& dat, CVectorFixedDouble<3>& out)
{
  const CVectorFixedDouble<6> incr(&eps[0]);
  const CPose3D incrPose = Lie::SE<3>::exp(incr);
    ...
    double kernel_param)
{
  ...
  for (size_t k = 0; k < N; k++)
  {
    ...
    for (size_t i = 0; i < nPts; i++)
    {
      ...
      TPose3D auxPose;
      lm_stat.right2left_pose.composePose(lm_stat.left_cam_poses[k_idx], auxPose);
...
// ---------------------------------------------------------------------------------
      ...
      const auto lcp = mrpt::poses::CPose3D(lm_stat.left_cam_poses[k_idx]);
...
// ---- end of theoretical Jacobians ----
      ...
      TNumJacobData dat(
          lm_stat, lm_stat.obj_points[i], lm_stat.left_cam_poses[k_idx], lm_stat.right2left_pose,
          obs);

      mrpt::math::estimateJacobian(x0, &numeric_jacob_eval_function, x_incrs, dat, rje.J);
