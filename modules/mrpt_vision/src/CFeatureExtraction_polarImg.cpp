    ...
    const mrpt::img::CImage& in_img, CFeatureList& in_features)
{
  ...
  mrpt::system::CTimeLoggerEntry tle(profiler, "internal_computePolarImageDescriptors");
  ...
  CImage linpolar_frame(patch_w, patch_h, in_img.channels());
  ...
  for (auto& f : in_features)
  {
    ...
    cvLinearPolar(&cvin, &cvout, pt, radius, CV_INTER_LINEAR + CV_WARP_FILL_OUTLIERS);
#elif MRPT_OPENCV_VERSION_NUM < 0x030402
    cv::linearPolar(
        in(cv::Rect(
            mrpt::round(pt.x - radius), mrpt::round(pt.y - radius), 1 + 2 * radius,
            1 + 2 * radius)),
        out, pt, radius, CV_INTER_LINEAR + CV_WARP_FILL_OUTLIERS);
#else
    // Latest opencv versions:
    cv::warpPolar(
        in, out, cv::Size(patch_w, patch_h), pt, radius, cv::INTER_LINEAR + cv::WARP_FILL_OUTLIERS);
#endif
