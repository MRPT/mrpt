    ...
    post_tester_t post_tester)
{
  ...
  const auto ini_fil = mrpt::mrpt_data_dir() + "/config_files/icp-slam/"s + ini_filename;
...
TEST(ICP_SLAM_App, MapFromRawlog_PointMap)
{
  ...
  generic_icp_slam_test<mrpt::apps::ICP_SLAM_App_Rawlog>(
      "icp-slam_demo_classic.ini", "2006-01ENE-21-SENA_Telecom Faculty_one_loop_only.rawlog",
...
TEST(ICP_SLAM_App, MapFromRawlog_LM)
{
  ...
  generic_icp_slam_test<mrpt::apps::ICP_SLAM_App_Rawlog>(
      "icp-slam_demo_LM.ini", "2006-01ENE-21-SENA_Telecom Faculty_one_loop_only.rawlog",
      [](mrpt::config::CConfigFileBase&) {}, tester_for_2006_01_21);
}
