...
size_t CFileOutputStream::Read([[maybe_unused]] void* Buffer, [[maybe_unused]] size_t Count)
{
...
uint64_t CFileOutputStream::getTotalBytesCount() const
{
  ...
  const auto previousPos = f.tellp();
  f.seekp(0, ios_base::end);
  auto fileSize = f.tellp();
...
{
  return mrpt::format("mrpt::io::CFileOutputStream for file '%s'", m_filename.c_str());
}
