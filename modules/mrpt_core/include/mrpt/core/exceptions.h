...
{
unsigned int MAX_BACKTRACE_DEPTH();
...
std::string asrt_fail_near(
    std::string s, A&& a, B&& b, T&& tol, const char* astr, const char* bstr, const char* tolstr)
{
  s += "(";
  s += astr;
  s += ",";
...
struct ExceptionWithCallBack : public BASE_EXCEPTION, public ExceptionWithCallBackBase
{
  ExceptionWithCallBack(const BASE_EXCEPTION& originalException) :
      BASE_EXCEPTION(originalException),
      ExceptionWithCallBackBase(
          originalException.what(),
          mrpt::callStackBackTrace(2 /*skip 2 frames*/, 2U + mrpt::internal::MAX_BACKTRACE_DEPTH()))
  {
  }
...
#define THROW_TYPED_EXCEPTION(msg, exceptionClass)                  \
  throw mrpt::ExceptionWithCallBack<exceptionClass>(exceptionClass( \
      mrpt::internal::exception_line_msg(msg, __FILE__, __LINE__, __CURRENT_FUNCTION_NAME__)))
...
#define MRPT_THROW_UNKNOWN_SERIALIZATION_VERSION(__V) \
  THROW_EXCEPTION_FMT(                                \
      "Cannot parse object: unknown serialization version number: '%i'", static_cast<int>(__V))
 ...
 * \sa MRPT_TRY_START, MRPT_TRY_END
 */
#define ASSERT_(f) ASSERTMSG_(f, "Assert condition failed: " #f)
...
#define ASSERT_EQUAL_(__A, __B)                                  \
  ...
  {                                                              \
    const auto& _a = (__A);                                      \
    const auto& _b = (__B);                                      \
    if (_a != _b) ASRT_FAIL("ASSERT_EQUAL_", _a, _b, #__A, #__B) \
  } while (0)
...
#define ASSERT_NOT_EQUAL_(__A, __B)                                  \
  ...
  {                                                                  \
    const auto& _a = (__A);                                          \
    const auto& _b = (__B);                                          \
    if (_a == _b) ASRT_FAIL("ASSERT_NOT_EQUAL_", _a, _b, #__A, #__B) \
  } while (0)
...
#define ASSERT_NEAR_(__A, __B, __TOLERANCE)                                                        \
  ...
  {                                                                                                \
    const auto& _a = (__A);                                                                        \
    const auto& _b = (__B);                                                                        \
    const auto& _tol = (__TOLERANCE);                                                              \
    const auto _diff = mrpt::abs_diff<std::decay_t<decltype(_a)>>(_a, _b);                         \
    if (_diff > _tol)                                                                              \
      THROW_EXCEPTION(                                                                             \
          mrpt::internal::asrt_fail_near("ASSERT_NEAR_", _a, _b, _tol, #__A, #__B, #__TOLERANCE)); \
  } while (0)
...
#define ASSERT_LT_(__A, __B)                                  \
  ...
  {                                                           \
    const auto& _a = (__A);                                   \
    const auto& _b = (__B);                                   \
    if (_a >= _b) ASRT_FAIL("ASSERT_LT_", _a, _b, #__A, #__B) \
  } while (0)
...
#define ASSERT_LE_(__A, __B)                                 \
  ...
  {                                                          \
    const auto& _a = (__A);                                  \
    const auto& _b = (__B);                                  \
    if (_a > _b) ASRT_FAIL("ASSERT_LE_", _a, _b, #__A, #__B) \
  } while (0)
...
#define ASSERT_GT_(__A, __B)                                  \
  ...
  {                                                           \
    const auto& _a = (__A);                                   \
    const auto& _b = (__B);                                   \
    if (_a <= _b) ASRT_FAIL("ASSERT_GT_", _a, _b, #__A, #__B) \
  } while (0)
...
#define ASSERT_GE_(__A, __B)                                 \
  ...
  {                                                          \
    const auto& _a = (__A);                                  \
    const auto& _b = (__B);                                  \
    if (_a < _b) ASRT_FAIL("ASSERT_GE_", _a, _b, #__A, #__B) \
  } while (0)
