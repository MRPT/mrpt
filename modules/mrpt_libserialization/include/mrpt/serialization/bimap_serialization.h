...
CArchive& operator<<(CArchive& out, const mrpt::containers::bimap<KEY, VALUE>& obj)
{
  ...
  for (const auto& kv : obj.getDirectMap()) out << kv.first << kv.second;
  return out;
}
...
CArchive& operator>>(CArchive& in, mrpt::containers::bimap<KEY, VALUE>& obj)
{
  ...
  if (pref != "mrpt::containers::bimap")
    THROW_EXCEPTION_FMT(
        ...
        "wrong: '%s'",
        "mrpt::containers::bimap", kType.c_str(), mTp.c_str(), pref.c_str());
  in >> stored_K;
  if (stored_K != std::string(kType.c_str()))
    THROW_EXCEPTION_FMT(
        "Error: serialized container %s key type %s != %s", "mrpt::containers::bimap",
        stored_K.c_str(), kType.c_str());
  in >> stored_V;
  if (stored_V != std::string(mTp.c_str()))
    THROW_EXCEPTION_FMT(
        "Error: serialized container %s value type %s != %s", "mrpt::containers::bimap",
