...
CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::CVisualizer(
    const GRAPH_T& graph_in) :
    m_graph(graph_in)
{
...
////////////////////////////////////////////////////////////
template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::getAs3DObject(
    mrpt::viz::CSetOfObjects::Ptr& object, const mrpt::containers::yaml& viz_params) const
{
  using namespace mrpt::viz;

  // graph visualization parameters
  const bool show_ID_labels = viz_params.getOrDefault<bool>("show_ID_labels", false);
  const bool show_ground_grid = viz_params.getOrDefault<bool>("show_ground_grid", true);
  const bool show_edges = viz_params.getOrDefault<bool>("show_edges", true);
  const bool show_node_corners = viz_params.getOrDefault<bool>("show_node_corners", true);
  const bool show_edge_rel_poses = viz_params.getOrDefault<bool>("show_edge_rel_poses", false);
  const double nodes_point_size = viz_params.getOrDefault<double>("nodes_point_size", 0.);
...
template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::drawGroundGrid(
    mrpt::viz::CSetOfObjects::Ptr& object, const mrpt::containers::yaml* viz_params /*=NULL*/) const
{
  ...
  CGridPlaneXY::Ptr grid =
      CGridPlaneXY::Create(BB_min.x, BB_max.x, BB_min.y, BB_max.y, BB_min.z, grid_frequency);
  grid->setColor(0.3f, 0.3f, 0.3f);
  object->insert(grid);
}

template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::drawNodePoints(
    mrpt::viz::CSetOfObjects::Ptr& object, const mrpt::containers::yaml* viz_params /*=NULL*/) const
{
  ASSERTMSG_(viz_params, "Pointer to viz_params was not provided.");
  ...
  const unsigned int nodes_point_color =
      viz_params->getOrDefault<unsigned int>("nodes_point_color", (unsigned int)0xA0A0A0);
...
template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::drawNodeCorners(
    mrpt::viz::CSetOfObjects::Ptr& object, const mrpt::containers::yaml* viz_params /*=NULL*/) const
{
  using namespace mrpt::viz;
  ...
  const bool show_node_corners = viz_params->getOrDefault<bool>("show_node_corners", true);
  const bool show_ID_labels = viz_params->getOrDefault<bool>("show_ID_labels", false);
  const double nodes_corner_scale = viz_params->getOrDefault<double>("nodes_corner_scale", 0.7);
...
template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::drawEdgeRelPoses(
    mrpt::viz::CSetOfObjects::Ptr& object, const mrpt::containers::yaml* viz_params /*=NULL*/) const
{
  using namespace mrpt::viz;
  using namespace mrpt::img;
  ...
  const unsigned int edge_rel_poses_color =
      viz_params->getOrDefault<unsigned int>("edge_rel_poses_color", (unsigned int)0x40FF8000);
  const TColor col8bit(edge_rel_poses_color & 0xffffff, edge_rel_poses_color >> 24);
  const double edge_width = viz_params->getOrDefault<double>("edge_width", 2.);

  for (auto edge_it = m_graph.begin(); edge_it != m_graph.end(); ++edge_it)
  {
    // Node ID of the source pose:
    const TNodeID node_id_start =
        m_graph.edges_store_inverse_poses ? edge_it->first.second : edge_it->first.first;
    ...
    if (n_it != m_graph.nodes.end())
    {
      ...
      // coords:
      mrpt::viz::CSetOfObjects::Ptr gl_rel_edge = mrpt::viz::CSetOfObjects::Create();
      ...
      mrpt::viz::CSetOfObjects::Ptr gl_edge_corner =
          (m_is_3D_graph
      ...
      mrpt::viz::CSimpleLine::Ptr gl_line = mrpt::viz::CSimpleLine::Create(
...
template <class CPOSE, class MAPS_IMPLEMENTATION, class NODE_ANNOTATIONS, class EDGE_ANNOTATIONS>
void CVisualizer<CPOSE, MAPS_IMPLEMENTATION, NODE_ANNOTATIONS, EDGE_ANNOTATIONS>::drawEdges(
    mrpt::viz::CSetOfObjects::Ptr& object, const mrpt::containers::yaml* viz_params /*=NULL*/) const
{
  using namespace mrpt::viz;
  using namespace mrpt::img;
  ...
  const unsigned int edge_color =
      viz_params->getOrDefault<unsigned int>("edge_color", (unsigned int)0x400000FF);
  const double edge_width = viz_params->getOrDefault<double>("edge_width", 2.);
