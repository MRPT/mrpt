...
TUserOptionsChecker<GRAPH_t>::TUserOptionsChecker() : sep_header(40, '='), sep_subheader(20, '-')
{
}
...
TUserOptionsChecker<GRAPH_t>::~TUserOptionsChecker()
{
  ...
  // deciders/optimizers
  for (auto it = regs_descriptions.begin(); it != regs_descriptions.end(); ++it)
  {
    delete *it;
  }
  for (auto it = optimizers_descriptions.begin(); it != optimizers_descriptions.end(); ++it)
  {
    delete *it;
  }
}
...
void TUserOptionsChecker<GRAPH_t>::createDeciderOptimizerMappings()
{
  ...
  node_regs_map["CEmptyNRD"] = &createNodeRegistrationDecider<CEmptyNRD<GRAPH_t>>;
  node_regs_map["CFixedIntervalsNRD"] = &createNodeRegistrationDecider<CFixedIntervalsNRD<GRAPH_t>>;
  // edge registration deciders
  edge_regs_map["CEmptyERD"] = &createEdgeRegistrationDecider<CEmptyERD<GRAPH_t>>;
  // optimizers
  optimizers_map["CLevMarqGSO"] = &createGraphSlamOptimizer<CLevMarqGSO<GRAPH_t>>;
...
template <class GRAPH_t>
void TUserOptionsChecker<GRAPH_t>::dumpRegistrarsToConsole(std::string reg_type /*="all"*/) const
{
  MRPT_START
  using namespace std;
    ...
    std::string given_reg, std::string reg_type) const
{
  ...
  for (auto dec_it = regs_descriptions.begin(); dec_it != regs_descriptions.end(); ++dec_it)
  {
    ...
    pose_t p;
    if ((!dec->is_slam_2d && IS_CLASS(p, CPose2D)) || (!dec->is_slam_3d && IS_CLASS(p, CPose3D)))
    {
      continue;
    }
...
bool TUserOptionsChecker<GRAPH_t>::checkOptimizerExists(std::string given_opt) const
{
       ...
       ++opt_it)
  {
    ...
    pose_t p;
    if ((!opt->is_slam_2d && IS_CLASS(p, CPose2D)) && (!opt->is_slam_3d && IS_CLASS(p, CPose3D)))
    {
      continue;
    }
