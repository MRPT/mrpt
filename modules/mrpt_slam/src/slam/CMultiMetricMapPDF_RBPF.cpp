...
void KLF_loadBinFromParticle(
    ...
    const TPose3D* newPoseToBeInserted)
{
  const size_t lenBinPath =
      (currentParticleValue != nullptr) ? currentParticleValue->robotPath.size() : 0;
...
struct TAuxRangeMeasInfo
{
  ...
  int64_t beaconID;
  size_t nGaussiansInMap{0};  // Number of Gaussian modes in the map representation
    ...
    const bayes::CParticleFilter::TParticleFilterOptions& PF_options)
{
  ...
  for (i = 0, partIt = m_particles.begin(); partIt != m_particles.end(); partIt++, i++)
  {
    ...
    const CPose3D ith_last_pose =
        CPose3D(*partIt->d->robotPath.rbegin());  // The last robot pose in the path
    ...
    if (options.pfOptimalProposal_mapSelection == 0 ||
        options.pfOptimalProposal_mapSelection == 1 || options.pfOptimalProposal_mapSelection == 3)
    {
      ...
      // Use ICP to align to each particle's map:
      {
        CPosePDF::Ptr alignEst =
            icp.Align(map_to_align_to, &localMapPoints, CPose2D(initialPoseEstimation), icpInfo);
      ...
      MRPT_LOG_DEBUG_FMT(
          "gridICP[particle %u]: %.02f%%", static_cast<unsigned int>(i), 100 * icpInfo.goodness);
      ...
      getRandomGenerator().drawGaussianMultivariate(rndSamples, finalEstimatedPoseGauss.cov);
      // Add noise:
      finalPose.setFromValues(
          finalPose.x() + rndSamples[0], finalPose.y() + rndSamples[1], finalPose.z(),
          finalPose.yaw() + rndSamples[2], finalPose.pitch(), finalPose.roll());
    }
    else if (options.pfOptimalProposal_mapSelection == 2)
    {
      ...
      if (!robotActionSampler.isPrepared())
      {
        ...
        else
        {
          ...
          if (beacMap->get(0).m_typePDF == CBeacon::pdfSOG)
          {
            ...
            beacMap->get(0).m_locationGauss.cov.setDiagonal(3, 1e-6);
          }
        }
      ...
      for (const auto& itObs : *sf)
      {
        ...
        {
          const auto* obs = static_cast<const CObservationBeaconRanges*>(itObs.get());
          ...
          for (itRanges = obs->sensedData.begin(); itRanges != obs->sensedData.end(); itRanges++)
          {
            ...
            for (auto itBeacs = beacMap->begin(); itBeacs != beacMap->end(); ++itBeacs)
            {
              if ((itBeacs)->m_ID == itRanges->beaconID)
              {
                ...
                newMeas.nGaussiansInMap = (itBeacs)->m_typePDF == CBeacon::pdfSOG
      ...
      if (methodSOGorGrid)
      {
        ...
        for (auto itBeacs = beacMap->begin(); itBeacs != beacMap->end(); ++itBeacs)
        {
          ...
          for (auto& lstObservedRange : lstObservedRanges)
          {
            if ((itBeacs)->m_ID == lstObservedRange.beaconID)
            {
              ...
              {
                std::cout << "#modes bef: " << fusedObsModels.size()
                ...
                for (it = fusedObsModels.begin(); it != fusedObsModels.end(); it++)
                  max_w = max(max_w, (it)->log_w);  // keep the
                // maximum
                // mode weight
                ...
                std::cout << "#modes after: " << fusedObsModels.size() << "\n";
              }
              ...
              if (sqrt(currentCov(0, 0)) < 0.10f && sqrt(currentCov(1, 1)) < 0.10f &&
        ...
        if (beacMap->size()) fusedObsModels.drawSingleSample(newDrawnPosition);
      }
      else
      {
        // =============================
        ...
        do
        {
          auto pdfGrid = std::make_unique<CPosePDFGrid>(
              grid_min_x, grid_max_x, grid_min_y, grid_max_y, grid_resXY, 180.0_deg, 0, 0);
                              ...
                              exp(-0.5 *
                                  square(
                                      ...
                                      (it)->val.mean.distance2DTo(
                                          grid_x + lstObservedRange.sensorLocationOnRobot.x(),
                                          grid_y + lstObservedRange.sensorLocationOnRobot.y())) /
                                  sensorSTD2);
                        }
                      }
                      break;
    ...
    {
      partIt->log_w += PF_options.powFactor * (PF_SLAM_computeObservationLikelihoodForParticle(
...
{
  auto* map = const_cast<CMultiMetricMap*>(&m_particles[particleIndexForMap].d->mapTillNow);
  double ret = 0;
  for (const auto& it : observation) ret += map->computeObservationLikelihood(*it, x);
  return ret;
}
