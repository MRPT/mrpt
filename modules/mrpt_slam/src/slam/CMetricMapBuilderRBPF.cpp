    ...
    CActionCollection& action, CSensoryFrame& observations)
{
    ...
    CActionCollection fakeActs;
    {
      ...
      else
      {
        ...
        newAct.computeFromOdometry(
            CPose2D(odoIncrementSinceLastLocalization.mean), act2D->motionModelConfiguration);
  ...
  if (do_map_update)
  {
    ...
    if (!anymap_update)
      MRPT_LOG_WARN_STREAM(
          "**No map was updated** after inserting a CSensoryFrame with " << observations.size());
  ...
  for (auto& m_particle : mapPDF.m_particles) m_particle.d->mapTillNow.auxParticleFilterCleanUp();
...
/*---------------------------------------------------------------
          initialize
  ---------------------------------------------------------------*/
void CMetricMapBuilderRBPF::initialize(const CSimpleMap& initialMap, const CPosePDF* x0)
{
...
CPose3DPDF::Ptr CMetricMapBuilderRBPF::getCurrentPoseEstimation() const
{
  ...
  {
    p.d.composePose(this->odoIncrementSinceLastLocalization.mean.asTPose(), p.d);
  }
  return posePDF;
}
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
double CMetricMapBuilderRBPF::getCurrentJointEntropy() { return mapPDF.getCurrentJointEntropy(); }
...
void CMetricMapBuilderRBPF::TConstructionOptions::dumpToTextStream(std::ostream& out) const
{
  ...
  out << mrpt::format("localizeLinDistance                     = %f m\n", localizeLinDistance);
  out << mrpt::format(
    ...
    const mrpt::config::CConfigFileBase& iniFile, const std::string& section)
{
  ...
  verbosity_level =
      iniFile.read_enum<mrpt::system::VerbosityLevel>(section, "verbosity_level", verbosity_level);
