...
class CFeature : public mrpt::serialization::CSerializable
{
 ...
 public:
  ...
  struct TDescriptors
  {
    ...
    bool hasDescriptorLogPolarImg() const { return LogPolarImg.has_value(); }
  ...
  float descriptorPolarImgDistanceTo(
      const CFeature& oFeature, float& minDistAngle, bool normalize_distances = true) const;
 ...
 protected:
  ...
  static float internal_distanceBetweenPolarImages(
      const mrpt::math::CMatrixF& desc1,
...
class CFeatureList : public mrpt::math::KDTreeCapable<CFeatureList>
{
 ...
 public:
  ...
  inline CFeature& operator[](const unsigned int index) { return m_feats[index]; }
  inline const CFeature& operator[](const unsigned int index) const { return m_feats[index]; }
  ...
  /** @name getFeature*() methods for template-based access to feature list
    @{ */
  inline float getFeatureX(size_t i) const { return m_feats[i].keypoint.pt.x; }
  inline float getFeatureY(size_t i) const { return m_feats[i].keypoint.pt.y; }
  inline TFeatureID getFeatureID(size_t i) const { return m_feats[i].keypoint.ID; }
  inline float getFeatureResponse(size_t i) const { return m_feats[i].keypoint.response; }
  ...
  inline TFeatureTrackStatus getTrackStatus(size_t i) { return m_feats[i].keypoint.track_status; }

  inline void setFeatureX(size_t i, float x) { m_feats[i].keypoint.pt.x = x; }
  inline void setFeatureXf(size_t i, float x) { m_feats[i].keypoint.pt.x = x; }
  inline void setFeatureY(size_t i, float y) { m_feats[i].keypoint.pt.y = y; }
  inline void setFeatureYf(size_t i, float y) { m_feats[i].keypoint.pt.y = y; }
  inline void setFeatureID(size_t i, TFeatureID id) { m_feats[i].keypoint.ID = id; }
  inline void setFeatureResponse(size_t i, float r) { m_feats[i].keypoint.response = r; }
  inline void setScale(size_t i, uint8_t s) { m_feats[i].keypoint.octave = s; }
