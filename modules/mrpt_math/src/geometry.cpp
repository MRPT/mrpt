...
void math::closestFromPointToSegment(
    double Px, double Py, double x1, double y1, double x2, double y2, double& out_x, double& out_y)
{
  if (x1 == x2 && y1 == y2)
  {
    out_x = x1;
    ...
    double R2_pose_phi)
{
  ...
  if (poly.contains({R1_x_min, R1_y_min}))
  {
    return true;
  }
  if (poly.contains({R1_x_max, R1_y_min}))
  {
    return true;
  }
  if (poly.contains({R1_x_max, R1_y_max}))
  {
    return true;
  }
  if (poly.contains({R1_x_min, R1_y_max}))
  {
    return true;
  }
  ...
  {
    const auto seg2 =
        TSegment2D::FromPoints({xs[idx], ys[idx]}, {xs[(idx + 1) % 4], ys[(idx + 1) % 4]});

    if (mrpt::math::intersect(
            TSegment2D::FromPoints({R1_x_min, R1_y_min}, {R1_x_max, R1_y_min}), seg2, intersectObj))
    {
      return true;
    }
    if (mrpt::math::intersect(
            TSegment2D::FromPoints({R1_x_max, R1_y_min}, {R1_x_max, R1_y_max}), seg2, intersectObj))
    {
      return true;
    }
    if (mrpt::math::intersect(
            TSegment2D::FromPoints({R1_x_max, R1_y_max}, {R1_x_min, R1_y_max}), seg2, intersectObj))
    {
      return true;
    }
    if (mrpt::math::intersect(
            TSegment2D::FromPoints({R1_x_min, R1_y_max}, {R1_x_min, R1_y_min}), seg2, intersectObj))
    {
      return true;
    }
  }
...
bool intersectInCommonPlane(
    const T3D& o1, const U3D& o2, const mrpt::math::TPlane& p, mrpt::math::TObject3D& obj)
{
  T3D proj1;
  U3D proj2;
...
bool intersectInCommonLine(
    const TSegment2D& s1, const TSegment2D& s2, const TLine2D& lin, TObject2D& obj)
{
  // Move in a free coordinate, searching for minima and maxima
...
inline void unsafeProjectPoint(const TPoint3D& point, const TPose3D& pose, TPoint2D& newPoint)
{
  TPoint3D dummy;
...
bool intersect(const TPolygonWithPlane& p1, const TLine3D& l2, double& d, double bestKnown)
{
  ...
  if (intersect(p1.plane, l2, obj))
    if (obj.isPoint())
    {
      ...
      mrpt::math::internal::unsafeProjectPolygon(p1.poly, p1.inversePose, newPoly);
...
bool math::intersect(const TPlane& p1, const TPlane& p2, TObject3D& obj)
{
      ...
      (std::abs(lin.director[2]) < geometryEpsilon))
  {
    // Planes are parallel
    for (size_t i = 0; i < 3; i++)
      if (std::abs(p1.coefs[i] * p2.coefs[3] - p1.coefs[3] * p2.coefs[i]) >= geometryEpsilon)
        return false;
    // Planes are the same
    obj.data = p1;
    return true;
...
bool math::intersect(const TLine3D& r1, const TLine3D& r2, TObject3D& obj)
{
  ...
  {
    double sysDet =
        -r1.director[c1[i]] * r2.director[c2[i]] + r2.director[c1[i]] * r1.director[c2[i]];
...
bool math::intersect(const TLine2D& r1, const TLine2D& r2, TObject2D& obj)
{
  ...
  // Lines are parallel
  if (std::abs(r1.coefs[0] * r2.coefs[2] - r1.coefs[2] * r2.coefs[0]) >= geometryEpsilon ||
      std::abs(r1.coefs[1] * r2.coefs[2] - r1.coefs[2] * r2.coefs[1]) >= geometryEpsilon)
  {
    return false;
  }
...
bool math::intersect(const TSegment2D& s1, const TSegment2D& s2, TObject2D& obj)
{
  ...
  if (obj.isLine())
  {
    // Segments' lines are parallel
    return intersectInCommonLine(s1, s2, lin, obj);
  }
...
double math::getAngle(const TPlane& s1, const TPlane& s2)
{
  ...
  {
    THROW_EXCEPTION("Invalid plane(s)");
  }
...
double math::getAngle(const TPlane& s1, const TLine3D& r2)
{
  ...
  if (s < geometryEpsilon)
  {
    THROW_EXCEPTION("Invalid plane or line");
  }
  if (std::abs(s) < std::abs(c))
  {
    return (c > 0) ? 0 : M_PI;
...
double math::getAngle(const TLine3D& r1, const TLine3D& r2)
{
  ...
  if (s < geometryEpsilon)
  {
    THROW_EXCEPTION("Invalid line(s)");
  }
  if (std::abs(s) < std::abs(c))
  {
    return (c / s < 0) ? M_PI : 0;
...
void math::project3D(const TLine3D& line, const TPose3D& newXYpose, TLine3D& newLine)
{
  ...
  for (size_t i = 0; i < 3; i++)
  {
    ...
    for (size_t j = 0; j < 3; j++) newLine.director[i] += mat(i, j) * line.director[j];
  }
  newLine.unitarize();
}

void math::project3D(const TPlane& plane, const TPose3D& newXYpose, TPlane& newPlane)
{
  CMatrixDouble44 mat;
  newXYpose.getHomogeneousMatrix(mat);
  for (size_t i = 0; i < 3; i++)
  {
    newPlane.coefs[i] = 0;
    for (size_t j = 0; j < 3; j++) newPlane.coefs[i] += mat(i, j) * plane.coefs[j];
  }
  ...
  newPlane.coefs[3] =
      ...
      sqrt(squareNorm<3, double>(newPlane.coefs) / squareNorm<3, double>(plane.coefs));
  newPlane.unitarize();
}
...
void math::project3D(const TPolygon3D& polygon, const TPose3D& newXYpose, TPolygon3D& newPolygon)
{
...
void math::project3D(const TObject3D& object, const TPose3D& newXYpose, TObject3D& newObject)
{
  std::visit(
      [&](auto& o)
      {
...
void math::project2D(const TPoint2D& point, const TPose2D& newXpose, TPoint2D& newPoint)
{
  newPoint = newXpose.composePoint(point);
}

void math::project2D(const TLine2D& line, const TPose2D& newXpose, TLine2D& newLine)
{
  ...
  newLine.coefs[2] =
      line.coefs[2] - (newLine.coefs[0] * newXpose.x + newLine.coefs[1] * newXpose.y);
  return;
}

void math::project2D(const TPolygon2D& line, const TPose2D& newXpose, TPolygon2D& newLine)
{
  size_t N = line.size();
  newLine.resize(N);
...
void math::project2D(const TObject2D& obj, const TPose2D& newXpose, TObject2D& newObject)
{
  std::visit(
      [&](auto& o)
      {
...
bool math::intersect(const TPolygon2D& p1, const TLine2D& r2, TObject2D& obj)
{
  ...
  switch (pnts.size())
  {
    ...
    default:
      THROW_EXCEPTION("Polygon is not convex");
  }
}
...
struct TCommonRegion
{
  ...
  TCommonRegion(const TSegment2D& s) { segment = std::make_shared<TSegment2D>(s); }
};
...
struct TTempIntersection
{
  ...
  TTempIntersection(const T2ListsOfSegments& s) { segms = std::make_shared<T2ListsOfSegments>(s); }
  TTempIntersection(const TCommonRegion& c) { common = std::make_shared<TCommonRegion>(c); }
};
struct TSegmentWithLine
{
  TSegment2D segment;
  TLine2D line;
  explicit TSegmentWithLine(const TSegment2D& s) : segment(s) { line = TLine2D(s[0], s[1]); }
...
TPolygon2D mrpt::math::intersect(const TPolygon2D& subject, const TPolygon2D& clipping)
{
  ...
  for (const auto& clipEdge : segsClip)
  {
    ...
    for (size_t i = 0; i < inputList.size(); i++)
    {
      ...
      const auto prev_point = inputList.at(i > 0 ? (i - 1) : (inputList.size() - 1));

      const bool curInside = (clipEdge.line.signedDistance(current_point) < 0) ^ clipIsCW;
      const bool prevInside = (clipEdge.line.signedDistance(prev_point) < 0) ^ clipIsCW;

      mrpt::math::TObject2D intersectPt;
      const bool doIntersect = mrpt::math::intersect(
          clipEdge.line, mrpt::math::TLine2D::FromTwoPoints(current_point, prev_point),
          intersectPt);
      std::optional<mrpt::math::TPoint2D> iPt;
      if (doIntersect)
      {
        intersectPt.getPoint(iPt.emplace());
      }
    ...
    TObject3D& obj)
{
  ...
  if (obj.isPlane())
    return intersectInCommonPlane<TPolygon2D, TPolygon2D>(
        p1, p2, pl1,
        obj);  // Polygons are on the same plane
  else if (obj.getLine(ln))
  {
    TObject3D obj1, obj2;
...
void getMinAndMaxBounds(
    const std::vector<TPolygon3D>& v1, std::vector<TPoint3D>& minP, std::vector<TPoint3D>& maxP)
{
  minP.resize(0);
  maxP.resize(0);
  size_t N = v1.size();
    ...
    CSparseMatrixTemplate<TObject3D>& objs)
{
  ...
  for (size_t i = 0; i < M; i++)
    for (size_t j = 0; j < N; j++)
      if (!compatibleBounds(minBounds1[i], maxBounds1[i], minBounds2[j], maxBounds2[j]))
        continue;
    ...
    std::vector<TObject3D>& objs)
{
  ...
  for (auto it1 = v1.begin(); it1 != v1.end(); ++it1, ++itP1, ++itMin1, ++itMax1)
  {
    ...
    for (auto it2 = v2.begin(); it2 != v2.end(); ++it2, ++itP2, ++itMin2, ++itMax2)
...
{
  if (std::abs(r1.coefs[0] * r2.coefs[1] - r2.coefs[0] * r1.coefs[1]) < geometryEpsilon)
  {
    // Lines are parallel
...
double math::distance([[maybe_unused]] const TPolygon2D& p1, [[maybe_unused]] const TPolygon2D& p2)
{
  THROW_EXCEPTION("TO DO:distance(TPolygon2D,TPolygon2D)");
}

double math::distance([[maybe_unused]] const TPolygon2D& p1, [[maybe_unused]] const TSegment2D& s2)
{
  THROW_EXCEPTION("TO DO:distance(TPolygon2D,TSegment)");
}

double math::distance([[maybe_unused]] const TPolygon2D& p1, [[maybe_unused]] const TLine2D& l2)
{
  THROW_EXCEPTION("TO DO:distance(TPolygon2D,TLine2D)");
}

double math::distance([[maybe_unused]] const TPolygon3D& p1, [[maybe_unused]] const TPolygon3D& p2)
{
  THROW_EXCEPTION("TO DO:distance(TPolygon3D,TPolygon3D");
}

double math::distance([[maybe_unused]] const TPolygon3D& p1, [[maybe_unused]] const TSegment3D& s2)
{
  THROW_EXCEPTION("TO DO:distance(TPolygon3D,TSegment3D");
}

double math::distance([[maybe_unused]] const TPolygon3D& p1, [[maybe_unused]] const TLine3D& l2)
{
  THROW_EXCEPTION("TO DO:distance(TPolygon3D,TLine3D");
}

double math::distance([[maybe_unused]] const TPolygon3D& po, [[maybe_unused]] const TPlane& pl)
{
...
CMatrixDouble44 math::generateAxisBaseFromDirectionAndAxis(const TVector3D& vec, uint8_t coord)
{
  CMatrixDouble44 m;
...
double math::getRegressionPlane(const vector<TPoint3D>& points, TPlane& plane)
{
  ...
  for (size_t i = 0; i < 3; ++i)
  {
    if (eigenVal[i] < 0 && std::abs(eigenVal[i]) < geometryEpsilon) eigenVal[i] = 0;
  }

  if (std::abs(eigenVal.at(1)) < geometryEpsilon)
    THROW_EXCEPTION("Points are aligned, cannot fit a plane (infinite solutions)");
      ...
      else if (firstOrNonPresent(i, current))
      {
        ...
        if (current.size() >= 2 && current[0].seg1 == i)
        {
          if (s2p != current[0].seg1Point)
          {
            ...
            for (auto it = current.begin(); it != current.end(); ++it) used[it->seg2] = true;
    ...
    const mrpt::optional_ref<std::vector<mrpt::math::TObject3D>>& others = std::nullopt)
{
  ...
  {
    if (o.isPolygon())
      r.emplace_back(o.getAs<TPolygon3D>());
    else if (others)
...

{
  ...
  {
    if (o.isSegment())
      r.emplace_back(o.getAs<TSegment3D>());
    else if (others)
      others->get().push_back(o);
  }
  return r;
    ...
    for (size_t j = ii; j != i_; j = (j + 1) % N)
      if (intersect(segms[i].line, segms[j], obj) && obj.getPoint(pnt))
      {
        TSegmentWithLine sTmp = TSegmentWithLine(
            pnt,
            segms[i].segment
                [(distance(pnt, segms[i].segment.point1) < distance(pnt, segms[i].segment.point2))
                     ? 0
                     : 1]);
        bool cross = false;
        TPoint2D pTmp;
        for (size_t k = 0; (k < N) && !cross; k++)
          ...
          {
            if (obj.getPoint(pTmp) && (distance(pTmp, sTmp.segment[0]) >= geometryEpsilon) &&
        ...
        if (splitInConvexComponents(p1, tempComps))
          components.insert(components.end(), tempComps.begin(), tempComps.end());
        else
...
bool math::splitInConvexComponents(const TPolygon3D& poly, vector<TPolygon3D>& components)
{
  ...
  if (splitInConvexComponents(poly2D, components2D))
  {
    ...
    transform(
        components2D.begin(), components2D.end(), components.begin(), FUnprojectPolygon2D(pose1));
    return true;
  }
  else
    return false;
...
bool math::traceRay(const vector<TPolygonWithPlane>& vec, const TPose3D& pose, double& dist)
{
  dist = HUGE_VAL;
  double nDist = 0;
  TLine3D lin;
