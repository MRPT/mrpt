 ...
 */
template <typename T>
struct MRPT_EMPTY_BASES TPoint2D_ : public TPoint2D_data<T>, public TPoseOrPoint
{
  static constexpr std::size_t static_size = 2;

  // Inlined from internal::ProvideStaticResize<TPoint2D_<T>>
   ...
   *  Enables branch-free iteration and SIMD-friendly access to (x, y).
   *  Valid for standard-layout struct with no padding between x and y. */
  ...
  // Inlined from internal::ProvidesStringConversion<TPoint2D_<T>>
