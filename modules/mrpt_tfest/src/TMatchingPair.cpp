...
template <typename T>
void TMatchingPairListTempl<T>::squareErrorVector(const CPose2D& q, vector<T>& out_sqErrs) const
{
  ...
  for (corresp = base_t::begin(), e_i = out_sqErrs.begin(); corresp != base_t::end();
       ++corresp, ++e_i)
  {
...
template <typename T>
void TMatchingPairListTempl<T>::squareErrorVector(const CPose3D& q, vector<T>& out_sqErrs) const
{
  out_sqErrs.clear();
...
void TMatchingPairListTempl<T>::filterUniqueRobustPairs(
    const size_t num_elements_this_map, TMatchingPairListTempl<T>& out_filtered_list) const
{
  ...
  for (auto& c : *this)
  {
    if (bestMatchForThisMap[c.globalIdx] == nullptr ||  // first one
        c.errorSquareAfterTransformation <
            bestMatchForThisMap[c.globalIdx]->errorSquareAfterTransformation  // or better
    )
    {
