    ...
    TSE3RobustResult& results)
{
  ...
  for (size_t iterations = 0; iterations < maxIters; iterations++)
  {
    ...
    bool res = mrpt::tfest::se3_l2(mbInliers, mbOutQuat, scale, params.forceScaleToUnity);
    if (!res)
    {
    ...
    for (size_t k = n; k < nCorrs; k++)
    {
      ...
      res = mrpt::tfest::se3_l2(mbInliers, csOutQuat, scale, params.forceScaleToUnity);
      ...
      const double angDist =
          mrpt::poses::Lie::SO<3>::log((csOut - mbOut).getRotationMatrix()).norm();
      const double scaleDist = std::abs(mbOut_vec[6] - scale);

      if (linDist < params.ransac_threshold_lin && angDist < params.ransac_threshold_ang &&
      ...
      {
        // std::cout << " It " << iterations << " - RANSAC Outlier Detected:
        // " << k << "\n";
      }
    }  // end 'inner' for
    ...
    if (cSet.size() >= d)
    {
      ...
      const double err = std::sqrt(
          ...
          square(mbOut_vec[2] - cIOut.z()) + square(mbOut_vec[3] - cIOut.yaw()) +
          square(mbOut_vec[4] - cIOut.pitch()) + square(mbOut_vec[5] - cIOut.roll()) +
