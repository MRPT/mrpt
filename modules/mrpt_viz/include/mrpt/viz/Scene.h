 ...
 * (mrpt::viz::Viewport), named "main". Optionally, any
...
class Scene : public mrpt::serialization::CSerializable, public std::enable_shared_from_this<Scene>
{
 ...
 public:
   ...
   */
  void insert(
      const CVisualObject::Ptr& newObject, const std::string& viewportName = std::string("main"));
  ...
  */
  ...
  {
    return std::const_pointer_cast<T>(static_cast<const Scene*>(this)->getByClass<T>(ith));
  }
   ...
   */
  void removeObject(
      const CVisualObject::Ptr& obj, const std::string& viewportName = std::string("main"));
 ...
 protected:
  ...
  template <typename FUNCTOR>
  static void internal_visitAllObjects(FUNCTOR functor, const CVisualObject::Ptr& o)
  {
    functor(o);
 ...
 * chaining.  \sa mrpt::viz::Scene::insert */
template <class T>
