 ...
 * https://wikipedia.org
 *
 * ![mrpt::viz::CPolyhedron](preview_CPolyhedron.png)
 *
 * \sa opengl::Scene
...
{
  DEFINE_SERIALIZABLE(CPolyhedron, mrpt::viz)
 public:
  /**
  ...
  struct TPolyhedronFace
  {
    ...
    void getCenter(
        const std::vector<mrpt::math::TPoint3D>& vertices, mrpt::math::TPoint3D& p) const;
  };
  ...
  */
  ...
  static CPolyhedron::Ptr CreateDoublePyramid(
      const std::vector<mrpt::math::TPoint2D>& baseVertices, double height1, double height2);
  /**
  ...
  static CPolyhedron::Ptr CreateTruncatedPyramid(
      const std::vector<mrpt::math::TPoint2D>& baseVertices, double height, double ratio);
  /**
  ...
  */
  static CPolyhedron::Ptr CreateJohnsonSolidWithConstantBase(
      uint32_t numBaseEdges, double baseRadius, const std::string& components, size_t shifts = 0);
  /** @}
   */
   ...
   */
  void getVertices(std::vector<mrpt::math::TPoint3D>& vertices) const { vertices = m_Vertices; }
  /**
   * Gets a list with the polyhedron's edges.
   */
  void getEdges(std::vector<TPolyhedronEdge>& edges) const { edges = m_Edges; }
  /**
   * Gets a list with the polyhedron's faces.
   */
  void getFaces(std::vector<TPolyhedronFace>& faces) const { faces = m_Faces; }
  /**
 ...
 private:
  ...
  static bool checkConsistence(
      const std::vector<mrpt::math::TPoint3D>& vertices, const std::vector<TPolyhedronFace>& faces);
  /**
 ...
 public:
   ...
   */
  CPolyhedron() : m_Vertices(), m_Edges(), m_Faces() {}
  /**
  ...
  CPolyhedron(
      ...
      bool doCheck = true) :
      m_Vertices(vertices), m_Edges(), m_Faces(faces), m_Wireframe(false), polygonsUpToDate(false)
  {
    InitFromVertAndFaces(vertices, faces, doCheck);
  }
  void InitFromVertAndFaces(
      const std::vector<mrpt::math::TPoint3D>& vertices,
      const std::vector<TPolyhedronFace>& faces,
      bool doCheck = true)
  {
...
MRPT_DECLARE_TTYPENAME_NAMESPACE(CPolyhedron::TPolyhedronEdge, mrpt::viz)
MRPT_DECLARE_TTYPENAME_NAMESPACE(CPolyhedron::TPolyhedronFace, mrpt::viz)
}  // namespace mrpt::typemeta
