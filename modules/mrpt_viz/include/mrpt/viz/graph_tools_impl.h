...
CSetOfObjects::Ptr graph_visualize(const GRAPH_T& g, const mrpt::containers::yaml& ep)
{
  MRPT_TRY_START
  ...
  const bool show_ground_grid = ep.getOrDefault<bool>("show_ground_grid", true);
  const bool show_edges = ep.getOrDefault<bool>("show_edges", true);
  const bool show_node_corners = ep.getOrDefault<bool>("show_node_corners", true);
  const bool show_edge_rel_poses = ep.getOrDefault<bool>("show_edge_rel_poses", false);
  const double nodes_point_size = ep.getOrDefault<double>("nodes_point_size", 0.);
  const double nodes_corner_scale = ep.getOrDefault<double>("nodes_corner_scale", 0.7);
  const double nodes_edges_corner_scale = ep.getOrDefault<double>("nodes_edges_corner_scale", 0.4);
  const unsigned int nodes_point_color =
      ep.getOrDefault<unsigned int>("nodes_point_color", (unsigned int)0xA0A0A0);
  ...
  const unsigned int edge_rel_poses_color =
      ep.getOrDefault<unsigned int>("edge_rel_poses_color", (unsigned int)0xFF800040);
  ...
  if (nodes_point_size > 0)
  {
    ...
    pnts->setColor(mrpt::img::TColorf(mrpt::img::TColor(nodes_point_color)));
  ...
  {
    const mrpt::img::TColor col8bit(edge_rel_poses_color & 0xffffff, edge_rel_poses_color >> 24);

    for (const auto& edge : g)
    {
      // Node ID of the source pose:
      const auto node_id_start = g.edges_store_inverse_poses ? edge.first.second : edge.first.first;
      ...
      if (itNod != g.nodes.end())
      {
        ...
        // relative coords:
        auto gl_rel_edge = mrpt::viz::CSetOfObjects::Create();
        ...
        auto gl_line = mrpt::viz::CSimpleLine::Create(
