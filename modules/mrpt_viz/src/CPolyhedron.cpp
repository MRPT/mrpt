...
bool analyzeJohnsonPartsString(
    const std::string& components, uint32_t numBaseEdges, vector<JohnsonBodyPart>& parts)
{
...
bool searchForFace(
    const vector<CPolyhedron::TPolyhedronFace>& fs, uint32_t v1, uint32_t v2, uint32_t v3)
{
...
bool searchForEdge(
    const vector<CPolyhedron::TPolyhedronEdge>& es, uint32_t v1, uint32_t v2, size_t& where)
{
...
double getHeight(const TPolygon3D& p, const TPoint3D& c)
{
  ...
  for (size_t i = 1; i < N; i++)
    ...
    else if (std::abs(mrpt::math::distance(p[i], p[(i + 1) % N]) - l) >= mrpt::math::getEpsilon())
      throw std::logic_error("There is a non-regular polygon.");
...
double CPolyhedron::TPolyhedronFace::area(const vector<TPoint3D>& vs) const
{
  ...
  for (size_t i = 1; i < N - 1; i++)
    res += sqrt(square(d[i - 1].mod * d[i].mod) - square(dotProduct<3, double>(d[i - 1], d[i])));
  return res / 2;
}
    ...
    m_Edges(), m_Wireframe(false), polygonsUpToDate(false)
{
  ...
  if (!getVerticesAndFaces(polys, vertices, faces)) throw std::logic_error("Can't create CPolygon");
...
CPolyhedron::Ptr CPolyhedron::CreatePyramid(const vector<TPoint2D>& baseVertices, double height)
{
  ...
  for (auto baseVertice : baseVertices) verts.emplace_back(baseVertice.x, baseVertice.y, 0);
  TPolyhedronFace f, g;
...
CPolyhedron::Ptr CPolyhedron::CreateCustomAntiprism(
    const vector<TPoint2D>& bottomBase, const vector<TPoint2D>& topBase, const double height)
{
...
CPolyhedron::Ptr CPolyhedron::CreateParallelepiped(
    const TPoint3D& base, const TPoint3D& v1, const TPoint3D& v2, const TPoint3D& v3)
{
...
CPolyhedron::Ptr CPolyhedron::CreateJohnsonSolidWithConstantBase(
    uint32_t numBaseEdges, double baseRadius, const std::string& components, size_t shifts)
{
  ...
  double antiPrism_height =
      sqrt(square(edgeLength) - square(baseRadius) * (2 - 2 * cos(M_PI / numBaseEdges)));
  // Vertices' and faces' vectors are computed
  size_t nVerts = numBaseEdges * (nParts - 1) + additionalVertices(parts[0], numBaseEdges) +
  ...
  for (auto it = basePositionInfo.begin(); it != basePositionInfo.end(); ++it)
    generateShiftedBase(numBaseEdges, baseRadius, it->first - m_height, semi * it->second, verts);
  ...
  for (size_t p = 0; p < nParts; p++)
  {
    switch (parts[p])
    {
      ...
      case UPWARDS_CUPOLA:
        ...
        // vertices.
        insertCupola(
            numBaseEdges, basePositionInfo.rbegin()->second * semi, baseRadius, edgeLength, false,
      ...
      case DOWNWARDS_CUPOLA:
        ...
        // vertices.
        insertCupola(
            numBaseEdges, basePositionInfo[0].second * semi, baseRadius, edgeLength, false, false,
            endBase, verts, faces);
        break;
      case ROTATED_UPWARDS_CUPOLA:
        ...
        insertCupola(
            numBaseEdges, basePositionInfo.rbegin()->second * semi, baseRadius, edgeLength, true,
            true, initialBase, verts, faces);
        break;
      case ROTATED_DOWNWARDS_CUPOLA:
        ...
        insertCupola(
            numBaseEdges, basePositionInfo[0].second * semi, baseRadius, edgeLength, true, false,
            endBase, verts, faces);
        break;
      case PRISM:
        // Archimedean prism.
      ...
      case UPWARDS_ROTUNDA:
        ...
        // exactly 10 vertices.
        insertRotunda(
            basePositionInfo.rbegin()->second * semi, baseRadius, false, true, initialBase, verts,
            faces);
        break;
      case DOWNWARDS_ROTUNDA:
        ...
        // exactly 10 vertices.
        insertRotunda(
            basePositionInfo[0].second * semi, baseRadius, false, false, endBase, verts, faces);
        break;
      case ROTATED_UPWARDS_ROTUNDA:
        // Upwards-pointing, slightly rotated, pentagonal rotunda. Only
        // for bases of exactly 10 vertices.
        insertRotunda(
            basePositionInfo.rbegin()->second * semi, baseRadius, true, true, initialBase, verts,
            faces);
        break;
      case ROTATED_DOWNWARDS_ROTUNDA:
        ...
        insertRotunda(
            basePositionInfo[0].second * semi, baseRadius, true, false, endBase, verts, faces);
        break;
      default:
...
void CPolyhedron::getEdgesLength(std::vector<double>& lengths) const
{
  ...
  for (auto it = m_Edges.begin(); it != m_Edges.end(); ++it, ++it2) *it2 = it->length(m_Vertices);
}
...
void CPolyhedron::getFacesArea(std::vector<double>& areas) const
{
  ...
  for (auto it = m_Faces.begin(); it != m_Faces.end(); ++it, ++it2) *it2 = it->area(m_Vertices);
}
...
void CPolyhedron::getSetOfPolygonsAbsolute(std::vector<math::TPolygon3D>& vec) const
{
  ...
  transform(
      m_Faces.begin(), m_Faces.end(), vec.begin(), FCreatePolygonFromFace<TPolygon3D>(nVerts));
}
...
void CPolyhedron::makeConvexPolygons()
{
  ...
  {
    if (!setNormal(mFace, false)) throw std::logic_error("Bad face specification");
    addEdges(mFace);
  }
}
  ...
  switch (mrpt::random::getRandomGenerator().drawUniform32bit() % 34)
  {
    ...
    case 23:
      return CreateArchimedeanRegularPrism(
          (mrpt::random::getRandomGenerator().drawUniform32bit() % 10) + 3, radius);
    case 24:
      return CreateArchimedeanRegularAntiprism(
          (mrpt::random::getRandomGenerator().drawUniform32bit() % 10) + 3, radius);
    case 25:
      return CreateJohnsonSolidWithConstantBase(
          ((mrpt::random::getRandomGenerator().drawUniform32bit() % 4) << 1) + 4, radius, "C+");
    case 26:
      return CreateJohnsonSolidWithConstantBase(
          ((mrpt::random::getRandomGenerator().drawUniform32bit() % 4) << 1) + 4, radius, "C-C+");
    case 27:
      return CreateJohnsonSolidWithConstantBase(
          ((mrpt::random::getRandomGenerator().drawUniform32bit() % 4) << 1) + 4, radius, "C-PRC+");
    case 28:
      return CreateJohnsonSolidWithConstantBase(
          ((mrpt::random::getRandomGenerator().drawUniform32bit() % 4) << 1) + 4, radius, "C-AC+");
    case 29:
    ...
    case 32:
      return CreateCatalanTrapezohedron(
          (mrpt::random::getRandomGenerator().drawUniform32bit() % 5) + 3, radius);
    case 33:
...
CPolyhedron::Ptr CPolyhedron::getDual() const
{
  ...
  for (size_t i = 0; i < NV; i++)
  {
    ...
    for (auto it = m_Faces[i].vertices.begin(); it != m_Faces[i].vertices.end(); ++it)
    {
      incidence(i, *it) = true;
  ...
  for (size_t i = 0; i < NE; i++)
    for (size_t j = 0; j < NV; j++) arrayEF(i, j) = faceContainsEdge(m_Faces[j], m_Edges[i]);
  for (auto& it : faces)
  {
    ...
    for (size_t i = 0; i < NV; i++)
    {
      ...
      for (size_t j = 1; j < sf - 1; j++)
      {
        ...
        for (;;)
        {
          ...
          if (searchForFace(m_Faces, i, (e1.v1 == i) ? e1.v2 : e1.v1, (e2.v1 == i) ? e2.v2 : e2.v1))
            break;
          uint32_t tmpV = f[j];
          f.erase(f.begin() + j);
          f.push_back(tmpV);
...
CPolyhedron::Ptr CPolyhedron::cantellate(double factor) const
{
  ...
  for (size_t i = 0; i < NF; i++)
  {
    ...
    for (size_t j = 0; j < oPS; j++)
    {
      ...
      size_t edge;
      searchForEdge(m_Edges, curr, m_Faces[i].vertices[(j + oPS - 1) % oPS], edge);
      faces[NF + NV + edge].vertices.push_back(j + ind);
      searchForEdge(m_Edges, curr, m_Faces[i].vertices[(j + 1) % oPS], edge);
      faces[NF + NV + edge].vertices.push_back(j + ind);
    }
    ind += oPS;
  }
  auto begin = faces.begin(), edgeBegin = faces.begin() + NF + NV, end = faces.end();
...
CPolyhedron::Ptr CPolyhedron::augment(double height) const
{
  ...
  for (size_t i = 0; i < NF; i++)
  {
    ...
    if (pTmp.evaluatePoint(phCenter) > 0)
      for (size_t j = 0; j < 3; j++) vertex[j] = cTmp[j] - height * pTmp.coefs[j];
    else
      for (size_t j = 0; j < 3; j++) vertex[j] = cTmp[j] + height * pTmp.coefs[j];
...
void CPolyhedron::generateBase(
    uint32_t numBaseEdges, double baseRadius, double height, vector<TPoint3D>& vec)
{
  for (size_t i = 0; i < numBaseEdges; i++)
  {
    double ang = 2 * M_PI * i / numBaseEdges;
...
void CPolyhedron::generateShiftedBase(
    uint32_t numBaseEdges, double baseRadius, double height, double shift, vector<TPoint3D>& vec)
{
  for (size_t i = 0; i < numBaseEdges; i++)
  {
...
void CPolyhedron::addEdges(const TPolyhedronFace& f)
{
  ...
  {
    e.v2 = *it;
    if (find(m_Edges.begin(), m_Edges.end(), e) == m_Edges.end()) m_Edges.push_back(e);
    e.v1 = e.v2;
    ++it;
  }
...
size_t CPolyhedron::facesInVertex(size_t vertex) const
{
  size_t res = 0;
  for (const auto& mFace : m_Faces)
    if (find(mFace.vertices.begin(), mFace.vertices.end(), vertex) != mFace.vertices.end()) res++;
  return res;
}

CArchive& mrpt::viz::operator>>(CArchive& in, CPolyhedron::TPolyhedronEdge& o)
{
  in >> o.v1 >> o.v2;
  return in;
}

CArchive& mrpt::viz::operator<<(CArchive& out, const CPolyhedron::TPolyhedronEdge& o)
{
  out << o.v1 << o.v2;
  return out;
}

CArchive& mrpt::viz::operator>>(CArchive& in, CPolyhedron::TPolyhedronFace& o)
{
  in >> o.vertices >> o.normal[0] >> o.normal[1] >> o.normal[2];
  return in;
}

CArchive& mrpt::viz::operator<<(CArchive& out, const CPolyhedron::TPolyhedronFace& o)
{
  out << o.vertices << o.normal[0] << o.normal[1] << o.normal[2];
  return out;
}

uint8_t CPolyhedron::serializeGetVersion() const { return 2; }
void CPolyhedron::serializeTo(mrpt::serialization::CArchive& out) const
{
  writeToStreamRender(out);
  // version 0
  out << m_Vertices << m_Faces << m_Wireframe;
...
void CPolyhedron::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  switch (version)
  {
    case 0:
    case 1:
      THROW_EXCEPTION("Old version deserialization not supported");
      break;

    case 2:
      readFromStreamRender(in);
      in >> m_Vertices >> m_Faces >> m_Wireframe;
      ...
      for (auto& mFace : m_Faces)
      {
        if (!setNormal(mFace)) throw std::logic_error("Bad face specification");
        addEdges(mFace);
      }
...
auto CPolyhedron::internalBoundingBoxLocal() const -> mrpt::math::TBoundingBoxf
{
  ...
  for (const auto& pt : m_Vertices) bb.updateWithPoint(pt);
  return bb;
}
...
{
  return CreateJohnsonSolidWithConstantBase(
      8, radius / sqrt(1 + square(sin(M_PI / 8))), type ? "C-PRC+" : "GC-PRC+", 3);
}
    ...
    uint32_t numBaseEdges, double baseRadius, double height)
{
  return CreateCustomAntiprism(
      generateBase(numBaseEdges, baseRadius), generateShiftedBase(numBaseEdges, baseRadius),
      height);
}
