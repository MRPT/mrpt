...
#include "viz-precomp.h"  // Precompiled headers
//
...
{ /* description of an element */
  ...
  int other_offset = NO_OTHER_PROPS; /* offset to un-asked-for props, or -1 if none*/
...
PlyFile* ply_open_for_writing(
    const char* name, const vector<string>& elem_names, int file_type, float* version)
{
  PlyFile* plyfile;
  FILE* fp;
...
void ply_describe_element(
    PlyFile* plyfile, const string& elem_name, int nelems, vector<PlyProperty>& prop_list)
{
...
void ply_header_complete(PlyFile* plyfile)
{
  ...
  for (auto& comment : plyfile->comments) fprintf(fp, "comment %s\n", comment.c_str());
...
PlyFile* ply_open_for_reading(
    const char* filename, vector<string>& elem_names, int* file_type, float* version)
{
  FILE* fp;
  PlyFile* plyfile;
...
void ply_get_comments(PlyFile* plyfile, vector<string>& comments) { comments = plyfile->comments; }
...
void ply_get_obj_info(PlyFile* plyfile, vector<string>& obj_info) { obj_info = plyfile->obj_info; }
...
void ascii_get_element(PlyFile* plyfile, char* elem_ptr)
{
  ...
  if (words.empty()) throw std::runtime_error("ply_get_element: unexpected end of file");
  ...
  for (size_t j = 0; j < elem->props.size(); j++)
  {
    ...
    { /* a list */
      ...
      get_ascii_item(
          words[which_word++].c_str(), prop->count_external, &int_val, &uint_val, &double_val);
      if (store_it)
      {
        item = elem_data + prop->count_offset;
      ...
      else
      {
        ...
        {
          get_ascii_item(
              words[which_word++].c_str(), prop->external_type, &int_val, &uint_val, &double_val);
          if (store_it)
          {
...
int get_binary_item(
    FILE* fp, int bin_file_type, int type, int* int_val, unsigned int* uint_val, double* double_val)
{
  char c[8];
  void* ptr;
  ...
  try
  {
    ...
    for (const auto& elem_name : elist)
    {
      ...
      for (int j = 0; j < num_elems; j++)
      {
        ...
        // timestamp?
        if (pt.timestamp != VAL_NOT_SET) this->PLY_import_set_vertex_timestamp(j, pt.timestamp);
      }
    }
    ...
    const std::vector<std::string>& file_obj_info) const
{
  try
  {
    ...
    for (const auto& face_prop : face_props) ply_describe_property(ply, "face", &face_prop);

    /* write a comment and an object information field */
    for (const auto& file_comment : file_comments) ply_put_comment(ply, file_comment.c_str());
