...
void CSetOfObjects::insert(const CVisualObject::Ptr& newObject)
{
...
void CSetOfObjects::dumpListOfObjects(std::vector<std::string>& lst) const
{
  for (auto& o : m_objects)
  {
    ...
    if (o->GetRuntimeClass() == CLASS_ID_NAMESPACE(CSetOfObjects, mrpt::viz))
    {
...
/*--------------------------------------------------------------
          removeObject
  ---------------------------------------------------------------*/
void CSetOfObjects::removeObject(const CVisualObject::Ptr& obj)
{
...
bool CSetOfObjects::contains(const CVisualObject::Ptr& obj) const
{
  return find(m_objects.begin(), m_objects.end(), obj) != m_objects.end();
}

CVisualObject& CSetOfObjects::setColor_u8(const mrpt::img::TColor& c)
{
  {
...
CVisualObject& CSetOfObjects::setColorA_u8(const uint8_t a)
{
  {
    std::unique_lock<std::shared_mutex> lckWrite(m_stateMtx.data);
    m_state.color.A = a;
...
auto CSetOfObjects::internalBoundingBoxLocal() const -> mrpt::math::TBoundingBoxf
{
  ...
  for (const auto& o : m_objects)
  {
    ...
    const mrpt::math::TBoundingBoxf childBBf(
        childBB.min.cast<float>(), childBB.max.cast<float>());
    if (first)
    {
      bb = childBBf;
      first = false;
