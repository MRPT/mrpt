...
CSetOfObjects::Ptr CSetOfObjects::posePDF2opengl(const CPosePDF& o)
{
  ...
  if (IS_CLASS(o, CPosePDFSOG))
  {
    ...
    viz::CSetOfLines::Ptr lins = std::make_shared<viz::CSetOfLines>();
    lins->setColor(0.0f, 0.0f, 1.0f, 0.6f);
    lins->setLineWidth(POSE_TAIL_WIDTH);

    for (const auto& it : *p)
    {
      auto ellip = mrpt::viz::CEllipsoid2D::Create();
      ...
      lins->appendLine(
          it.mean.x(), it.mean.y(), 0, it.mean.x() + POSE_TAIL_LENGTH * cos(it.mean.phi()),
  ...
  else if (IS_CLASS(o, CPosePDFGaussian))
  {
    ...
    lins->appendLine(
        p->mean.x(), p->mean.y(), 0, p->mean.x() + POSE_TAIL_LENGTH * cos(p->mean.phi()),
  ...
  else if (IS_CLASS(o, CPosePDFParticles))
  {
    ...
    auto pnts = viz::CPointCloud::Create();
    pnts->setColor(POSE_COLOR, 0.6f);
    pnts->setPointSize(POSE_POINT_SIZE);

    auto lins = viz::CSetOfLines::Create();
    lins->setColor(POSE_COLOR, 0.6f);
    lins->setLineWidth(POSE_TAIL_WIDTH);
...
CSetOfObjects::Ptr CSetOfObjects::posePDF2opengl(const CPointPDF& o)
{
  ...
  else if (IS_CLASS(o, CPointPDFParticles))
  {
    ...
    for (size_t i = 0; i < N; i++)
      obj->setPoint(i, p->m_particles[i].d->x, p->m_particles[i].d->y, p->m_particles[i].d->z);
    outObj->insert(obj);
  }
...
CSetOfObjects::Ptr CSetOfObjects::posePDF2opengl(const CPose3DPDF& o)
{
  ...
  if (IS_CLASS(o, CPose3DPDFSOG))
  {
    ...
    for (const auto& it : *p)
    {
      ...
      viz::CSetOfObjects::Ptr axes = viz::stock_objects::CornerXYZ();
  ...
  else if (IS_CLASS(o, CPose3DPDFGaussian))
  {
    ...
    auto obj = std::make_shared<viz::CEllipsoid3D>();
  ...
  else if (IS_CLASS(o, CPose3DPDFParticles))
  {
    ...
    for (size_t i = 0; i < p->size(); i++)
    {
      viz::CSetOfObjects::Ptr axes = viz::stock_objects::CornerXYZSimple(POSE_AXIS_SCALE);
