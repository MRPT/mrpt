...
DECLARE_OP_FUNCTION(op_camera_params)
{
  ...
  class CRawlogProcessor_CamParams : public CRawlogProcessorOnEachObservation
  {
   ...
   public:
    ...
    CRawlogProcessor_CamParams(CCompressedInputStream& in_rawlog, CLI::App& cmdline, bool Verbose) :
        CRawlogProcessorOnEachObservation(in_rawlog, cmdline, Verbose)
    {
      ...
      if (!fileExists(fil))
        THROW_EXCEPTION_FMT("--camera-params op: config file can't be open: '%s'", fil.c_str());
      ...
      if (cfg.sectionExists("CAMERA_PARAMS"))
      {
        ...
        VERBOSE_COUT << "Found camera configuration file type: Monocular.\n";
      }
      else if (cfg.sectionExists("CAMERA_PARAMS_LEFT") && cfg.sectionExists("CAMERA_PARAMS_RIGHT"))
      {
        // Stereo:
        stereoCam.emplace();
        stereoCam->loadFromConfigFile("CAMERA_PARAMS", cfg);

        VERBOSE_COUT << "Found camera configuration file type: Stereo.\n";
      }
      else if (cfg.sectionExists("DEPTH_CAM_PARAMS") && cfg.sectionExists("INTENSITY_CAM_PARAMS"))
      {
        // Depth:
        depthCam.emplace();
        ...
        VERBOSE_COUT << "Found camera configuration file type: Depth.\n";
      }
      else
      {
        THROW_EXCEPTION(
    ...
    bool processOneObservation(CObservation::Ptr& obs) override
    {
      ...
      else if (auto obsStereo = std::dynamic_pointer_cast<CObservationStereoImages>(obs); obsStereo)
      {
        obsStereo->setStereoCameraParams(*stereoCam);
        m_changedCams++;
      }
      else if (auto obsDepth = std::dynamic_pointer_cast<CObservation3DRangeScan>(obs); obsDepth)
      {
