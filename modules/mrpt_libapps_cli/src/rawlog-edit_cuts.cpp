...
DECLARE_OP_FUNCTION(op_cut)
{
  ...
  class CRawlogProcessor_Cut : public CRawlogProcessorFilterObservations
  {
   ...
   public:
        ...
        m_to_time(0)
    {
      ...
      if (verbose)
      {
        if (has_from_index) std::cout << "Using cut filter: from-index=" << m_from_index << "\n";
        if (has_to_index) std::cout << "Using cut filter:   to-index=" << m_to_index << "\n";
        if (has_from_time)
          std::cout << "Using cut filter: from-time ="
                    << dateTimeLocalToString(mrpt::Clock::fromDouble(m_from_time)) << "\n";
        if (has_to_time)
          std::cout << "Using cut filter:   to-time ="
  ...
  CRawlogProcessor_Cut proc(in_rawlog, cmdline, verbose, outrawlog.out_rawlog_io);
