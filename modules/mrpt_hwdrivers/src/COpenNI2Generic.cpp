...
bool setONI2StreamMode(
    openni::VideoStream& stream, int w, int h, int fps, openni::PixelFormat format);
...
#if MRPT_HAS_OPENNI2
    :
    m_rgb_format(openni::PIXEL_FORMAT_RGB888), m_depth_format(openni::PIXEL_FORMAT_DEPTH_1_MM)
#endif
{
...
#if MRPT_HAS_OPENNI2
    THROW_EXCEPTION(
        mrpt::format("After initialization:\n %s\n", openni::OpenNI::getExtendedError()));
#endif
  }
}
...
bool COpenNI2Generic::getDeviceIDFromSerialNum(
    [[maybe_unused]] const unsigned int SerialRequired, [[maybe_unused]] int& sensor_id) const
{
#if MRPT_HAS_OPENNI2
  std::lock_guard<std::recursive_mutex> lock(vDevices_mx);
...
void COpenNI2Generic::getNextFrameRGB(
    ...
    [[maybe_unused]] bool& there_is_obs,
    [[maybe_unused]] bool& hardware_error,
    [[maybe_unused]] unsigned sensor_id)
{
  ...
  if (vDevices[sensor_id]->getNextFrameRGB(rgb_img, timestamp, there_is_obs, hardware_error) ==
      false)
  {
    showLog(mrpt::format("[%s]\n", __FUNCTION__));
    ...
    [[maybe_unused]] unsigned sensor_id)
{
  ...
  if (vDevices[sensor_id]->getNextFrameD(depth_img_mm, timestamp, there_is_obs, hardware_error) ==
      false)
  {
    showLog(mrpt::format("[%s]\n", __FUNCTION__));
    showLog(mrpt::format(" Error [%d]th Sensor.\n", sensor_id));
...
bool COpenNI2Generic::getColorSensorParam(
    [[maybe_unused]] mrpt::img::TCamera& param, [[maybe_unused]] unsigned sensor_id) const
{
#if MRPT_HAS_OPENNI2
  std::lock_guard<std::recursive_mutex> lock(vDevices_mx);
...
std::string oni2DevInfoStr(const openni::DeviceInfo& info, int tab)
{
  ...
  sst << space << "name=" << info.getName() << "\n";
  sst << space << "uri=" << info.getUri() << "\n";
  sst << space << "vendor=" << info.getVendor() << "\n";
...
bool COpenNI2Generic::CDevice::synchMirrorMode()
{
  ...
  for (auto& m_stream : m_streams)
  {
    ...
    if (mirror_support == false)
    {
      m_log << "[" << __FUNCTION__ << "]"
            << "\n";
      m_log << " openni::STREAM_PROPERTY_MIRRORING is not supported on " << m_stream->getName()
            << "."
            << "\n";
...
bool COpenNI2Generic::CDevice::open(int w, int h, int fps)
{
  ...
  if (m_streams[DEPTH_STREAM])
  {
    ...
    m_log << " Close range: " << (CloseRange ? "On" : "Off") << "\n";
  }
  ...
  if (m_device.isImageRegistrationModeSupported(openni::IMAGE_REGISTRATION_DEPTH_TO_COLOR) &&
      m_streams[DEPTH_STREAM] && m_streams[DEPTH_STREAM]->isValid() && m_streams[COLOR_STREAM] &&
      m_streams[COLOR_STREAM]->isValid())
  {
    // SEB
...
bool COpenNI2Generic::CDevice::getNextFrameD(
    mrpt::math::CMatrix_u16& depth_mm,
    mrpt::system::TTimeStamp& timestamp,
    bool& there_is_obs,
    bool& hardware_error)
{
...
bool COpenNI2Generic::CDevice::getNextFrameRGBD(
    mrpt::obs::CObservation3DRangeScan& obs, bool& there_is_obs, bool& hardware_error)
{
  MRPT_START
  clearLog();
  there_is_obs = false;
...
bool COpenNI2Generic::CDevice::CStream::setMirror(bool flag)
{
  ...
  if (m_stream.setMirroringEnabled(flag) != openni::STATUS_OK)
  {
    m_log << "[" << __FUNCTION__ << "]"
          << "\n"
          << " setMirroringEnabled() failed: " << openni::OpenNI::getExtendedError() << "\n";
    return false;
  }
  return true;
}

bool COpenNI2Generic::CDevice::CStream::isValid() const { return m_stream.isValid(); }
...
bool COpenNI2Generic::CDevice::CStream::open(int w, int h, int fps)
{
  ...
  if (rc != openni::STATUS_OK)
  {
    m_log << "[" << __FUNCTION__ << "]"
          << "\n"
          << " Couldn't find sensor " << m_strName << ":" << openni::OpenNI::getExtendedError()
          << "\n";
    if (m_type == openni::SENSOR_COLOR)
    {
      m_type = openni::SENSOR_IR;
  ...
  m_log << " "
        << mrpt::format(
               "-> (%d, %d) FPS %d Format %d", options.getResolutionX(), options.getResolutionY(),
...
bool COpenNI2Generic::CDevice::CStream::start()
{
  ...
  if (m_stream.start() != openni::STATUS_OK)
  {
    m_log << "[" << __FUNCTION__ << "]"
          << "\n"
          << " Couldn't start " << getName() << " stream:" << openni::OpenNI::getExtendedError()
          << "\n";
    this->destroy();
    return false;
  }
...
COpenNI2Generic::CDevice::CStream::Ptr COpenNI2Generic::CDevice::CStream::create(
    openni::Device& device,
    openni::SensorType type,
    openni::PixelFormat format,
    std::ostream& log,
