...
{
#if MRPT_HAS_MYNTEYE_D
  std::shared_ptr<mynteyed::Camera> cam = std::make_shared<mynteyed::Camera>();
    ...
    m_capture(mrpt::make_impl<CMyntEyeCamera::Impl>())
{
  ...
  mynteyed::util::print_stream_infos(*m_capture->cam, m_capture->dev_info.index);
  ...
  mynteyed::StreamIntrinsics si = m_capture->cam->GetStreamIntrinsics(stream_mode);
...
bool CMyntEyeCamera::getObservation(mrpt::obs::CObservation3DRangeScan& out)
{
  ...
  {
    if (!image_color.img)
      image_color = m_capture->cam->GetStreamData(mynteyed::ImageType::IMAGE_LEFT_COLOR);

    if (!image_depth.img)
      image_depth = m_capture->cam->GetStreamData(mynteyed::ImageType::IMAGE_DEPTH);
  ...
  if (image_color.img)
  {
    ...
    ASSERT_EQUAL_(i->data_size(), static_cast<size_t>(i->width() * i->height() * 3));
  ...
  if (image_depth.img)
  {
    ...
    ASSERT_EQUAL_(i->data_size(), static_cast<size_t>(i->width() * i->height() * 2));
