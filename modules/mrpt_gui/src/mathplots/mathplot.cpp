...
// %d x %d"), m_winX, m_winY, scrx, scry);
      ...
      if (m_winX != 1)
      {
        m_dim.x = static_cast<int>(floor(static_cast<double>(m_dim.x * scrx) / m_winX));
      }
      if (m_winY != 1)
      {
        m_dim.y = static_cast<int>(floor(static_cast<double>(m_dim.y * scry) / m_winY));
        UpdateReference();
      }
...
void mpInfoCoords::UpdateInfo(mpWindow& w, wxEvent& event)
{
  ...
  {
    int mouseX = static_cast<wxMouseEvent&>(event).GetX();
    int mouseY = static_cast<wxMouseEvent&>(event).GetY();
...
// label.c_str());
    ...
    {  // Don't draw any thing if there are no
       // visible layers
  ...
  if (m_visible)
  {
    ...
    if (!m_name.IsEmpty() && m_showName)
    {
      ...
      */
      ...
      dc.DrawText(
          m_name, tx,
          w.y2p(GetY(w.p2x(tx))));  // (wxCoord) ((w.GetPosY()
                                    // - GetY( (double)tx /
                                    // w.GetScaleX() +
                                    // w.GetPosX())) *
                                    // w.GetScaleY()) );
...
void mpProfile::Plot(wxDC& dc, mpWindow& w)
{
  if (m_visible)
  {
    ...
    for (wxCoord i = startPx; i < endPx; ++i)
    {
      ...
      // static_cast<double>(i+1) / w.GetXscl() +
      //(w.GetXpos() ) ) ) * w.GetYscl());
...
void mpScaleX::Plot(wxDC& dc, mpWindow& w)
{
  if (m_visible)
  {
    ...
    {
      orgy = w.y2p(0);  // static_cast<int>(w.GetPosY() * w.GetScaleY());
    }
    ...
    const double end = w.GetPosX() + static_cast<double>(extend) / w.GetScaleX();

    wxCoord tx, ty;
    wxString s;
    wxString fmt;
    int tmp = static_cast<int>(dig);
    ...
    // double n = floor( (w.GetPosX() - static_cast<double>(extend) / w.GetScaleX()) /
    // step ) * step ;
    double n0 =
			floor(
				(w.GetPosX() /* - static_cast<double>(extend - w.GetMarginLeft() - w.GetMarginRight())/ w.GetScaleX() */) /
				step) *
			step;
    double n = 0;
#ifdef MATHPLOT_DO_LOGGING
    ...
    {
      const int p = static_cast<int>((n - w.GetPosX()) * w.GetScaleX());
#ifdef MATHPLOT_DO_LOGGING
      ...
      if ((p >= startPx) && (p <= endPx))
      {
        ...
        else if (m_labelType == mpX_DATETIME)
        {
          ...
          s.Printf(
              ...
              static_cast<double>(tm.tm_min), static_cast<double>(tm.tm_sec));
        }
        else if (m_labelType == mpX_DATE)
        {
          auto when = (time_t)n;
        ...
        else if ((m_labelType == mpX_TIME) || (m_labelType == mpX_HOURS))
        {
          ...
          if (fmt.Len() == 20)
          {  // Format with hours has 11 chars
    ...
    double labelStep =
        ceil((maxExtent + mpMIN_X_AXIS_LABEL_SEPARATION) / (w.GetScaleX() * step)) * step;
...
void mpScaleY::Plot(wxDC& dc, mpWindow& w)
{
  if (m_visible)
  {
    ...
    if (m_flags == mpALIGN_CENTER)
    {
      orgx = w.x2p(0);  // static_cast<int>(w.GetPosX() * w.GetScaleX());
    }
    ...
    const double step = exp(mpLN10 * dig);
    const double end = w.GetPosY() + static_cast<double>(extend) / w.GetScaleY();
    ...
    double n =
        floor(
            (w.GetPosY() -
             static_cast<double>(extend - w.GetMarginTop() - w.GetMarginBottom()) / w.GetScaleY()) /
            step) *
        step;
    ...
    for (; n < end; n += step)
    {
      const int p = static_cast<int>((w.GetPosY() - n) * w.GetScaleY());
...
mpWindow::mpWindow(
    wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long flag) :
    wxWindow(parent, id, pos, size, flag, wxT("mathplot"))
{
  ...
  m_popmenu.AppendCheckItem(
      mpID_LOCKASPECT, _("Lock aspect"), _("Lock horizontal and vertical zoom aspect."));
  m_popmenu.Append(mpID_PRINT, _("Print..."), _("Allows printing the graph."));
  m_popmenu.Append(
      mpID_HELP_MOUSE, _("Show mouse commands..."), _("Show help about the mouse commands."));
...
#ifdef MATHPLOT_DO_LOGGING
    wxLogMessage(
        _("[mpWindow::OnMouseMove] Ax:%i Ay:%i m_posX:%f m_posY:%f"), Ax, Ay, m_posX, m_posY);
#endif
  }
  else
  {
    if (event.m_leftDown)
    {
      if (m_movingInfoLayer == nullptr)
      {
        ...
        m_dottedWindowSize = {event.GetX() - m_mouseLClick_X, event.GetY() - m_mouseLClick_Y};
      }
      else
      {
...
// JL
void mpWindow::Fit(
    double xMin, double xMax, double yMin, double yMax, wxCoord* printSizeX, wxCoord* printSizeY)
{
...
void mpWindow::ZoomIn(const wxPoint& centerPoint)
{
  ...
  m_desiredXmin = m_posX;
  m_desiredXmax = m_posX + (m_scrX - m_marginLeft - m_marginRight) /
                               m_scaleX;  // m_desiredXmax = m_posX + m_scrX / m_scaleX;
  m_desiredYmax = m_posY;
  m_desiredYmin = m_posY - (m_scrY - m_marginTop - m_marginBottom) /
...
void mpWindow::OnPrintMenu(wxCommandEvent& WXUNUSED(event))
{
  ...
  wxPreviewFrame* frame =
      new wxPreviewFrame(preview, nullptr, wxT("Print Plot"), wxPoint(100, 100), wxSize(600, 650));
  frame->Centre(wxBOTH);
  frame->Initialize();
  frame->Show();
}
...
void mpWindow::OnCenter(wxCommandEvent& WXUNUSED(event))
{
  ...
  // static_cast<double>(m_clickedX-m_scrX/2) / m_scaleX + m_posX,
  // static_cast<double>(m_scrY/2-m_clickedY) / m_scaleY + m_posY);
}
...
void mpWindow::OnPaint(wxPaintEvent&)
{
  ...
  if (m_drawDottedSelectedWindow)
  {
    ...
    dc.DrawRectangle(m_mouseLClick_X, m_mouseLClick_Y, m_dottedWindowSize.x, m_dottedWindowSize.y);
  }
  ...
  if (UpdateBBox())
  {
    if (m_enableScrollBars)
    {
      ...
      // Do x scroll bar
      {
        ...
        int sizeX = static_cast<int>((maxX - minX) * m_scaleX);
        int thumbX = static_cast<int>(((m_posX + leftMargin) - minX) * m_scaleX);
      ...
      // Do y scroll bar
      {
        ...
        // Calculate scroll bar size and thumb position
        int sizeY = static_cast<int>((maxY - minY) * m_scaleY);
        int thumbY = static_cast<int>((maxY - (m_posY - topMargin)) * m_scaleY);
...
void mpWindow::SetColourTheme(
    const wxColour& bgColour, const wxColour& drawColour, const wxColour& axesColour)
{
...
void mpText::Plot(wxDC& dc, mpWindow& w)
{
  if (m_visible)
  {
    ...
    /*    dc.DrawText( GetName(),
      static_cast<int>((((float)width/100.0) * m_offsety) + left - (tw/2)),
      static_cast<int>((((float)height/100.0) * m_offsetx) - bottom) );*/
...
bool mpPrintout::OnPrintPage(int page)
{
  ...
  if ((trgDc) && (page == 1))
  {
    ...
    plotWindow->Fit(
        plotWindow->GetDesiredXmin(), plotWindow->GetDesiredXmax(), plotWindow->GetDesiredYmin(),
    ...
    plotWindow->Fit(
        plotWindow->GetDesiredXmin(), plotWindow->GetDesiredXmax(), plotWindow->GetDesiredYmin(),
        plotWindow->GetDesiredYmax(), nullptr, nullptr);
...
void mpPolygon::setPoints(
    const std::vector<double>& points_xs, const std::vector<double>& points_ys, bool closedShape)
{
...
void mpPolygon::setPoints(
    const std::vector<float>& points_xs, const std::vector<float>& points_ys, bool closedShape)
{
  if (points_xs.size() != points_ys.size())
  {
    wxLogError(
  ...
  else
  {
    ...
    if (!points_xs.empty())
    {
      ...
      for (itX = points_xs.begin(), itY = points_ys.begin(), itXo = m_shape_xs.begin(),
          itYo = m_shape_ys.begin();
           itX != points_xs.end(); ++itX, ++itY, ++itXo, ++itYo)
      {
        *itXo = static_cast<double>(*itX);
        *itYo = static_cast<double>(*itY);
      }

      if (closedShape)
      {
        m_shape_xs.push_back(static_cast<double>(points_xs[0]));
        m_shape_ys.push_back(static_cast<double>(points_ys[0]));
      }
    }
    ...
    */
...
#ifdef MATHPLOT_DO_LOGGING
    wxLogMessage(
        _("[mpBitmapLayer::Plot] screenPixel: x=%f y=%f  d_width=%ix%i"), screenPixelX,
        screenPixelY, d_width, d_height);
    wxLogMessage(
        _("[mpBitmapLayer::Plot] offset: x=%i y=%i  bmpWidth=%ix%i"), offset_x, offset_y, b_width,
        b_height);
#endif
    ...
    if (d_width > 0 && d_height > 0)
    {
          ...
          m_scaledBitmap_offset_x != offset_x || m_scaledBitmap_offset_y != offset_y)
      {
        ...
        m_scaledBitmap =
            wxBitmap(wxBitmap(m_bitmap).GetSubBitmap(r).ConvertToImage().Scale(d_width, d_height));
