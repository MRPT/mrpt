...
void CDisplayWindowGUI::drawContents()
{
  ...
  try
  {
    ...
    if (!m_compiledScene || lastPtr.get() != scenePtr.get())
    {
      ...
      m_compiledScene->compile(*background_scene);
...
void CDisplayWindowGUI::setWindowTitle(const std::string& str) { Screen::setCaption(str); }
...
void CDisplayWindowGUI::setIconFromData(
    const char* imgData, unsigned int width, unsigned int height, const uint8_t transp)
{
...
#define HEADER_PIXEL(data, pixel)                                       \
  ...
  for (unsigned int y = 0; y < height; y++)
  {
    for (unsigned int x = 0; x < width; x++)
    {
      ...
      HEADER_PIXEL(in, out);
      // alpha
      out[3] = (out[0] == transp && out[1] == transp && out[2] == transp) ? 0x00 : 0xff;
      out += 4;
    }
  }
...
bool CDisplayWindowGUI::keyboardEvent(int key, int scancode, int action, int modifiers)
{
...
nanogui::Window* CDisplayWindowGUI::createManagedSubWindow(const std::string& title)
{
  ...
  w->buttonPanel()
      ...
      ->setCallback([this, thisWinIndex]() { m_subWindows.minimize(thisWinIndex); });
...
void CDisplayWindowGUI::createSubWindowsControlUI()
{
  ...
  w->setLayout(
      new nanogui::BoxLayout(nanogui::Orientation::Vertical, nanogui::Alignment::Fill, 1, 1));
  ...
  auto btnUnhide = page2->add<nanogui::Button>("", ENTYPO_ICON_CHEVRON_WITH_CIRCLE_RIGHT);
  ...
  m_subWindows.uiCombo->setCallback(
      [this](int index)
      {
  ...
  btnMin->setCallback([this]() { m_subWindows.minimize(m_subWindows.uiCombo->selectedIndex()); });
  btnMin->setTooltip("Minimize");

  auto btnMax = pn->add<nanogui::Button>("", ENTYPO_ICON_BROWSER);
  btnMax->setCallback([this]() { m_subWindows.restore(m_subWindows.uiCombo->selectedIndex()); });
  ...
  auto btnHide = pn->add<nanogui::Button>("", ENTYPO_ICON_CHEVRON_WITH_CIRCLE_LEFT);
  btnHide->setCallback(
      [=]()
      {
